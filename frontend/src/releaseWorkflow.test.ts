...
describe('release workflow contract', () => {
  it('builds and packages the Cargo binary targets that actually exist', () => {
    const packages = ['crates/server', 'crates/cinyuverse-mcp', 'crates/review'].map(
      (directory) => ({
        directory,
    ...
    expect(packages).toEqual([
      {
        ...
        packageName: 'server',
        binaryNames: ['cinyuverse-server'],
      },
      {
        directory: 'crates/cinyuverse-mcp',
        packageName: 'cinyuverse-mcp',
        binaryNames: ['cinyuverse-mcp'],
      },
      {
  ...
  it('uses the current server binary target for backend development', () => {
    ...
    expect(launcher).toContain('run --bin cinyuverse-server');
  ...
  it('publishes native desktop installers for version tags', () => {
    ...
    expect(workflow).toContain('cinyuverse.exe');
    expect(workflow).toContain('bundles: appimage');
    expect(workflow).toContain('bundles: app');
