...
export function PluginCatalogPage() {
  ...
  const installPlugin = useCallback(
    async (path: string) => {
      ...
      setImporting(true);
      try {
        const preview = await api.previewImport(path, false, 'cinyuverse');
        if (preview.conflict) {
  ...
  const confirmMarketplaceInstall = useCallback(async () => {
    ...
    try {
      ...
      const imported = path
        ? await api.import(path, false, already ? 'replace' : 'reject', 'cinyuverse')
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const listing: CatalogListing = {
                            owner: plugin.publisher ?? 'cinyuverse',
