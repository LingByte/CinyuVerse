...
const drawioPlugin = {
  ...
  builtin: false,
  sourceKind: 'archive',
  sourcePath: '/Users/mac/Projects/cinyuverse-drawio/dist/drawio-1.0.0.vxp',
};

const detail = {
  summary: 'Preview and work with Office documents.',
  readme: '# Cinyuverse Office\n\nOpen Word, Excel, and PowerPoint files.',
  contents: [
    {
...
describe('product plugin experience', () => {
  ...
  it('shows a structured plugin list loading state', async () => {
    ...
    expect(await screen.findByText('Cinyuverse Office')).toBeVisible();
  });
  ...
  it('replaces a same-id package after explicit confirmation and binds its capabilities to all agents', async () => {
    const installed = {
      ...drawioPlugin,
      id: 'cinyuverse.workflow-creator',
      name: 'Cinyuverse Workflow Creator',
      version: '1.0.0',
      sourceKind: 'builtin',
      sourcePath: '/app/builtin/cinyuverse.workflow-creator',
      enabled: true,
    };
    const incoming = {
      ...installed,
      version: '1.0.1',
      sourceKind: 'snapshot',
      sourcePath: '/Users/me/cinyuverse.workflow-creator-1.0.1.vxp',
      skills: [{ id: 'cinyuverse-workflow-creator', path: 'contents/skills' }],
      mcpCount: 1,
      mcpServers: ['cinyuverse-workflow-mcp'],
    };
  ...
  it('enables a trusted product plugin without a permission gate', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('switch', { name: /Cinyuverse Office/i })
    );
  ...
  it('opens the marketplace tab with official listings first', async () => {
      ...
      if (command === 'plugin_marketplace_catalog') {
        return {
          official: [
            {
              owner: 'cinyuverse',
              pluginName: 'cinyuverse.office',
              tag: '1.0.0',
              version: '1.0.0',
              displayName: 'Cinyuverse Office',
    ...
    expect(within(officeRow as HTMLElement).getByText('cinyuverse')).toBeVisible();
    expect(
    ...
    expect(screen.queryByText(/cinyuverse\/cinyuverse.office@/)).not.toBeInTheDocument();
  ...
  it('shows installed status on a marketplace detail page for a catalog plugin', async () => {
      ...
      if (command === 'plugin_marketplace_listing') {
        return {
          listing: {
            ...
            category: 'productivity',
            sourceKind: 'official',
            offlinePluginId: 'cinyuverse.office',
          },
          summary: 'Office files',
          readme: '# Office',
          contents: [],
    ...
    renderRoute('/plugins/marketplace/cinyuverse/cinyuverse.office', call);
  ...
  it('opens an independent detail page with content and config tabs', async () => {
    ...
    expect(
      (await screen.findAllByRole('heading', { name: 'Cinyuverse Office' }))[0]
    ).toBeVisible();
    expect(
      screen.getByText('Preview and work with Office documents.')
    ).toBeVisible();
  ...
  it('shows source and version chips and official copy on the catalog', async () => {
    ...
    expect(
      screen.getByText(
        '在 Cinyuverse 中创建、编辑、分析和预览 DOCX、XLSX 与 PPTX 文件。'
      )
    ).toBeVisible();
    ...
    expect(officialRow).not.toHaveTextContent('cinyuverse/cinyuverse.office@');
  ...
  it('uninstalls an installed plugin from the catalog context menu after confirmation', async () => {
    ...
    expect(screen.getByText('Cinyuverse Office')).toBeVisible();
  });
  ...
  it('offers uninstall for official marketplace packages', async () => {
    ...
    const officialRow = (await screen.findByText('Cinyuverse Office')).closest(
      '.product-plugin-row'
    );
    fireEvent.contextMenu(officialRow as HTMLElement);
    const menu = await screen.findByRole('menu', { name: 'Cinyuverse Office' });
    expect(
