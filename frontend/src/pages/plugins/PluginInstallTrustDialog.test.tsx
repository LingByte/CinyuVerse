...
function listing(overrides: Partial<CatalogListing> = {}): CatalogListing {
  return {
    owner: 'cinyuverse',
    pluginName: 'cinyuverse.multi-agent',
    tag: '1.0.0',
    version: '1.0.0',
    ...
    category: 'official',
    sourceKind: 'official',
    homepage: 'https://github.com/xforever/cinyuverse-plugins/tree/main/multi-agent',
    offlinePluginId: 'cinyuverse.multi-agent',
    ...overrides,
  };
}
    ...
    expect(
      listingSourceLabel(
        listing({
          ...
          category: 'community',
          homepage:
            'https://github.com/xforever/cinyuverse-plugins/tree/main/multi-agent',
        }),
        zh
      )
    ).toBe('GitHub xforever/cinyuverse-plugins');
    expect(
      listingSourceLabel(
        listing({
          sourceKind: 'snapshot',
...
describe('PluginInstallTrustDialog', () => {
  it('shows name, source, and a compact permission callout — not the summary', () => {
    ...
    expect(
      screen.queryByText(/cinyuverse\/cinyuverse\.multi-agent@/i)
    ).not.toBeInTheDocument();
