...
const FORMAT_LABELS: Record<string, string> = {
  cinyuverse: 'Cinyuverse',
  codex: 'Codex',
  claude_code: 'Claude Code',
};

export type PluginEcosystem = 'codex' | 'claude_code' | 'cinyuverse';
type ImportEcosystem = 'codex' | 'claude_code' | 'cinyuverse';

const PLUGIN_TABS: Array<{ id: PluginEcosystem; label: string }> = [
  { id: 'codex', label: 'Codex' },
  { id: 'claude_code', label: 'Claude Code' },
  { id: 'cinyuverse', label: 'Cinyuverse' },
];
...
const PLUGIN_LIST_PANE_STORAGE_KEY = 'cinyuverse.pluginHub.listPanePercent';
...
export function PluginsSettings({
  ...
  const selectedRegistryContributions = useMemo(
    () =>
      ecosystem === 'cinyuverse' && contributionCatalog && selected
  ...
  return (
    <div className="space-y-4">
      ...
      {effectiveTab === 'cinyuverse' && catalog?.runtimes.length ? (
        <SettingsSection
          icon={TerminalSquare}
      ...
      >
        <DialogContent className="max-w-xl">
          ...
          <div className="plugin-import-methods">
            ...
            {importEcosystem !== 'cinyuverse' && canUseCliImport ? (
            ...
            {importEcosystem === 'cinyuverse' ? (
              <div className="plugin-import-method-row">
                <Archive aria-hidden="true" />
                <div>
                  <strong>{t('plugins.cinyuverseZipTitle')}</strong>
                  <p>{t('plugins.cinyuverseZipDescription')}</p>
                </div>
                <Button
                  size="sm"
                  onClick={() => void chooseZipImport('cinyuverse')}
                  disabled={busy}
                >
                  {t('plugins.chooseZip')}
                </Button>
