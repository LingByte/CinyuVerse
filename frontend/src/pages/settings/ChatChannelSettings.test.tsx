...
describe('ChatChannelSettings', () => {
  beforeEach(() => {
    ...
    mocks.getCommandPrefix.mockResolvedValue({ prefix: '/cinyuverse' });
  ...
  it('switches among 渠道, 指令, and 事件 without an 其他 tab', async () => {
    ...
    expect(screen.getByText('/cinyuverse folder [n|name]')).toBeVisible();
    expect(screen.getByText('/cinyuverse resume [n|id]')).toBeVisible();
    expect(screen.queryByText('/cinyuverse ping')).not.toBeInTheDocument();
