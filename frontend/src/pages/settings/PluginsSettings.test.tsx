  ...
  plugins: [
    {
      ...
      name: 'Cinyuverse Office',
      version: '2.0.0',
      description: 'Create and edit office artifacts.',
      enabled: true,
      builtin: true,
      publisher: 'Cinyuverse',
      packageDigest: 'sha256:office-package',
      sourceKind: 'builtin',
      sourcePath: '/Applications/Cinyuverse/assets/plugins/office',
      formats: ['cinyuverse'],
      skills: [
    ...
    },
    {
      ...
      formats: ['cinyuverse', 'codex', 'claude_code'],
      skills: [
  ...
  runtimes: [
    {
      ...
      referencedPlugins: ['cinyuverse.office'],
    },
    {
      id: 'officecli',
      version: '1.0.139',
...
describe('PluginsSettings', () => {
  ...
  it('previews only Agent-native resources in Codex and Claude Code settings', async () => {
    ...
    expect(within(detail).queryByText('扩展 Cinyuverse')).not.toBeInTheDocument();
  ...
  it('scopes the product module to Cinyuverse plugins and reads the activation registry', async () => {
    const backend = transport();
    renderSettings(backend.value, false, 'cinyuverse');

    expect((await screen.findAllByText('Cinyuverse Office')).length).toBeGreaterThan(
      0
    );
    ...
    expect(await screen.findByText('扩展 Cinyuverse')).toBeVisible();
    ...
    await userEvent.type(
      screen.getByRole('searchbox', { name: '搜索 Cinyuverse 插件' }),
      'office'
    );
    expect(
  ...
  it('shows declared app extensions while a product plugin is disabled', async () => {
    const disabledCatalog: PluginControlCatalog = {
      ...
      plugins: catalog.plugins.map((plugin) =>
        plugin.id === 'cinyuverse.office' ? { ...plugin, enabled: false } : plugin
      ),
    };
    const backend = transport({
  ...
  it('keeps Web plugin browsing and surfaces read-only without invoking local file UX', async () => {
    ...
    renderSettings(webTransport, false, 'cinyuverse');
  ...
  it('allows remote writes and Host ZIP import when plugin.write is present', async () => {
    ...
    renderSettings(remoteTransport, false, 'cinyuverse');
    ...
    await waitFor(() =>
      expect(screen.getByRole('switch', { name: /Cinyuverse Office/ })).toBeEnabled()
    );
  });
  ...
  it('separates native ecosystems into tabs and searches only the active tab', async () => {
    ...
    expect(
      screen.getByText('在此管理 Codex、ClaudeCode 以及 Cinyuverse 平台支持的插件')
    ).toBeVisible();
    expect(
    ...
    expect(screen.queryByText('Cinyuverse Office')).not.toBeInTheDocument();
    expect(screen.queryByText('Frontend Design')).not.toBeInTheDocument();
    ...
    await user.click(within(tabs).getByRole('tab', { name: 'Cinyuverse' }));
    expect((await screen.findAllByText('Cinyuverse Office')).length).toBeGreaterThan(
      0
    );
    expect(screen.getByText('Research Toolkit')).toBeVisible();
    const officeRow = screen.getByRole('button', { name: /Cinyuverse Office/ });
    expect(within(officeRow).queryByText('Cinyuverse')).not.toBeInTheDocument();
    ...
    expect(
      within(detailHeader as HTMLElement).getByText('Cinyuverse')
    ).toBeVisible();
    expect(
    ...
    expect(within(detail).getByText('Cinyuverse')).toBeVisible();
  ...
  it('resizes the catalog and preview panes by dragging or using the keyboard', async () => {
    ...
    expect(localStorage.getItem('cinyuverse.pluginHub.listPanePercent')).toBe('42');
  });
  ...
  it('orders the overview sections and shows MCP names without their configuration', async () => {
    ...
    await user.click(await screen.findByRole('tab', { name: 'Cinyuverse' }));
    await user.click(
      await screen.findByRole('button', { name: /Research Toolkit/ })
    );
  ...
  it('enables through the unified command and offers all-agent or MCP settings', async () => {
    ...
    expect(backend.call).toHaveBeenCalledWith('plugin_control_set_enabled', {
      pluginId: 'dev.cinyuverse.research',
      enabled: true,
    });
    ...
    expect(screen.getByLabelText('Current location')).toHaveTextContent(
      '/settings/mcp?plugin=dev.cinyuverse.research'
    );
  });
  ...
  it('falls back to existing file IPC when the desktop backend has not restarted', async () => {
    ...
    const backend = transport({
      ...
      read_file_content: (args: unknown) => {
        ...
        if (path.endsWith('.cinyuverse-plugin/plugin.json')) {
  ...
  it('runs an official CLI import with visible logs and result', async () => {
    ...
    expect(within(dialog).getByRole('button', { name: /Cinyuverse/ })).toBeVisible();
    ...
    await user.click(within(dialog).getByRole('button', { name: /^Cinyuverse/ }));
    expect(within(dialog).getByText('Cinyuverse 插件 ZIP')).toBeVisible();
    expect(within(dialog).queryByText(/CLI/)).not.toBeInTheDocument();
  });

  it('previews a Cinyuverse ZIP and requires an explicit same-id replacement decision', async () => {
    const user = userEvent.setup();
    ...
    await user.click(within(chooser).getByRole('button', { name: /^Cinyuverse/ }));
  ...
  it('renders each Runtime identity and only marks an exact lock ready', async () => {
    ...
    const detail = await screen.findByRole('region', { name: 'Cinyuverse Office' });
  ...
  it('uses the same permission decision for builtin packages', async () => {
    ...
    await user.click(
      await screen.findByRole('switch', { name: '启用 Cinyuverse Office' })
    );
    const dialog = await screen.findByRole('alertdialog', {
      name: '允许 Cinyuverse Office 使用这些能力？',
    });
    expect(dialog.tagName).toBe('DIALOG');
    expect(within(dialog).getAllByText('Cinyuverse')[0]).toBeVisible();
  ...
  it('prepares required runtimes before enabling and toasts activation failures', async () => {
    ...
    await waitFor(() => {
      ...
      expect(toast.error).toHaveBeenCalledWith('无法启用 Cinyuverse Office', {
