  ...
  items: [
    {
      ...
      detail: '',
      version: '1.1.0',
      path: '/opt/cinyuverse/bin/codex-acp',
      source: null,
      repairable: true,
...
describe('AgentDetail', () => {
  ...
  it('renders state-driven repair and preflight actions without a detail banner', async () => {
    ...
    expect(
      within(acpResult).queryByTitle('/opt/cinyuverse/bin/codex-acp')
    ).not.toBeInTheDocument();
    ...
    expect(
      within(acpResult).getByTitle('/opt/cinyuverse/bin/codex-acp')
  ...
  it('hides the diagnostic list when operation diagnostics are disabled', async () => {
    ...
    expect(localStorage.getItem('cinyuverse:operation-diagnostics')).toBe('off');
  });
});
