...
describe('SystemSettings', () => {
  beforeEach(() => {
    ...
    mocks.getSettingsPath.mockResolvedValue('/Users/test/.cinyuverse/settings.json');
  ...
  it('exports and inspects portable backups through the backend boundary', async () => {
    ...
    await user.type(
      screen.getByPlaceholderText(/cinyuverse-backup\.cinyuversebak/),
      '/tmp/cinyuverse-backup.cinyuversebak'
    );
    await user.click(screen.getByRole('button', { name: '导出' }));
    await waitFor(() => {
      expect(mocks.backupCreate).toHaveBeenCalledWith({
        path: '/tmp/cinyuverse-backup.cinyuversebak',
        passphrase: null,
      });
    });

    await user.type(
      screen.getByPlaceholderText('选择或粘贴 .cinyuversebak 文件路径'),
      '/tmp/restore.cinyuversebak'
    );
    await user.click(screen.getByRole('button', { name: '预览' }));
    await waitFor(() => {
      expect(mocks.backupInspect).toHaveBeenCalledWith({
        path: '/tmp/restore.cinyuversebak',
        passphrase: null,
      });
    });
  ...
  it('shows the shared JSON settings source on the system page', async () => {
    ...
    expect(
      screen.getByText('/Users/test/.cinyuverse/settings.json')
    ).toBeInTheDocument();
  });
