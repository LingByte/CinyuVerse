...
const OPERATION_DIAGNOSTICS_KEY = 'cinyuverse:operation-diagnostics';
...
type AgentDetailProps = {
  ...
  onPreflight: () => void;
  onInstall: () => void;
  onInstallVersion?: (input: {
    version?: string;
    runtimeVersion?: string;
    acpVersion?: string;
  }) => void;
  onPreviewVersions?: (input: {
    runtimeVersion?: string;
    acpVersion?: string;
  }) => Promise<string | null>;
  onRepair: () => void;
...
export function AgentDetail({
  ...
  const toggleOperationDiagnostics = (enabled: boolean) => {
    ...
    void persistFrontendPreference('cinyuverse:operation-diagnostics', enabled);
  };
  ...
  useEffect(() => {
    if (!supportsCustomVersion || !customVersionValid || !onPreviewVersions) {
      setSpecifiedCompatibilityWarning(null);
      return;
    }
    let cancelled = false;
