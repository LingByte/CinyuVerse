...
describe('TurnToolCalls subagent expansion', () => {
  it('renders the host product card for MCP delegation and shows the poll tool', () => {
    ...
    expect(
      screen.queryByText(/cinyuverse-delegation-mcp__delegate_to_agent/)
    ...
    expect(
      capturedNames().some((name) =>
        /cinyuverse-delegation-mcp__delegate_to_agent/.test(name)
      )
    ).toBe(false);
    expect(
      captured.groups
  ...
  it('renders only the product card for prefixed and Grok-envelope MCP names', () => {
    ...
    render(
      <BackendTransportProvider transport={transport}>
        <TurnToolCalls
          ...
          items={[
            toolItem(
              'cinyuverse-delegation-mcp__delegate_to_agent',
              { agent_type: 'codex', task: 'introduce yourself' },
              null,
              0
            ),
    ...
    expect(
      capturedNames().some((name) =>
        /搜索|Search|delegate_to_agent|cinyuverse-delegation/.test(name)
      )
    ).toBe(false);
  });
});
