...
export function ToolCallTarget({
  ...
  const labelClass = overflows
    ? 'cinyuverse-tool-call-target-label is-overflow'
    : 'cinyuverse-tool-call-target-label';
  ...
  return (
    <span className="cinyuverse-tool-call-target">
      {resolvedPath ? (
        <FileIcon
          filePath={resolvedPath}
          isFolder={isFolder}
          className="cinyuverse-tool-call-file-icon"
        />
      ) : null}
      {name}
      {suffix ? (
        <span className="cinyuverse-tool-call-target-suffix">{suffix}</span>
      ) : null}
    </span>
  );
}
