...
describe('conversation tool cards', () => {
  ...
  it('lists directory entries instead of raw list_dir arguments', () => {
    render(
      <ToolCallCard
        entry={toolEntry({
          toolName: 'list_dir',
          content: '/Users/mac/Projects/Cinyuverse/frontend',
          actionType: {
            action: 'tool',
            tool_name: 'list_dir',
            arguments: { path: '/Users/mac/Projects/Cinyuverse/frontend' },
            result: {
              type: { type: 'json' },
              value: 'src/\npackage.json\nREADME.md',
            },
  ...
  it('shows the search query and a URL list instead of the raw payload', () => {
    const payload = {
      action: {
        type: 'search',
        query: 'site:github.com cinyuverse workflow creator plugin',
        sources: [
          { type: 'url', url: '' },
          {
            type: 'url',
    ...
    expect(
      screen.getByText('site:github.com cinyuverse workflow creator plugin')
    ).toBeInTheDocument();
    expect(
