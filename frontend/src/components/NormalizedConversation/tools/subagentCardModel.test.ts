...
describe('subagent card model', () => {
  it('recognizes Grok spawn_subagent and Claude Task payloads', () => {
    ...
    expect(
      isHostDelegationTool(
        use('mcp__cinyuverse-delegation-mcp__delegate_to_agent', {
          agent_type: 'codex',
          task: 'look',
        })
      )
    ...
    expect(
      isHostDelegationLifecycleTool(
        use('use_tool', {
          tool_name: 'cinyuverse-delegation-mcp__get_delegation_status',
    ...
    expect(
      isCompanionMcpDiscoverySearch(
        use('search_tool', { query: 'cinyuverse-delegation-mcp' })
      )
    ).toBe(true);
