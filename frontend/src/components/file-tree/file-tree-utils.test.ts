...
describe('deriveFileTreeContextMenuHeader', () => {
  it('uses the workspace root label for the root context menu', () => {
    expect(deriveFileTreeContextMenuHeader('', 'Cinyuverse')).toEqual({
      title: 'Cinyuverse',
      subtitle: null,
    });
  });
  ...
  it('uses the path leaf as title and full relative path as subtitle', () => {
    expect(
      deriveFileTreeContextMenuHeader('src/components/Button.tsx', 'Cinyuverse')
    ).toEqual({
      title: 'Button.tsx',
      subtitle: 'src/components/Button.tsx',
    });

    expect(deriveFileTreeContextMenuHeader('/tmp/file.txt', 'Cinyuverse')).toEqual({
      title: 'file.txt',
