...
const CUSTOM_VALUE = '__cinyuverse_custom_answer__';
...
function questionTabsFromRequest(
  ...
  const cinyuverseQuestions = root?.['x-cinyuverse-questions'];
  if (Array.isArray(cinyuverseQuestions)) {
    const parsed = cinyuverseQuestions.flatMap((rawQuestion, index) => {
...
function responseContent(
  ...
  const root = asObject(request.schema);
  if (Array.isArray(root?.['x-cinyuverse-questions'])) {
    return {
