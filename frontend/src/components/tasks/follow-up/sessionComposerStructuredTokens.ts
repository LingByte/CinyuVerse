...
const CONVERSATION_URI_PREFIX = 'cinyuverse://conversation/';
const COMMIT_URI_PREFIX = 'cinyuverse://commit/';
...
const LEGACY_PREVIEW_ELEMENT_STORAGE_PREFIX =
  'cinyuverse:session-composer-preview-element:';
...
function parseAgentMentionAt(
  ...
  if (
    !namePart ||
    source.slice(namePart.end + 1, namePart.end + 16) !== '(cinyuverse://agent/'
  ) {
    return null;
  }
