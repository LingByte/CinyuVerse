...
describe('FirstRunExperience', () => {
  ...
  it('renders the product-site hero copy, equation, and primary next action', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Agent 需要新的 IDE 于是有了 Cinyuverse' })
    ).toBeInTheDocument();
    expect(
      screen.getByText('All in One 的综合 VibeCoding 强大平台')
    ).toBeInTheDocument();
    expect(screen.getByTestId('onboarding-eq-line')).toHaveTextContent('Cinyuverse');
  ...
  it('enforces enabled/default selection and starts installation before welcome', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: '欢迎来到 Cinyuverse' })
    ).toBeInTheDocument();
  });
