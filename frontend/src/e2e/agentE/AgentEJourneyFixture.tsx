...
const PROJECTION_KEY = 'cinyuverse:e2e:agent-e:projection';
...
function parseMentionKinds(text: string): string[] {
  return Array.from(
    text.matchAll(/\[&(?:\\.|[^\]])+\]\(cinyuverse:\/\/agent\/([^)]+)\)/g),
...
export class FakeMcpDelegationTransport implements BackendTransport {
  ...
  async call(
    ...
    if (command === 'plugin_control_catalog') {
      return {
        plugins: [{ id: 'cinyuverse.multi-agent', enabled: true }],
        runtimes: [],
      };
    }
        ...
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-[70ch] text-sm text-foreground">
              Fake MCP-capable parent · persistent projection · real Cinyuverse
              composer and cards
            </p>
          </div>
