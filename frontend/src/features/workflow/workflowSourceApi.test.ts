...
describe('resolveWorkflowSourceRevision', () => {
  it('keeps a known revision without reading the file', async () => {
    ...
    await expect(
      resolveWorkflowSourceRevision(api, '~/workflow.cinyuverse-workflow.json', 'rev-1')
    ).resolves.toBe('rev-1');
  ...
  it('reads the current file when the editor has no known revision', async () => {
    const read = vi.fn().mockResolvedValue({
      path: '~/workflow.cinyuverse-workflow.json',
      content: '{}',
      revision: 'rev-9',
    });
    const transport = {
    ...
    await expect(
      resolveWorkflowSourceRevision(api, '~/workflow.cinyuverse-workflow.json', null)
    ).resolves.toBe('rev-9');
  ...
  it('writes with the resolved revision so existing sources are not refused', async () => {
    ...
    const expectedRevision = await resolveWorkflowSourceRevision(
      api,
      '~/workflow.cinyuverse-workflow.json',
      null
    );
    await api.write(
      '~/workflow.cinyuverse-workflow.json',
