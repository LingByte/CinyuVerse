...
const session = (
  ...
  title: id,
  workspace_path: '/Users/mac/Projects/Cinyuverse',
  message_count: 2,
...
const folder = (
  ...
  project_name: projectId ? 'Cinyuverse' : null,
...
describe('importLocalSessions', () => {
  it('builds stable session keys and skips already imported rows', () => {
    const scanned = folder('/Users/mac/Projects/Cinyuverse', 'ws-1', [
      {
        agent_id: 'codex',
  ...
  it('uses only a matched workspace or an explicit destination', () => {
    const matched = folder('/Users/mac/Projects/Cinyuverse', 'ws-matched', []);
  ...
  it('filters by recency, keeps undated sessions, and scopes to existing Cinyuverse projects', () => {
    const now = new Date('2026-09-02T00:00:00Z');
    const currentRoot = folder('/Users/mac/Projects/Cinyuverse', 'ws-root', [
      session('recent'),
      session('old', { updated_at: '2026-01-01T00:00:00Z' }),
      session('undated', { updated_at: null }),
    ]);
    const currentWorktree = folder(
      '/Users/mac/Projects/Cinyuverse/.worktrees/feature',
      'ws-tree',
      [session('worktree')]
    );
