...
export class WebTransport implements BackendTransport {
  ...
  private async request(
    ...
    const response = await fetch(`${this.baseUrl}${path}`, {
      ...
      headers: {
        ...
        'X-Cinyuverse-Protocol-Version': PROTOCOL_VERSION,
      },
    });
    if (!response.ok) {
      ...
      throw new WebTransportError(
        envelope.message ?? `Cinyuverse Server returned HTTP ${response.status}`,
  ...
  private connectSocket(): void {
    ...
    const socket = new WebSocket(this.webSocketUrl(), [
      'cinyuverse.v1',
      `cinyuverse.token.${base64UrlEncode(this.token)}`,
    ]);
    this.socket = socket;
    socket.onopen = () => {
