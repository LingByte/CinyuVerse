...
test('Windows 通知显式使用 Cinyuverse 的 AppId 作为 toast sender', () => {
  ...
  assert.match(notificationService, /com\.cinyuverse\.app/);
  assert.match(notificationService, /\.arg\("-AppId"\)/);
  assert.match(notificationService, /\.arg\("-AppName"\)/);
  assert.match(toastScript, /\[string\]\$AppId = "com\.cinyuverse\.app"/);
