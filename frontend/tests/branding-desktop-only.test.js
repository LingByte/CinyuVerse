...
const brandPattern = /Cinyuverse|APP_NAME/;
...
test('Logo 组件与外观设置展示 Cinyuverse 内部品牌', () => {
...
test('关键前端页面品牌文案更新为 Cinyuverse', () => {
  ...
  assert.doesNotMatch(projectContextSource, /vibe-kanban(?!.*cinyuverse)/);
...
test('桌面宿主页使用可随主题更新的新应用图标入口', () => {
  ...
  assert.match(indexSource, /<title>Cinyuverse<\/title>/);
});

test('Tauri 桌面应用名称更新为 Cinyuverse', () => {
  const tauriConfigSource = readRepoFile('src-tauri/tauri.conf.json');

  assert.match(tauriConfigSource, /"productName":\s*"Cinyuverse"/);
  assert.match(tauriConfigSource, /"title":\s*"Cinyuverse"/);
});
...
test('创建 PR 对话框使用新的品牌后缀', () => {
  ...
  assert.match(createPrSource, /\(Cinyuverse\)|APP_NAME/);
});
