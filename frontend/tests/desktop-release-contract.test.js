...
test('installed desktop identity stays distinct from the development instance', () => {
  ...
  assert.equal(config.identifier, 'com.cinyuverse.app');
  assert.equal(config.productName, 'Cinyuverse');
  assert.deepEqual(config.plugins['deep-link'].desktop.schemes, ['cinyuverse']);
  assert.match(cefStager, /com\.cinyuverse\.app\.dev/);
...
test('desktop release compiles frontend once and reuses Rust and CEF caches', () => {
  ...
  assert.match(workflow, /CINYUVERSE_SKIP_FRONTEND_BUILD:\s*"1"/);
  ...
  assert.match(desktopBuild, /cinyuverse_cef_helper/);
