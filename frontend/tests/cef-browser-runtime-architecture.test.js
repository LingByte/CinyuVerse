...
test('closing an embedded CEF tab owns child teardown without closing the Tauri parent', () => {
  ...
  assert.match(
    cefHost,
    /struct CinyuverseDestroyBrowserViewTask[\s\S]*fn execute[\s\S]*native::destroy_browser_view\(&self\.browser\)/
  );
  assert.doesNotMatch(
...
test('macOS desktop dev runs CEF from the required app bundle', () => {
  ...
  const paths = resolveMacosDevPaths(
    '/workspace',
    '/workspace/target/debug/cinyuverse'
  );
  assert.equal(
    paths.appExecutable,
    '/workspace/target/cef-runtime/macos/app/cinyuverse.app/Contents/MacOS/cinyuverse'
  );
  assert.equal(
    paths.frameworkResources,
    '/workspace/target/cef-runtime/macos/app/cinyuverse.app/Contents/Frameworks/Chromium Embedded Framework.framework/Resources'
  );
...
test('macOS desktop dev re-signs the bundle after replacing executables', () => {
  ...
  assert.match(runner, /'-p', 'cinyuverse-mcp'/);
});
...
test('macOS CEF dev runner removes leftover codesign temp files before signing', () => {
  ...
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-sign-'));
  const macos = path.join(fixture, 'Contents', 'MacOS');

  try {
    fs.mkdirSync(macos, { recursive: true });
    fs.writeFileSync(path.join(macos, 'cinyuverse'), 'app');
    fs.writeFileSync(path.join(macos, 'cinyuverse.cstemp'), 'stale codesign copy');
    fs.writeFileSync(path.join(macos, 'cinyuverse.next-42277'), 'interrupted copy');
    removeCodesignTempFiles(fixture);
    assert.equal(fs.existsSync(path.join(macos, 'cinyuverse.cstemp')), false);
    assert.equal(fs.existsSync(path.join(macos, 'cinyuverse.next-42277')), false);
    assert.equal(fs.readFileSync(path.join(macos, 'cinyuverse'), 'utf8'), 'app');
  } finally {
...
test('macOS CEF dev bundle cache ignores packaging-only overlay links', () => {
  ...
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-dev-'));
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        schemaVersion: 1,
        bundleIdentifier: 'com.cinyuverse.app.dev',
...
test(
  ...
  () => {
    ...
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-bin-'));
...
test('macOS CEF dev runner identifies only its staged app command', () => {
  ...
  assert.equal(
    isTrackedDevAppCommand(
      '/Applications/Cinyuverse.app/Contents/MacOS/cinyuverse',
      stagedExecutable
    ),
    false
  );
  ...
  assert.deepEqual(
    collectStaleDevAppPids(
      [
        ...
        '  611 /Applications/Cinyuverse.app/Contents/MacOS/cinyuverse',
