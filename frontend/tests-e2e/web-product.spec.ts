...
test('production Web UI authenticates and exposes only Server capabilities', async ({
  ...
  const baseUrl = process.env.CINYUVERSE_E2E_BASE_URL;
  const token = process.env.CINYUVERSE_E2E_TOKEN;
  ...
  await expect(
    page.getByRole('form', { name: /Connect to Cinyuverse|连接到 Cinyuverse/ })
  ).toBeVisible();
