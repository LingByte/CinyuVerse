# Cinyuverse Plugin Dev Control Protocol 1.0

The Plugin CLI uses this loopback-only control protocol for linked development.
It is separate from Cinyuverse user/session authentication.
...
- The CLI reads the endpoint only from `--host` or `CINYUVERSE_PLUGIN_DEV_HOST`.
- The CLI reads the opaque token only from `--token` or
  `CINYUVERSE_PLUGIN_DEV_TOKEN`. It never reads or sends the main Cinyuverse bearer.
- Every request sends `X-Cinyuverse-Plugin-Dev-Protocol: 1.0` and
  `X-Cinyuverse-Plugin-Dev-Token: <opaque token>`.
...
`.cinyuverse-plugin/package.lock.json`, and
`.cinyuverse-plugin/developer-link.json`; SHA-256 every file; then SHA-256 the UTF-8
