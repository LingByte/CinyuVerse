...
export async function packPlugin(root: string, output?: string) {
  ...
  zip.addFile(
    '.cinyuverse-plugin/package.lock.json',
  ...
  const signature = {
    ...
    publisher: process.env.CINYUVERSE_PLUGIN_PUBLISHER ?? 'local',
    signedAt: new Date(0).toISOString(),
  };
  zip.addFile(
    '.cinyuverse-plugin/signature.json',
...
async function packageFiles(root: string) {
  ...
  async function visit(directory: string) {
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (path === '.cinyuverse-plugin/package.lock.json') continue;
      if (path === '.cinyuverse-plugin/developer-link.json') continue;
