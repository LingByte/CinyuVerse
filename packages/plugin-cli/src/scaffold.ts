...
export async function scaffoldPlugin(
  ...
  await mkdir(join(root, ".cinyuverse-plugin"), { recursive: true });
  await writeJson(join(root, ".cinyuverse-plugin", "plugin.json"), {
  ...
  await writeFile(
    join(root, "README.md"),
    `---\nsummary: Add ${id} features to Cinyuverse.\n---\n# ${id}\n\nDescribe how to use this plugin.\n`,
  );
  await writeFile(join(root, "config.json"), "{}\n");
  await writeJson(join(root, ".cinyuverse-plugin", "content.index.json"), {
    schemaVersion: 1,
  ...
  await writeJson(join(root, "package.json"), {
    ...
    type: "module",
    scripts: {
      build: "cinyuverse-plugin build",
      test: "cinyuverse-plugin test",
      ...(spec.nodeHandlers
        ? { "test:plugin": "node --test test/plugin.test.mjs" }
        : {}),
      dev: "cinyuverse-plugin dev",
    },
    ...(spec.usesJsSdk
      ? { dependencies: { "@cinyuverse/plugin-sdk": "^1.0.0" } }
      : {}),
    devDependencies: { "@cinyuverse/plugin-cli": "^1.0.0" },
  });
...
async function writeNodeWorker(
  ...
  const entry = `import { runStdioPluginWorker } from '@cinyuverse/plugin-sdk/stdio';
...
function nodeWorkerSource(spec: TemplateSpec) {
  const handles = (spec.nodeHandlers ?? [])
    .map((handler) => {
      ...
      return "  plugin.handle('hello', async (input) => ({ message: 'Hello from Cinyuverse', input }));";
    })
    .join("\n");
  return `import { definePluginWorker } from '@cinyuverse/plugin-sdk/worker';\n\nexport default definePluginWorker((plugin) => {\n${handles}\n});\n`;
}
...
def setup(registrar, _environment):
    def hello(value, _env):
        return {"message": "Hello from Cinyuverse", "input": value}
...
dependencies = ["cinyuverse-plugin>=1.0.0"]

[tool.cinyuverse.plugin]
...
[dependencies]
serde_json = "1"
cinyuverse-plugin-sdk = "1.0.0"
`,
  );
  await writeFile(
    join(root, "runtime", "src", "main.rs"),
    `use serde_json::json;
use cinyuverse_plugin_sdk::{define_plugin_worker, run_stdio_plugin_worker_blocking};

fn main() {
    let definition = define_plugin_worker(|registrar, _env| {
        registrar.handle_sync("hello", |input, _env| {
            Ok(json!({
                "message": "Hello from Cinyuverse",
                "input": input,
            }))
        });
    });
