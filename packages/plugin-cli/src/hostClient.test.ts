  ...
  it("uses only explicit Plugin Dev configuration and gives flags precedence", () => {
    expect(
      resolvePluginDevConnection(["--host", "http://localhost:4312"], {
        CINYUVERSE_PLUGIN_DEV_HOST: "http://127.0.0.1:4313",
        CINYUVERSE_PLUGIN_DEV_GRANT: "host-grant",
        CINYUVERSE_AUTH_TOKEN: "main-bearer-must-not-be-read",
      }),
  ...
  it("installs a linked candidate through the authenticated localhost control protocol", async () => {
    ...
    const server = createServer(async (request, response) => {
      requests.push({
        ...
        url: request.url,
        token: request.headers["x-cinyuverse-plugin-dev-token"],
        protocol: request.headers["x-cinyuverse-plugin-dev-protocol"],
