...
export function resolvePluginDevConnection(
  ...
  if (option(args, "--token") || environment.CINYUVERSE_PLUGIN_DEV_TOKEN) {
    throw new Error("dev_link_host_only");
  }
  const endpoint = option(args, "--host") ?? environment.CINYUVERSE_PLUGIN_DEV_HOST;
  const token = environment.CINYUVERSE_PLUGIN_DEV_GRANT;
  ...
  async #request<T>(method: string, path: string, body?: unknown): Promise<T> {
    const response = await fetch(new URL(path, this.#endpoint), {
      ...
      headers: {
        ...
        "x-cinyuverse-plugin-dev-token": this.#token,
        "x-cinyuverse-plugin-dev-protocol": PLUGIN_DEV_PROTOCOL_VERSION,
      },
