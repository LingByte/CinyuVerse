...
describe("scaffoldPlugin templates", () => {
  it.each(PLUGIN_TEMPLATES)(
    ...
    async (template) => {
      ...
      await expect(
        access(join(root, ".cinyuverse-plugin", "plugin.json")),
      ).resolves.toBeUndefined();
    },
  );

  it("defaults to the full template", async () => {
    const parent = await mkdtemp(join(tmpdir(), "cinyuverse-plugin-default-"));
    roots.push(parent);
  ...
  it("rejects unknown templates including agent", async () => {
    const parent = await mkdtemp(join(tmpdir(), "cinyuverse-plugin-unknown-"));
    roots.push(parent);
  ...
  it("scaffolds each specialized integration contract", async () => {
    ...
    await expect(readFile(join(python, "runtime", "worker.py"), "utf8")).resolves.toContain(
      "from cinyuverse_plugin import define_plugin_worker",
    );
    ...
    ).resolves.toContain("cinyuverse-plugin-sdk");
    await expect(
...
async function scaffold(template: PluginTemplate) {
  const parent = await mkdtemp(join(tmpdir(), `cinyuverse-plugin-${template}-`));
  roots.push(parent);
...
async function manifest(root: string) {
  return JSON.parse(
    await readFile(join(root, ".cinyuverse-plugin", "plugin.json"), "utf8"),
  ) as {
    ...
    engines?: { cinyuverse: string; pluginSdk: string };
  };
}
