...
describe("clean-room App + Agent build", () => {
  it("builds its own scaffold into a self-contained surface document", async () => {
    const parent = await mkdtemp(join(tmpdir(), "cinyuverse-plugin-build-"));
    roots.push(parent);
    ...
    expect(html).toContain("cinyuverseSurface");
  ...
  it("bundles a Host-managed MCP into its declared package entrypoint", async () => {
    const parent = await mkdtemp(join(tmpdir(), "cinyuverse-plugin-mcp-"));
    roots.push(parent);
    const root = await scaffoldPlugin(join(parent, "managed-mcp"), "cleanroom");
    const manifestPath = join(root, ".cinyuverse-plugin/plugin.json");
