...
export function discoverProductHost(
  ...
  const envUrl = (environment.CINYUVERSE_URL || "").replace(/\/+$/, "");
  const envToken = (environment.CINYUVERSE_TOKEN || "").trim();
  let token = envToken;
  const port = 17891;
  if (!token) {
    for (const dir of [
      join(homedir(), ".cinyuverse"),
      join(homedir(), "Library", "Application Support", "com.xircth.cinyuverse"),
    ]) {
      try {
...
function queueLinkedInstall(sourcePath: string) {
  const inbox = join(homedir(), ".cinyuverse", "imports");
