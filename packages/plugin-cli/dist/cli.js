...
const helpText = `Cinyuverse Plugin CLI 1.0\n\nCommands:\n  init [dir] [--publisher id] [--template full|app|agent]\n  validate [dir] [--json]\n  build [dir]\n  test [dir]\n  dev [dir] [--host url] [--token token]\n  install --link [dir] [--host url] [--token token]\n  uninstall [dir] [--delete-data] [--host url] [--token token]\n  pack [dir] [--output file.vxp]\n  doctor [dir] [--host url] [--token token]\n\nPlugins run with full trust. Connection can also use CINYUVERSE_PLUGIN_DEV_HOST and CINYUVERSE_PLUGIN_DEV_TOKEN.`;
const [command = "help", ...args] = process.argv.slice(2);
try {
    switch (command) {
        case "init": {
            const root = await scaffoldPlugin(positional(args) ?? "cinyuverse-plugin", flag(args, "--publisher") ?? "local", (flag(args, "--template") ?? "full"));
...
async function ensureValid(root) {
