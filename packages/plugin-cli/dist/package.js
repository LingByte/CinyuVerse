...
export async function packPlugin(root, output) {
    const releaseFiles = (await packageFiles(root)).filter((path) => path !== 'runtime' &&
    ...
    const lockPath = join(root, '.cinyuverse-plugin', 'package.lock.json');
    ...
    if (configEntry)
        configEntry.header.time = new Date('1980-01-01T00:00:00.000Z');
    zip.addFile('.cinyuverse-plugin/package.lock.json', Buffer.from(`${JSON.stringify(lock, null, 2)}\n`), '', 0o100644 << 16);
    const lockEntry = zip.getEntry('.cinyuverse-plugin/package.lock.json');
    if (lockEntry)
        lockEntry.header.time = new Date('1980-01-01T00:00:00.000Z');
    const manifest = JSON.parse(await readFile(join(root, '.cinyuverse-plugin', 'plugin.json'), 'utf8'));
...
async function packageFiles(root) {
    ...
    async function visit(directory) {
        ...
        for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            if (path === 'config.json')
                continue;
            if (path === '.cinyuverse-plugin/package.lock.json')
                continue;
            if (path === '.cinyuverse-plugin/developer-link.json')
                continue;
            if (entry.isSymbolicLink())
                throw new Error(`package_symlink_rejected: ${path}`);
            if (entry.isDirectory())
                await visit(absolute);
            else if (entry.isFile() && !entry.name.endsWith('.vxp'))
                paths.push(path);
        }
    }
    await visit(root);
...
export function defaultPluginName(path) {
