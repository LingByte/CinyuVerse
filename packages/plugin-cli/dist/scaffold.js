...
export async function scaffoldPlugin(target, publisher = "local", template = "full") {
    ...
    await writeFile(join(root, ".cinyuverse-plugin", "plugin.json"), `${JSON.stringify({
    ...
    await writeFile(join(root, ".cinyuverse-plugin", "content.index.json"), `${JSON.stringify({
        schemaVersion: 1,
        items: hasAgent
            ? [
                {
