...
export async function buildPlugin(root) {
    ...
    const manifest = JSON.parse(await readFile(join(pluginRoot, ".cinyuverse-plugin", "plugin.json"), "utf8"));
    ...
    if (await exists(appSource)) {
        ...
        const appBundle = await build({
          ...
          const surface = globalThis.cinyuverseSurface;
          if (!surface) throw new Error('cinyuverse_app_surface_unavailable');
          ...
          const bridge = Object.freeze({
            ...
            subscribe: (channel, listener) => {
              ...
              addEventListener('cinyuversesurfacecontext', receive);
              return () => removeEventListener('cinyuversesurfacecontext', receive);
            },
        ...
        `,
                ...
                sourcefile: "cinyuverse-app-bootstrap.mjs",
                loader: "js",
            },
            bundle: true,
        ...
        if (await exists(document)) {
            ...
            const script = appBundle.outputFiles.find((file) => file.path.endsWith(".js"))?.text;
            if (!script)
                throw new Error("plugin_app_bundle_missing");
            const css = appBundle.outputFiles.find((file) => file.path.endsWith(".css"))?.text;
            const withoutExternalScripts = html.replace(/<script\b[\s\S]*?<\/script\s*>/giu, "");
    ...
    for (const integration of manifest.integrations ?? []) {
        if (integration.kind !== "content.mcp" || !integration.resource)
            continue;
        const resource = JSON.parse(await readFile(join(pluginRoot, integration.resource), "utf8"));
        const managed = resource.managedRuntime;
        if (!managed?.source || !managed.entrypoint)
            continue;
    ...
    if (!result.valid) {
        throw new Error(`plugin_validation_failed: ${result.diagnostics
...
export function sdkAliases() {
    ...
    return {
        "@cinyuverse/plugin-sdk": require.resolve("@cinyuverse/plugin-sdk"),
        "@cinyuverse/plugin-sdk/worker": require.resolve("@cinyuverse/plugin-sdk/worker"),
        "@cinyuverse/plugin-sdk/app": require.resolve("@cinyuverse/plugin-sdk/app"),
        "@cinyuverse/plugin-sdk/testing": require.resolve("@cinyuverse/plugin-sdk/testing"),
        "@cinyuverse/plugin-sdk/protocol": require.resolve("@cinyuverse/plugin-sdk/protocol"),
        "@cinyuverse/plugin-sdk/stdio": require.resolve("@cinyuverse/plugin-sdk/stdio"),
    };
}
