...
const SERVER_NAME = "cinyuverse.workflow-creator.cinyuverse-workflow-mcp";
const SOURCE_PATH =
  "~/.cinyuverse/workflows/cinyuverse-full-repository-review.cinyuverse-workflow.json";
...
async function openClient() {
  ...
  const client = new Client(
    { name: "cinyuverse-workflow-live-verification", version: "1.0.0" },
  ...
  await client.connect(
    new StdioClientTransport({
      ...
      args: managed.args,
      cwd: process.env.CINYUVERSE_WORKSPACE_ROOT || process.cwd(),
...
async function debugSource(client, workspaceId, stepId) {
  ...
  const path =
    "~/.cinyuverse/workflows/cinyuverse-mcp-unpublished-debug.cinyuverse-workflow.json";
  const definition = {
    ...source.definition,
    name: "Cinyuverse MCP unpublished debug boundary",
  };
