...
const SERVER_URL = (process.env.CINYUVERSE_SERVER_URL || "").replace(/\/+$/, "");
const SERVER_TOKEN = process.env.CINYUVERSE_SERVER_TOKEN || "";
...
const tools = [
  tool(
    ...
    {
      path: stringProperty(
        "Project-relative path or ~/.cinyuverse/workflows/*.cinyuverse-workflow.json",
      ),
    },
    ["path"],
  ),
  tool(
    ...
    {
      path: stringProperty("Project-relative *.cinyuverse-workflow.json path"),
  ...
  tool(
    ...
    "Start a durable Workflow Run from a published immutable version.",
    {
      ...
      workspaceId: stringProperty("Cinyuverse workspace ID"),
      input: {},
  ...
  tool(
    ...
    "Test the current source definition through an unpublished durable debug Run. Reuses completed ancestors when parentRunId is provided and never publishes or retargets an Automation.",
    {
      ...
      workspaceId: stringProperty(
        "Cinyuverse workspace ID; required only for the first debug Run",
      ),
      input: {},
      policyOverride: { type: ["object", "null"] },
...
async function safeSourcePath(relative) {
  if (
    ...
    (isAbsolute(relative) && !relative.startsWith("~/.cinyuverse/workflows/")) ||
    !relative.endsWith(".cinyuverse-workflow.json")
  ) {
    throw new Error("path must end with .cinyuverse-workflow.json");
  }
  const workspaceRoot = await realpath(process.cwd());
  const sharedPrefix = "~/.cinyuverse/workflows/";
  const root = relative.startsWith(sharedPrefix)
    ? resolve(homedir(), ".cinyuverse/workflows")
    : workspaceRoot;
...
async function call(command, args) {
  if (!SERVER_URL || !SERVER_TOKEN) {
    throw new Error("Cinyuverse Host did not inject its managed workflow gateway");
  }
...
function buildServer() {
  const server = new McpServer(
    { name: "cinyuverse-workflow-mcp", version: "1.0.0" },
    {
      capabilities: { tools: {} },
      instructions:
