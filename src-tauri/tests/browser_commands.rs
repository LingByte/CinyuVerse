...
use cinyuverse::commands::browser;
...
fn unavailable_cef_runtime_reports_the_initialization_error() {
    ...
    assert_eq!(
        error.message,
        "browser engine rejected command: CEF child windows require X11/XWayland; install xwayland and launch Cinyuverse with DISPLAY set"
    );
}
