        ...
        for binding in self.official_mcp.bindings() {
            match binding.product.as_str() {
                ...
                "workflow" => servers.push(InjectedMcpServer {
                    name: "cinyuverse-workflow-mcp".to_string(),
                    command: locate_named_sibling("cinyuverse-workflow-mcp"),
                    args: Vec::new(),
                }),
                _ => {}
            }
...
impl VibexDelegationInjector {
    fn product_server(
        ...
        InjectedMcpServer {
            name: name.to_string(),
            command: locate_cinyuverse_mcp_binary(),
            args: {
                let mut args = vec![
                ...
                if let Some(url) = self.official_mcp.http_base() {
                    ...
                    args.push(url);
                    if name == "cinyuverse-delegation-mcp" {
...
pub(crate) fn locate_cinyuverse_mcp_binary() -> PathBuf {
    utils::host_bin::locate_host_family_binary("cinyuverse-mcp")
}
...
mod tests {
    ...
    fn gate(products: &[(&str, u8)]) -> Arc<OfficialMcpRuntime> {
        ...
        for (product, features) in products {
            runtime.publish_binding(OfficialMcpBinding {
                plugin_id: format!("cinyuverse.{product}"),
                binary_id: "cinyuverse-mcp".into(),
    ...
    fn injects_two_named_servers_when_both_plugins_are_on() {
        ...
        let injector = VibexDelegationInjector {
            tokens,
            socket_path: PathBuf::from("/tmp/cinyuverse-delegation-test.sock"),
            official_mcp: gate(&[
    ...
    fn session_plugin_can_inject_without_delegation() {
        ...
        assert_eq!(servers[0].name, "cinyuverse-session-mcp");
        assert!(
            servers[0]
                .args
                .windows(2)
    ...
    fn grok_still_receives_acp_session_injection() {
        ...
        assert_eq!(servers[0].name, "cinyuverse-delegation-mcp");
    }
