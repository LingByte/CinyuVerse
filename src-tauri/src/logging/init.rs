...
fn env_level_override() -> Option<String> {
    ["CINYUVERSE_LOG", "RUST_LOG"].iter().find_map(|key| {
        std::env::var(key)
            .ok()
...
const FIREHOSE_BACKSTOPS: &str = concat!(
    ...
    "tonic=info,h2::codec=warn,cinyuverse::logging=off"
);
...
mod tests {
    ...
    fn valid_targets_are_module_paths() {
        ...
        assert!(!is_valid_target("agents::"));
        assert!(!is_valid_target("agents/runtime"));
        assert!(!is_valid_target("cinyuverse::logging"));
        assert!(!is_valid_target("cinyuverse::logging::hub"));
    }
