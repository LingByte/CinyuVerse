...
fn pairing_token_from_input(token: &str) -> String {
    ...
    if let Some(payload) = trimmed.strip_prefix("cinyuverse-pairing:")
...
fn local_device_name() -> String {
    utils::net::local_hostname()
        .map(|name| format!("Cinyuverse ({name})"))
        .unwrap_or_else(|| "Cinyuverse Workstation".to_string())
        .chars()
        .take(128)
        .collect()
}
