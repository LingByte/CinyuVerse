...
fn cef_subprocess_path_from(resource_dir: Option<&std::path::Path>) -> Option<PathBuf> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let staged = directory.join(
            "../Frameworks/Chromium Embedded Framework.framework/Helpers/cinyuverse Helper.app/Contents/MacOS/cinyuverse Helper",
        );
        if staged.is_file() {
            return Some(staged);
        }
        let development_helper = directory.join("cinyuverse_cef_helper");
        if development_helper.is_file() {
            return Some(development_helper);
        }
        if let Some(resource_dir) = resource_dir {
            let helper = resource_dir.join("cinyuverse_cef_helper");
            if helper.is_file() {
                return Some(helper);
            }
        }
    ...
    #[cfg(target_os = "windows")]
    {
        let helper_name = "cinyuverse_cef_helper.exe";
    ...
    {
        let helper_name = "cinyuverse_cef_helper";
        let helper = directory.join(helper_name);
        if helper.is_file() {
            return Some(helper);
        }
...
pub fn run(cef_bootstrap: Result<CefBootstrap, String>) {
    ...
    // a second launch's args (carrying the cinyuverse:// URL on Windows/Linux) into
