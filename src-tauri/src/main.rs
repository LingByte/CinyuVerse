...
async fn search_workspace(path: String, query: String, max_results: Option<usize>) -> Result<Vec<serde_json::Value>, String> {
    ...
    if let Ok(output) = rg_output {
        ...
        for line in stdout.lines().take(min(limit, 10_000)) {
            ...
            let file_rel = file.strip_prefix("./").unwrap_or(file);
            let abs_path = base_path.join(file_rel);
...
async fn git_create_branch(path: String, branch: String) -> Result<(), String> {
    ...
    if !output.status.success() {
        ...
        return Err(if stderr.trim().is_empty() {
            "Git create branch failed".to_string()
        } else {
            stderr.trim().to_string()
        });
    }
...
async fn git_show(path: String, commit: String) -> Result<String, String> {
    let safe = commit.trim();
    if safe.is_empty() {
        return Err("Commit hash is required".to_string());
    }
    if !safe.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err("Invalid commit hash".to_string());
    }

    let output = Command::new("git")
        .args(["show", "--format=fuller", "--stat", "--patch", safe])
        .current_dir(&path)
        .output()
        .map_err(|e| e.to_string())?;
...
async fn git_branch_graph(path: String) -> Result<serde_json::Value, String> {
    ...
    for line in stdout.lines() {
        ...
        let Some((graph, parts)) = split_git_graph_line(line) else {
            continue;
        };
...
fn split_git_graph_line(line: &str) -> Option<(String, Vec<&str>)> {
    ...
    while i < len {
        if bytes[i].is_ascii_hexdigit() {
            ...
            if i - start >= 4 && i < len && bytes[i] == b'|' {
                let graph = line[..start].to_string();
                let commit_part = &line[start..];
                let parts: Vec<&str> = commit_part.split('|').collect();
                return Some((graph, parts));
            }
        } else {
            i += 1;
        }
