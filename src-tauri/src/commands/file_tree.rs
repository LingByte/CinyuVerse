...
fn workflow_source_path(path: &str) -> Result<std::path::PathBuf, AppError> {
    let expanded = if path.starts_with("~/") {
        ...
        if path.starts_with("~/.cinyuverse/workflows/")
    ...
    if !path
        ...
        .is_some_and(|name| name.ends_with(".cinyuverse-workflow.json"))
    {
        return Err(AppError::BadRequest(
            "Workflow source must end with .cinyuverse-workflow.json".to_string(),
        ));
    }
    Ok(path)
}
...
mod tests {
    ...
    fn temp_file_path(prefix: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("cinyuverse-{prefix}-{unique}.tmp"))
    }
    ...
    fn temp_dir_path(prefix: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        std::env::temp_dir().join(format!("cinyuverse-{prefix}-{unique}"))
    }
    ...
    fn workflow_source_home_path_expands_without_changing_relative_paths() {
        ...
        assert_eq!(
            expand_workflow_source_home("flows/demo.cinyuverse-workflow.json", home),
            PathBuf::from("flows/demo.cinyuverse-workflow.json")
        );
    }
