...
const BACKUP_FORMAT: &str = "cinyuverse-portable-backup";
const BACKUP_VERSION: u32 = 1;
const BACKUP_PROGRESS_EVENT: &str = "cinyuverse://backup-progress";
...
fn cinyuverse_root() -> Result<PathBuf, AppError> {
    dirs::home_dir()
        .map(|home| home.join(".cinyuverse"))
...
fn collect_directory_sources(
    ...
    for entry in fs::read_dir(current).map_err(|error| {
        ...
        // Skip leftover restore staging temps (`.<name>.cinyuverse-restore.N.tmp`) so an
        // orphan from an abnormally-terminated restore is never baked into a backup.
        if path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name.contains(".cinyuverse-restore.") && name.ends_with(".tmp"))
        {
            continue;
        }
...
fn collect_sources() -> Result<Vec<BackupSource>, AppError> {
    ...
    let cinyuverse_root = cinyuverse_root()?;
    let settings_path = cinyuverse_root.join("settings.json");
    if settings_path.exists() {
        sources.push(BackupSource {
            logical_path: "cinyuverse/settings.json".to_string(),
            file_path: settings_path,
        });
    }
    let mcp_path = cinyuverse_root.join("mcp.json");
    if mcp_path.exists() {
        sources.push(BackupSource {
            logical_path: "cinyuverse/mcp.json".to_string(),
            file_path: mcp_path,
        });
    }

    collect_directory_sources(
        &cinyuverse_root.join("skills"),
        "cinyuverse/skills",
        &cinyuverse_root.join("skills"),
        &mut sources,
    )?;
...
fn resolve_restore_target(logical_path: &str) -> Result<PathBuf, AppError> {
    ...
    } else if prefix == OsStr::new("cinyuverse") {
        cinyuverse_root()?
    } else {
        return Err(AppError::BadRequest(format!(
...
pub async fn backup_create(
    ...
    emit_progress(
        ...
        0,
        sources.len(),
        "Collected Cinyuverse data files",
    );
...
pub async fn backup_restore_stage(
    ...
    for (index, entry) in backup.entries.iter().enumerate() {
        ...
        let temp = parent.join(format!(".{file_name}.cinyuverse-restore.{index}.tmp"));
