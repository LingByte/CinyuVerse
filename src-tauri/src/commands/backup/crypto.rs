...
//! `CINYUBAK` envelope: an unencrypted header (magic + KDF params + salt + nonce
...
//! to Cinyuverse's in-memory payload (bytes in / bytes out, no file streaming and no
...
pub const ENVELOPE_MAGIC: &[u8; 8] = b"CINYUBAK";
...
/// Cleartext header at the front of a `CINYUBAK` payload. Carries everything
...
/// Encrypt `plaintext` into a `CINYUBAK` envelope.
...
/// Decrypt a `CINYUBAK` envelope. A wrong passphrase (or tampering) surfaces as
