...
const DEFAULT_UPDATE_REPOSITORY: &str = "LingByte/CinyuVerse";
...
async fn check_latest_release() -> AppReleaseStatus {
    ...
    let response = match client
        .get(url)
        .header(reqwest::header::USER_AGENT, "Cinyuverse")
        .send()
        .await
    {
...
pub(crate) fn update_repository() -> Option<String> {
    std::env::var("CINYUVERSE_UPDATE_REPO")
        .ok()
...
mod tests {
    ...
    fn parses_github_remote_urls() {
        assert_eq!(
            normalize_github_repo("git@github.com:openai/cinyuverse.git").as_deref(),
            Some("openai/cinyuverse")
        );
        assert_eq!(
            normalize_github_repo("https://github.com/openai/cinyuverse.git").as_deref(),
            Some("openai/cinyuverse")
        );
    }
    ...
    fn default_repository_matches_signed_updater_feed() {
        assert_eq!(DEFAULT_UPDATE_REPOSITORY, "LingByte/CinyuVerse");
    }
    ...
    fn parses_release_notes_and_published_at() {
        let release: GitHubReleaseFixture = serde_json::from_value(serde_json::json!({
            "tag_name": "v0.1.3",
            "html_url": "https://github.com/LingByte/CinyuVerse/releases/tag/v0.1.3",
            "body": "## English\n\nNotes\n",
        ...
        assert!(release.html_url.contains("LingByte/CinyuVerse"));
