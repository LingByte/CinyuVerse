...
mod tests {
    ...
    use super::{
        ...
        reconcile_grok_cinyuverse_configuration, reconcile_kimi_cinyuverse_configuration,
    ...
    async fn configuration_rollback_never_overwrites_a_concurrent_external_edit() {
        ...
        let rollback = [NativeFileRollback {
            ...
            expected: Some(b"cinyuverse".to_vec()),
            sensitive: false,
        }];
    ...
    #[test]
    fn cinyuverse_managed_slot_shape_still_requires_openai_auth() {
        let complete: toml::Table = toml::from_str(
            r#"
model_provider = "cinyuverse"
[model_providers.cinyuverse]
wire_api = "responses"
...
"#,
    ...
    fn preflight_component_evidence_is_structured() {
        let item = AgentPreflightItemView {
            ...
            path: Some("/opt/cinyuverse/bin/agent-acp".to_string()),
            source: None,
            repairable: true,
        ...
        assert_eq!(value["path"], "/opt/cinyuverse/bin/agent-acp");
...
"#,
        ...
        reconcile_kimi_cinyuverse_configuration(
            temp.path(),
            &HashMap::new(),
        ...
        .unwrap();
        assert_eq!(config["default_model"].as_str(), Some("cinyuverse"));
        assert_eq!(
            config["models"]["cinyuverse"]["provider"].as_str(),
            Some("cinyuverse")
        );
        assert_eq!(
            config["models"]["cinyuverse"]["max_context_size"].as_integer(),
            Some(262_144)
        );
        ...
        .unwrap();
        assert_eq!(credential["_cinyuverse_synthetic"], true);
        ...
        assert!(preserved.get("_cinyuverse_synthetic").is_none());
    }
...
"#,
        ...
        .await
        .unwrap();
        reconcile_grok_cinyuverse_configuration(
            temp.path(),
            &HashMap::new(),
            &BTreeMap::from([(
        ...
        .unwrap();
        assert_eq!(grok["models"]["default"].as_str(), Some("cinyuverse"));
    }
    ...
    async fn native_launch_preferences_preserve_unrelated_agent_environment() {
        ...
        tokio::fs::write(
            ...
            br#"{"cinyuverse":{"model":"composer-1","force":true}}"#,
        )
        .await
        .unwrap();
    ...
    fn managed_uv_component_is_dequarantined_before_its_runtime_is_launched() {
        ...
        xattr::set(temp.path(), "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
        xattr::set(&runtime, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
        let readonly = temp.path().join("python/readonly.bin");
        std::fs::write(&readonly, b"payload").unwrap();
        xattr::set(&readonly, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
    ...
    fn published_binary_copy_is_dequarantined_for_launch() {
        ...
        xattr::set(&staged, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
    ...
    fn managed_artifacts_use_an_executable_user_directory_on_macos() {
        ...
        let app_data = home.join("Library/Application Support/com.cinyuverse.app");
        let root = managed_artifacts_directory(home, &app_data);

        assert_eq!(root, home.join(".local/share/cinyuverse"));
...
fn codex_provider_config_is_projected(table: &toml::Table) -> bool {
    table.get("model_provider").and_then(toml::Value::as_str) == Some("cinyuverse")
        && table
            .get("model_providers")
        ...
        if matches!(dependency.executable, "node" | "npm") {
            ...
            let detail = match runtime_source {
                ...
                Some(RuntimeSource::Managed) => {
                    format!("使用 Cinyuverse 托管版本。要求：{}", dependency.requirement)
                }
                None => format!(
                    "未找到兼容的 Node.js 与 npm；安装或修复时将自动准备 Cinyuverse 托管环境。要求：{}",
...
async fn resolve_install_plan(
    ...
    let _ = utils::shell::refresh_process_path_after_install().await;
    // Npx plans can bootstrap Cinyuverse's pinned, checksum-verified Node.js
    ...
    // Hermes can use Cinyuverse's pinned, checksum-verified uv distribution when no
...
async fn resolve_node_runtime(
    ...
    match discover_system_node_runtime(requirement).await {
        ...
        Err(error) => {
            log.emit(format!(
                "System Node.js is unavailable or incompatible ({error}); falling back to Cinyuverse-managed Node.js {MANAGED_NODE_VERSION}"
            ));
        }
    }
    ...
    if dependency_version_satisfied(requirement, MANAGED_NODE_VERSION) != Some(true) {
        anyhow::bail!(
            "Cinyuverse-managed Node.js {MANAGED_NODE_VERSION} does not satisfy {requirement}"
        );
    }
...
async fn set_codex_auth_mode(
    ...
    // Drop the Cinyuverse store binding without rewriting native files. Switching
...
async fn prepare_codex_auth_mode_mutations(
    ...
    if mode == "chatgpt_subscription" || mode == "api_key" {
        ...
        if let Some(source) = config_original.as_deref() {
            ...
            // Handwritten provider tables stay; only the Cinyuverse-owned slot is removed.
            changed |= table.remove("model_provider").is_some();
            if let Some(providers) = table
                .get_mut("model_providers")
                .and_then(toml::Value::as_table_mut)
            {
                changed |= providers.remove("cinyuverse").is_some();
...
pub async fn agent_management_run_action(
    ...
    if let Some(url) = action.url {
        ...
        if agent_id.as_str() == "kimi_code" && action.id == "login" {
            let mutations = prepare_kimi_cinyuverse_configuration_cleanup(&environment).await?;
...
fn native_config_view(
    ...
    let files = snapshot
        ...
        .map(|file| {
            ...
            // Sensitive files cross the boundary verbatim: Cinyuverse is local-first
...
pub async fn agent_management_config_write(
    ...
    let operation = async {
        ...
        if let Some(fields) = kimi_fields {
            reconcile_kimi_cinyuverse_configuration(&home, &agent_env, &fields).await?;
            refresh_native_file_rollback_expected(&mut file_rollback).await?;
            reconciled = true;
        }
        if let Some(fields) = grok_fields {
            reconcile_grok_cinyuverse_configuration(&home, &agent_env, &fields).await?;
            refresh_native_file_rollback_expected(&mut file_rollback).await?;
            reconciled = true;
        }
...
async fn reconcile_kimi_cinyuverse_configuration(
    home: &Path,
    environment: &HashMap<String, String>,
    ...
    if model_id.is_some() {
        ...
        let model = models
            ...
            .or_insert_with(|| toml::Value::Table(toml::Table::new()))
            .as_table_mut()
            .ok_or_else(|| internal_error("Kimi models.cinyuverse 必须是表"))?;
        model.insert(
            "provider".to_string(),
            toml::Value::String("cinyuverse".to_string()),
        );
    ...
    } else if root.get("default_model").and_then(toml::Value::as_str) == Some("cinyuverse") {
...
async fn reconcile_grok_cinyuverse_configuration(
    home: &Path,
    environment: &HashMap<String, String>,
    changed_fields: &BTreeMap<String, Option<String>>,
) -> Result<(), AgentManagementErrorView> {
    ...
    } else if models.get("default").and_then(toml::Value::as_str) == Some("cinyuverse") {
...
async fn prepare_kimi_cinyuverse_configuration_cleanup(
    environment: &HashMap<String, String>,
) -> Result<Vec<NativeFileMutation>, AgentManagementErrorView> {
    let home = dirs::home_dir().ok_or_else(|| internal_error("用户目录不可用"))?;
    let kimi_home = kimi_code_home(&home, environment);
    ...
    if let Some(source) = config_original.as_deref() {
        ...
        let root = document
            .as_table_mut()
            .ok_or_else(|| internal_error("Kimi config.toml 顶层必须是表"))?;
        if root.get("default_model").and_then(toml::Value::as_str) == Some("cinyuverse") {
            root.remove("default_model");
        }
...
fn kimi_credential_is_synthetic(value: &serde_json::Value) -> bool {
    value
        .get("_cinyuverse_synthetic")
        .and_then(serde_json::Value::as_bool)
        == Some(true)
        || value
            .get("access_token")
            .and_then(serde_json::Value::as_str)
            == Some("cinyuverse-local-gate")
}
