...
name = "tauri-plugin-cinyuverse-inspector"
version = "0.1.0"
edition = "2021"
publish = false
links = "tauri-plugin-cinyuverse-inspector"
...
description = "Allow the Cinyuverse development inspector bridge"
...
fn project_root() -> Result<PathBuf, String> {
    ...
    manifest
        ...
        .map(PathBuf::from)
        .ok_or_else(|| "Cinyuverse inspector project root is unavailable".to_string())
}
...
fn submit_capture(json: String) -> Result<(), String> {
    ...
    let inbox = project_root()?.join(".cinyuverse/inspector/inbox");
...
fn take_control() -> Result<Option<String>, String> {
    let path = project_root()?.join(".cinyuverse/inspector/control");
    if !path.is_file() {
        return Ok(None);
    }
...
(function () {
  if (window.__cinyuverseInspectorBridge) return;
  window.__cinyuverseInspectorBridge = true;
      ...
      blob.text().then(function (text) {
        try {
          ...
          if (Array.isArray(value.annotations)) {
            invoke('plugin:cinyuverse-inspector|submit_capture', { json: text });
          }
        } catch (_) {}
      });
    }
  ...
  window.setInterval(function () {
    invoke('plugin:cinyuverse-inspector|take_control').then(function (command) {
...
pub fn init<R: Runtime>() -> TauriPlugin<R> {
    Builder::<R>::new("cinyuverse-inspector")
...
fn inspect_project_root(project_root: &Path) -> Result<TauriInspectorStatus, AppError> {
    ...
    let installed = cargo.contains("tauri-plugin-redline")
        && cargo.contains("tauri-plugin-cinyuverse-inspector")
        && builder_installed
        && project_root
            .join(".cinyuverse/tauri-plugin-cinyuverse-inspector/Cargo.toml")
            .is_file();
    ...
    Ok(TauriInspectorStatus {
        ...
        message: if installed {
            "Cinyuverse Tauri inspector is installed".to_string()
        } else if is_tauri {
...
fn patch_cargo_manifest(source: &str) -> Result<String, AppError> {
    ...
    let has_companion = source.contains("tauri-plugin-cinyuverse-inspector");
    ...
    let replacement = r#"{
        ...
        if cfg!(debug_assertions) {
            builder = builder
                ...
                .plugin(tauri_plugin_cinyuverse_inspector::init());
        }
        builder
    }"#;
...
fn patch_capability(path: &Path) -> Result<(), AppError> {
    ...
    for permission in ["redline:default", "cinyuverse-inspector:default"] {
        if !permissions
            .iter()
...
fn install_companion(project_root: &Path) -> Result<(), AppError> {
    reject_symlink(&project_root.join(".cinyuverse"))?;
    let plugin_root = project_root.join(".cinyuverse/tauri-plugin-cinyuverse-inspector");
    ...
    let runtime_ignore = project_root.join(".cinyuverse/.gitignore");
...
fn install_project_root(project_root: &Path) -> Result<TauriInspectorStatus, AppError> {
    ...
    let capability = capability_paths.first().ok_or_else(|| {
        AppError::BadRequest(
            "No JSON capability file found; add redline:default and cinyuverse-inspector:default manually"
                .into(),
        )
    })?;
...
pub async fn control_tauri_inspector(
    ...
    let inspector_dir = root.join(".cinyuverse/inspector");
    reject_symlink(&root.join(".cinyuverse"))?;
...
pub async fn take_tauri_inspector_capture(
    ...
    let inbox = root.join(".cinyuverse/inspector/inbox");
    if !inbox.is_dir() {
        return Ok(None);
    }
    let canonical_root = fs::canonicalize(&root).map_err(|error| io_error(&root, error))?;
...
mod tests {
    ...
    fn cargo_patch_adds_redline_and_companion_once() {
        ...
        assert!(once.contains("tauri-plugin-cinyuverse-inspector"));
    ...
    fn builder_patch_keeps_inspector_debug_only_and_is_idempotent() {
        ...
        assert!(once.contains("tauri_plugin_cinyuverse_inspector::init()"));
        assert_eq!(once, twice);
    }
    ...
    fn installs_the_bridge_and_capabilities_into_a_standard_project() {
        ...
        assert!(
            temp.path()
                .join(".cinyuverse/tauri-plugin-cinyuverse-inspector/src/lib.rs")
                .is_file()
        );
        assert!(capability.contains("redline:default"));
        assert!(capability.contains("cinyuverse-inspector:default"));
        assert_eq!(
            fs::read_to_string(temp.path().join(".cinyuverse/.gitignore")).unwrap(),
            "inspector/\n"
        );
    }
}
