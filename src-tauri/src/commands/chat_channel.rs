...
/// Migrate legacy plaintext-JSON channel tokens into ~/.cinyuverse/.env (ADR-0004), once
...
pub async fn test_chat_channel(channel_id: String) -> Result<ChatChannelTestResult, AppError> {
    ...
    let msg = RichMessage::info("这是一条来自 Cinyuverse 设置页的测试消息。")
        .with_title("🔔 Cinyuverse 测试通知")
