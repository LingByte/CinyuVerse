...
const TOKEN_HEADER: &str = "x-cinyuverse-plugin-dev-token";
const PROTOCOL_HEADER: &str = "x-cinyuverse-plugin-dev-protocol";
...
mod tests {
    ...
    fn rust_digest_matches_the_public_cli_package_lock_algorithm() {
        ...
        std::fs::create_dir_all(root.path().join(".cinyuverse-plugin")).unwrap();
        ...
        std::fs::write(
            root.path().join(".cinyuverse-plugin/developer-link.json"),
            b"private path",
        )
        .unwrap();
