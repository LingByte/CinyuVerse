...
mod tests {
    ...
    fn managed_worktree_agent_cwd_joins_the_repo_folder_to_the_container() {
        let mut workspace = sample_workspace(true);
        workspace.agent_working_dir = Some("Cinyuverse".to_string());
        let repo = sample_repo("Cinyuverse", None);

        assert_eq!(
            resolve_workspace_default_open_path(
                &workspace,
                "/Users/mac/.cinyuverse-workspaces/workflow-debug",
                std::slice::from_ref(&repo),
            ),
            PathBuf::from("/Users/mac/.cinyuverse-workspaces/workflow-debug/Cinyuverse")
        );
    }
