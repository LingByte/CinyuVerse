//! `cinyuverse://` deep links (P2-5). Opens the app on a specific project/session from
//! an OS URL. Recognizes `cinyuverse://<any>?project=<id>&workspace=<id>&session=<id>`
...
pub fn route_deep_link_args(app: &AppHandle, args: &[String]) {
    let urls: Vec<url::Url> = args
        ...
        .filter(|url| url.scheme() == "cinyuverse")
        .collect();
    if urls.is_empty() {
...
fn route_one(app: &AppHandle, url: &url::Url) {
    if url.scheme() != "cinyuverse" {
        return;
    }
    // Always bring the app forward for any cinyuverse:// link.
    show_main_window(app);
