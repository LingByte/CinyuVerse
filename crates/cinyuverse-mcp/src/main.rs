//! `cinyuverse-mcp` — the per-launch companion MCP server.
//!
...
fn resolve_conversation_id(flag: Option<String>) -> Option<String> {
    flag.or_else(|| std::env::var("CINYUVERSE_CONVERSATION_ID").ok())
...
fn initialize_result() -> Value {
    json!({
        ...
        "capabilities": { "tools": {} },
        "serverInfo": { "name": "cinyuverse-mcp", "version": env!("CARGO_PKG_VERSION") }
    })
}
...
mod tests {
    ...
    fn initialize_returns_server_info() {
        ...
        assert_eq!(value["result"]["serverInfo"]["name"], "cinyuverse-mcp");
    ...
    fn tool_schema_explains_agent_mentions() {
        ...
        assert!(description.contains("[&Codex](cinyuverse://agent/codex)"));
    ...
    fn companion_prefers_http_when_a_server_url_is_present() {
        let injected = Companion::new(Args {
            parent_connection_id: "conn-1".to_string(),
            socket_path: "/tmp/cinyuverse-delegation.sock".to_string(),
            token: "tok".to_string(),
    ...
    fn conversation_id_falls_back_to_the_parent_process_env() {
        ...
        let previous = std::env::var("CINYUVERSE_CONVERSATION_ID").ok();
        unsafe {
            std::env::set_var("CINYUVERSE_CONVERSATION_ID", " from-env ");
        }
        assert_eq!(resolve_conversation_id(None).as_deref(), Some("from-env"));
        unsafe {
            match previous {
                Some(value) => std::env::set_var("CINYUVERSE_CONVERSATION_ID", value),
                None => std::env::remove_var("CINYUVERSE_CONVERSATION_ID"),
            }
        }
    }
    ...
    async fn call_broker_round_trips_over_named_pipe() {
        ...
        let path = format!(r"\\.\pipe\cinyuverse-mcp-test-{}", std::process::id());
    ...
    async fn call_broker_round_trips_over_uds() {
        ...
        let path = dir.join(format!("cinyuverse-mcp-test-{}.sock", std::process::id()));
