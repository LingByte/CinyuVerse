...
fn validate_source_path(source_path: &str) -> Result<(), WorkflowError> {
    ...
    if source_path.trim().is_empty()
        ...
        || !source_path.ends_with(".cinyuverse-workflow.json")
    {
        return Err(WorkflowError::Validation(
            "sourcePath must be a relative *.cinyuverse-workflow.json path".to_string(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    async fn publish_and_start(core: &WorkflowCore) -> (WorkflowVersionView, WorkflowRunView) {
        ...
        let run = core
            .start(StartWorkflow {
                ...
                input: serde_json::json!({"repo": "cinyuverse"}),
    ...
    async fn source_path_is_the_reusable_definition_identity() {
        ...
        let source_path = "flows/repository-review.cinyuverse-workflow.json";
        let first = core
            .publish(PublishWorkflow {
                definition_id: None,
                definition: definition(),
    ...
    async fn debug_snapshot_stays_out_of_published_history_until_publish() {
        let (core, _) = setup().await;
        let source_path = "flows/debug-only.cinyuverse-workflow.json";
        let debug = core
    ...
    async fn downstream_bindings_resolve_run_input_and_accepted_step_output() {
        ...
        assert_eq!(resolved.values["repo"], "cinyuverse");
