...
async fn missing_static_root_serves_a_host_listen_page() {
    ...
    assert!(text.contains("Cinyuverse Host"));
...
async fn production_assets_and_spa_routes_share_the_static_root() {
    ...
    std::fs::write(root.path().join("index.html"), "<main>Cinyuverse</main>").expect("index");
    ...
    for (path, expected_type, expected_body) in [
        ("/", "text/html; charset=utf-8", "<main>Cinyuverse</main>"),
        (
            "/app.js",
