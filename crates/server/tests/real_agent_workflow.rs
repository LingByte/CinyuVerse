...
#[tokio::test]
#[ignore = "release gate: requires CINYUVERSE_REAL_AGENT_ACP and an authenticated Codex CLI"]
async fn real_agent_completes_a_headless_workflow_through_conversation_control() {
    let acp_path = std::env::var("CINYUVERSE_REAL_AGENT_ACP")
        .expect("set CINYUVERSE_REAL_AGENT_ACP to an absolute codex-acp executable");
    let codex_path = std::env::var("CINYUVERSE_REAL_CODEX")
        .expect("set CINYUVERSE_REAL_CODEX to an absolute authenticated codex executable");
    ...
    let version = call(
        ...
        serde_json::json!({"request": {"definition": {
            ...
            "steps": [{
                ...
                "kind": "agent",
                "agentId": "codex",
                "prompt": "Reply with exactly: CINYUVERSE_REAL_AGENT_OK",
    ...
    let timeout_seconds = std::env::var("CINYUVERSE_REAL_AGENT_TIMEOUT_SECONDS")
        .ok()
    ...
    let output = output_events
        ...
        .collect::<String>();
    assert!(
        output.contains("CINYUVERSE_REAL_AGENT_OK"),
