...
async fn one_authenticated_application_surface_opens_product_domains_for_the_web_ui() {
    ...
    .await;
    assert_eq!(installed["id"], "cinyuverse.office");

    let product_plugins = call(app.clone(), "plugin_control_catalog", serde_json::json!({})).await;
    assert_eq!(product_plugins["plugins"][0]["id"], "cinyuverse.office");
    assert_eq!(
    ...
    let detail = call(
        ...
        serde_json::json!({ "pluginId": "cinyuverse.office" }),
    )
    .await;
    ...
    let missing_runtime = call_status(
        ...
        serde_json::json!({ "pluginId": "cinyuverse.office", "runtimeId": "missing" }),
    )
    .await;
