//! Host-side Agent catalog and user-environment installer for `cinyuverse-server`.
...
fn print_catalog(entries: &[AgentCatalogEntry]) {
    ...
    if registry.is_empty() {
        println!("  (refresh with `cinyuverse-server list --refresh`)");
    } else {
...
async fn ensure_membership(pool: &SqlitePool, agent_id: &AgentId) -> anyhow::Result<()> {
    ...
    let snapshot = load_registry_snapshot(pool, true).await?.ok_or_else(|| {
        anyhow::anyhow!("ACP Registry is empty; run `cinyuverse-server list --refresh`")
    })?;
    let entry = snapshot
        .entries
        .iter()
...
async fn install_binary(
    ...
    let bytes = reqwest::get(&component.resolved_source)
        ...
        .bytes()
        .await?;
    let staging = std::env::temp_dir().join(format!("cinyuverse-agent-{}", Uuid::new_v4()));
