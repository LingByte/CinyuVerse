...
mod tests {
    ...
    fn upgrade_rejects_a_bad_checksum_before_touching_data() {
        ...
        fs::write(release.join("cinyuverse-server"), "new").unwrap();
        fs::write(data.join("db.sqlite"), "keep").unwrap();
        fs::write(release.join("SHA256SUMS"), "deadbeef  cinyuverse-server\n").unwrap();
        ...
        assert!(!install.join("cinyuverse-server").exists());
    }
    ...
    fn upgrade_snapshots_data_and_replaces_binaries() {
        ...
        fs::create_dir_all(&install).unwrap();
        fs::create_dir_all(&data).unwrap();
        fs::write(release.join("cinyuverse-server"), "server-v2").unwrap();
        fs::write(release.join("cinyuverse-mcp"), "mcp-v2").unwrap();
        let digest_server = hex_sha256(b"server-v2");
        let digest_mcp = hex_sha256(b"mcp-v2");
        fs::write(
            release.join("SHA256SUMS"),
            format!("{digest_server}  cinyuverse-server\n{digest_mcp}  cinyuverse-mcp\n"),
        )
        .unwrap();
        fs::write(install.join("cinyuverse-server"), "server-v1").unwrap();
        fs::write(data.join("db.sqlite"), "state").unwrap();

        let plan = plan_host_upgrade(&release, &install, &data).unwrap();
        let snapshot = apply_host_upgrade(&plan).unwrap();
        assert_eq!(
            fs::read_to_string(install.join("cinyuverse-server")).unwrap(),
            "server-v2"
        );
        assert_eq!(
