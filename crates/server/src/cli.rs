    ...
    S: AsRef<str>,
{
    ...
    let mut allow_lan = std::env::var("CINYUVERSE_SERVER_ALLOW_LAN").as_deref() == Ok("1");
...
pub fn usage() -> &'static str {
    "Usage:\n  \
     cinyuverse-server                      Start on loopback\n  \
     cinyuverse-server serve                Start the Web UI on the LAN and print the host token\n  \
     cinyuverse-server serve --local        Start the Web UI on loopback only\n  \
     cinyuverse-server serve --port N\n  \
     cinyuverse-server serve --rotate-token\n  \
     cinyuverse-server list [--json] [--refresh]\n  \
     cinyuverse-server install <agent-id> [--yes]\n"
}
                ...
                match token.as_str() {
                    ...
                    "--help" | "-h" => {
                        return Err(ParseError(
                            "Usage: cinyuverse-server list [--json] [--refresh]".to_string(),
                        ));
                    }
        ...
        "install" => {
            ...
            while let Some(token) = tokens.next() {
                match token.as_str() {
                    ...
                    "--help" | "-h" => {
                        return Err(ParseError(
                            "Usage: cinyuverse-server install <agent-id> [--yes]".to_string(),
                        ));
                    }
            ...
            let agent_id = agent_id.ok_or_else(|| {
                ParseError("Usage: cinyuverse-server install <agent-id> [--yes]".to_string())
            })?;
...
fn env_listen_port() -> Option<u16> {
    std::env::var("CINYUVERSE_SERVER_LISTEN")
        .ok()?
        .parse::<SocketAddr>()
        .ok()
