...
/// 唯一真相源；Cinyuverse 预设只在原生 `model_provider = "cinyuverse"` 时才算启用。
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct NativeCodexState {
    pub providers: Vec<NativeCodexProvider>,
    /// `model_provider` 键，包括 Cinyuverse 投影槽 `cinyuverse`。
    ...
    /// `auth.json` 中的 `OPENAI_API_KEY` 原文，纳入 Cinyuverse 预设时复用。
...
const NATIVE_ENDPOINT_PROVIDER_ID: &str = "__cinyuverse_native_endpoint__";
...
pub async fn bind(
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        if is_native_codex_channel(&provider, native_codex.as_ref()) {
            if native_uses_cinyuverse_projection(native_codex.as_ref()) {
                let backup = store
                    .projection_backups
                    .get(agent_id.as_str())
                    .cloned()
...
/// 原生 Codex Provider 已纳入 Cinyuverse 预设。启用态以 `config.toml` 的
/// `model_provider` 为准：指向 `cinyuverse` 槽时沿用 store 绑定，否则按原生
/// 当前项的 id / URL 标记对应预设。
fn project_with_codex_native(
    ...
    if native_uses_cinyuverse_projection(Some(native)) {
        return view;
    }
...
fn apply_native_active_binding<'a>(
    ...
    let match_id = active_id.filter(|id| *id != "cinyuverse");
    let matched_id = view
        .providers
        .iter()
        .find(|provider| {
...
fn codex_native_drafts(native: Option<&NativeCodexState>) -> Vec<NativeProviderDraft> {
    ...
    let mut drafts = native
        .providers
        .iter()
        .filter(|provider| provider.id != "cinyuverse" && !provider.api_url.is_empty())
    ...
    drafts
        ...
        .filter(|draft| {
            !draft.api_url.is_empty()
                && draft.source_id != "cinyuverse"
                && !draft.source_id.ends_with(":cinyuverse")
...
fn reconcile_codex_store_bindings(
    ...
    if native_uses_cinyuverse_projection(native) {
        return false;
    }
    store.bindings.remove(agent_id.as_str());
    store.projection_backups.remove(agent_id.as_str());
...
fn native_uses_cinyuverse_projection(native: Option<&NativeCodexState>) -> bool {
    native.is_some_and(|state| state.active_provider.as_deref() == Some("cinyuverse"))
}
...
async fn native_kimi_draft(kimi_home: &Path) -> Result<Option<ImportDraft>, String> {
    ...
    let provider = table
        ...
        .and_then(|providers| providers.get("cinyuverse"))
        .and_then(toml::Value::as_table);
    ...
    let model = table
        .get("models")
        .and_then(toml::Value::as_table)
        .and_then(|models| models.get("cinyuverse"))
        .and_then(toml::Value::as_table)
...
const CODEX_PROVIDER_ENTRY_BACKUP_KEY: &str = "model_providers.cinyuverse";
const CODEX_CATALOG_FILE: &str = "cinyuverse-model-catalog.json";
const CODEX_SOURCE_FILE: &str = "cinyuverse-model-catalog.source.json";
...
fn restore_codex_toml_values(
    ...
    let original_cinyuverse = backup
        .toml_values
        .get(CODEX_PROVIDER_ENTRY_BACKUP_KEY)
        .cloned()
        .flatten();
    if original_cinyuverse.is_some() || table.contains_key("model_providers") {
        let providers = table
        ...
        match original_cinyuverse {
            Some(value) => {
                providers.insert("cinyuverse".to_string(), value);
            }
            None => {
                providers.remove("cinyuverse");
            }
        }
...
async fn apply_codex(
    ...
    let provider_name = "cinyuverse".to_string();
    table.insert(
        "model_provider".to_string(),
...
async fn apply_grok(grok_home: &Path, provider: &StoredProvider) -> Result<(), super::NativeError> {
    ...
    let cinyuverse = toml_table_entry(model_root, "cinyuverse")?;
    cinyuverse.insert(
        "model".to_string(),
    ...
    if let Some(context) = spec.context_window {
        cinyuverse.insert("context_window".to_string(), toml::Value::Integer(context));
    }
...
async fn apply_kimi(kimi_home: &Path, provider: &StoredProvider) -> Result<(), super::NativeError> {
    ...
    let cinyuverse_provider = toml_table_entry(providers, "cinyuverse")?;
    cinyuverse_provider.insert(
        "type".to_string(),
    ...
    let models = toml_table_entry(&mut table, "models")?;
    let cinyuverse_model = toml_table_entry(models, "cinyuverse")?;
    cinyuverse_model.insert(
        "provider".to_string(),
        toml::Value::String("cinyuverse".to_string()),
    );
    cinyuverse_model.insert(
...
async fn seed_kimi_gate_credential(path: &Path) -> Result<(), super::NativeError> {
    ...
    if let Some(bytes) = original.as_deref().filter(|bytes| !bytes.is_empty())
        ...
        && existing.get("_cinyuverse_synthetic") != Some(&Value::Bool(true))
        && existing
            .get("access_token")
            .and_then(Value::as_str)
    ...
    apply_projection_mutations(&[NativeFileMutation {
        ...
        replacement: Some(
            serde_json::to_vec_pretty(&serde_json::json!({
                "access_token": "cinyuverse-local-gate",
                "_cinyuverse_synthetic": true
            }))
...
async fn apply_openclaw(
    ...
    model.insert(
        "primary".to_string(),
        Value::String(format!("cinyuverse/{}", model_default(&provider.model))),
    );
...
/// Claude Code 会在 `ANTHROPIC_BASE_URL` 后自动追加 `/v1/messages`，
/// 因此写入原生配置时需要去掉用户填写的 `/v1` 后缀，避免拼出
/// `/v1/v1/messages` 这样的无效路径。
fn strip_api_version_suffix(url: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn strip_api_version_suffix_removes_trailing_v1() {
        assert_eq!(
            strip_api_version_suffix("https://llmapi.qiniu.io/v1"),
            "https://llmapi.qiniu.io"
        );
        assert_eq!(
            strip_api_version_suffix("https://llmapi.qiniu.io/v1/"),
            "https://llmapi.qiniu.io"
        ...
        assert_eq!(
            strip_api_version_suffix("https://api.openai.com/v1"),
            "https://api.openai.com"
        );
        // 不含 /v1 时保持原样
        assert_eq!(
            strip_api_version_suffix("https://llmapi.qiniu.io"),
            "https://llmapi.qiniu.io"
        );
        assert_eq!(
            strip_api_version_suffix("https://example.com/api/v1"),
    ...
    async fn enabling_native_codex_provider_restores_official_config_and_external_catalog() {
        ...
        assert!(
            config
                .get("model_providers")
                .and_then(toml::Value::as_table)
                .is_none_or(|providers| !providers.contains_key("cinyuverse"))
        );
        assert_eq!(
            tokio::fs::read_to_string(codex_home.join("external.json"))
                .await
    ...
    async fn codex_provider_projects_official_provider_table_and_restores_catalog_on_unbind() {
        ...
        assert_eq!(
            bound_config
                .get("model_catalog_json")
                .and_then(toml::Value::as_str),
            Some("cinyuverse-model-catalog.json")
        );
    ...
    #[tokio::test]
    async fn native_codex_provider_appears_as_bound_when_cinyuverse_has_no_binding() {
        let temp = tempfile::tempdir().unwrap();
    ...
    #[tokio::test]
    async fn cinyuverse_binding_takes_precedence_over_native_codex_provider() {
        let temp = tempfile::tempdir().unwrap();
        let codex_home = temp.path().join("home/.codex");
        let store_path = temp.path().join("data/agent-model-providers.json");
        tokio::fs::create_dir_all(&codex_home).await.unwrap();
        ...
        let created = save(
            ...
            AgentModelProviderSaveRequest {
                id: None,
                name: "Cinyuverse Gateway".to_string(),
                agent_id: agent_id.clone(),
                api_url: "https://cinyuverse.example/v1".to_string(),
                api_key: Some("cinyuverse-secret".to_string()),
                model: "cinyuverse-model".to_string(),
            },
        )
        .await
        .unwrap();
        let managed_id = created
            .providers
            .iter()
            .find(|provider| provider.name == "Cinyuverse Gateway")
            .expect("saved Cinyuverse provider")
            .id
            .clone();
        bind(
            &store_path,
        ...
        let native = view
            ...
            .expect("native Codex providers remain visible after a Cinyuverse bind");
        assert!(!native.bound);
        assert!(native.managed);
    }

    #[tokio::test]
    async fn native_config_wins_over_a_stale_cinyuverse_store_binding() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path().join("home");
        let codex_home = home.join(".codex");
        let store_path = temp.path().join("data/agent-model-providers.json");
        ...
        let managed = view
            ...
            .iter()
            .find(|provider| provider.id == managed_id)
            .expect("Cinyuverse preset remains listed");
        assert!(!managed.bound);
        assert!(managed.managed);
    ...
    #[tokio::test]
    async fn native_deepseek_provider_is_ready_without_a_cinyuverse_slot() {
        let temp = tempfile::tempdir().unwrap();
        ...
        assert!(native_codex_provider_ready(&state));
        assert!(!native_uses_cinyuverse_projection(Some(&state)));
    }
    ...
    async fn adopted_native_codex_provider_can_be_enabled_after_creating_another() {
        ...
        let cinyuverse = bound
            .providers
            .iter()
            .find(|provider| provider.name == "Cinyuverse Gateway")
            .unwrap();
        assert!(!cinyuverse.bound);
    }
