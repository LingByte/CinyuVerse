...
fn validate_model_endpoint(base_url: &str) -> Result<url::Url, String> {
    ...
    // 模型探测走 OpenAI 兼容的 `/v1/models` 路径。若用户填写的 URL
    // 不含 `/v1`（例如 `https://host`），自动补齐，避免拼出 `/models`
    // 这种无效路径；若已含 `/v1` 则保持原样。
    let path = url.path().trim_end_matches('/');
    if !path.ends_with("/v1") && !path.contains("/v1/") {
        url.set_path(&format!("/v1/"));
...
mod tests {
    ...
    fn endpoint_appends_v1_when_missing() {
        // 用户填写不带 /v1 的 URL 时，模型探测应自动补齐 /v1/，
        // 这样 .join("models") 才能拼出正确的 /v1/models 路径。
        assert_eq!(
            validate_model_endpoint("https://example.com")
                .unwrap()
                .as_str(),
            "https://example.com/v1/"
        );
