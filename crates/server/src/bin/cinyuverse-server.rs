...
async fn run(launch: ServerLaunch) -> ExitCode {
    ...
    if let Some(static_root) = std::env::var_os("CINYUVERSE_STATIC_ROOT") {
    ...
    if let Ok(origins) = std::env::var("CINYUVERSE_SERVER_ALLOWED_ORIGINS") {
    ...
    let mut headless = match HeadlessServer::bootstrap(bootstrap).await {
        ...
        Err(error) => {
            tracing::error!(%error, "failed to initialize cinyuverse-server");
    ...
    if let Some(token) = headless.take_issued_token() {
        ...
        if !launch.reveal_console {
            println!(
                "Cinyuverse server token (saved for `serve`; keep it secret): {}",
                token.expose_once()
            );
        }
    }
    ...
    match headless.serve().await {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            tracing::error!(%error, "cinyuverse-server stopped");
            ExitCode::from(1)
        }
    }
}
...
fn resolve_startup_token(
    ...
    if let Some(value) = std::env::var("CINYUVERSE_SERVER_TOKEN")
        .ok()
