...
fn apply_cors_headers(headers: &mut HeaderMap, origin: &str) {
    ...
    headers.insert(
        ...
        HeaderValue::from_static("authorization, content-type, x-cinyuverse-protocol-version"),
    );
...
async fn capabilities<R>(
    ...
    if let Some(requested) = headers
        .get("x-cinyuverse-protocol-version")
        .and_then(|value| value.to_str().ok())
    {
...
fn ws_token_from_protocols(protocols: &str) -> Option<String> {
    ...
    protocols
        .split(',')
        .map(str::trim)
        .find_map(|protocol| protocol.strip_prefix("cinyuverse.token."))
