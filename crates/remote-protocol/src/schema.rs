...
const JSON_SCHEMA_ID: &str = "https://schemas.cinyuverse.dev/remote-protocol/v1/schema.json";
...
pub fn protocol_schema_bundle() -> ProtocolSchemaBundle {
    ...
    root.insert(
        "title".to_owned(),
        Value::String("Cinyuverse Remote Protocol v1".to_owned()),
    );
...
fn openapi_document(json_schema: &Value) -> Value {
    ...
    json!({
        "openapi": "3.1.0",
        "info": {
            "title": "Cinyuverse Remote Protocol",
