...
const BUNDLED_SCRIPT: &[u8] =
    include_bytes!("../../../packages/cinyuverse-workflow-mcp/dist/mcp/workflow-control.mjs");

fn main() {
    if let Err(error) = run() {
        eprintln!("cinyuverse-workflow-mcp: {error}");
        std::process::exit(2);
    }
}
...
fn run() -> io::Result<()> {
    ...
    let node = resolve_node().ok_or_else(|| {
        io::Error::new(
            ...
            "Node.js is required to run cinyuverse-workflow-mcp",
        )
    })?;
...
fn materialize_script() -> io::Result<PathBuf> {
    if let Some(explicit) = env::var_os("CINYUVERSE_WORKFLOW_MCP_SCRIPT") {
    ...
    let dest = utils::path::get_cinyuverse_temp_dir().join("cinyuverse-workflow-mcp.mjs");
...
fn resolve_node() -> Option<PathBuf> {
    if let Some(explicit) = env::var_os("CINYUVERSE_NODE") {
        let path = PathBuf::from(explicit);
        if path.is_file() {
            return Some(path);
        }
