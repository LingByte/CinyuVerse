...
async fn rejects_digest_mismatch_before_probe() {
    ...
    let request = ToolRequest {
        ...
        url: "https://downloads.cinyuverse.dev/officecli".to_string(),
...
async fn authoritative_runtime_rejects_unsafe_download_urls_before_fetch() {
    for unsafe_url in [
        ...
        "https://downloads.cinyuverse.dev/officecli?token=release-secret",
        "https://downloads.cinyuverse.dev/officecli#release-secret",
        "https://user:release-secret@downloads.cinyuverse.dev/officecli",
    ] {
...
fn request(version: &str, bytes: &[u8], probe_args: &[&str]) -> ToolRequest {
    ToolRequest {
        ...
        target: "aarch64-apple-darwin".to_string(),
        url: format!("https://downloads.cinyuverse.dev/fixtures/officecli/{version}"),
    ...
    let runtime = ToolRuntime::new(
        ...
        Arc::new(CatalogDownloader {
            artifacts: HashMap::from([
                (
                    "https://downloads.cinyuverse.dev/fixtures/officecli/1.0.0".to_string(),
                    b"v1".to_vec(),
                ),
                (
                    "https://downloads.cinyuverse.dev/fixtures/officecli/2.0.0".to_string(),
                    b"v2".to_vec(),
                ),
            ]),
        }),
    ...
    let runtime = ToolRuntime::new(
        ...
        Arc::new(CatalogDownloader {
            artifacts: HashMap::from([
                ...
                (
                    "https://downloads.cinyuverse.dev/fixtures/officecli/3.0.0".to_string(),
                    b"v3".to_vec(),
                ),
            ]),
        }),
...
async fn lease_acquisition_is_rejected_while_release_deletes_a_version() {
    ...
    let v1_lock = ToolInstallationLock {
        ...
        source_url: "https://downloads.cinyuverse.dev/fixtures/officecli/1.0.0".into(),
    ...
    let v3_lock = ToolInstallationLock {
        version: "3.0.0".into(),
        source_url: "https://downloads.cinyuverse.dev/fixtures/officecli/3.0.0".into(),
