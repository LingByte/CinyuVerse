...
fn desktop_bundle_identity(dev_bundle: bool) -> DesktopBundleIdentity {
    if dev_bundle {
        DesktopBundleIdentity {
            display_name: "Cinyuverse Dev",
            identifier: "com.cinyuverse.app.dev",
            name: "Cinyuverse Dev",
        }
    } else {
        DesktopBundleIdentity {
            display_name: "Cinyuverse",
            identifier: "com.cinyuverse.app",
            name: "Cinyuverse",
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let target = env::var_os("CINYUVERSE_BUILD_TARGET")
...
fn stage_platform_runtime(
    ...
    let frameworks = app_root.join("cinyuverse.app/Contents/Frameworks");
    let framework_helpers = frameworks
        .join("Chromium Embedded Framework.framework")
        .join("Helpers");
    fs::create_dir_all(&framework_helpers)?;
    for helper_name in [
        "cinyuverse Helper.app",
        "cinyuverse Helper (Alerts).app",
        "cinyuverse Helper (GPU).app",
        "cinyuverse Helper (Plugin).app",
        "cinyuverse Helper (Renderer).app",
    ] {
        fs::rename(
    ...
    Ok(vec![
        "app/cinyuverse.app/Contents/Frameworks/Chromium Embedded Framework.framework/Chromium Embedded Framework",
        "app/cinyuverse.app/Contents/Frameworks/Chromium Embedded Framework.framework/Helpers/cinyuverse Helper.app/Contents/MacOS/cinyuverse Helper",
        "framework-links/cinyuverse Helper.app",
    ])
}
...
mod identity_tests {
    ...
    fn product_and_dev_bundles_use_distinct_identifiers() {
        ...
        assert_eq!(product.identifier, "com.cinyuverse.app");
        assert_eq!(product.name, "Cinyuverse");
        assert_eq!(dev.identifier, "com.cinyuverse.app.dev");
        assert_eq!(dev.name, "Cinyuverse Dev");
...
mod tests {
    ...
    fn uses_target_directory_when_it_contains_the_app_binary() {
        ...
        fs::write(
            target_path.join(format!("cinyuverse{}", std::env::consts::EXE_SUFFIX)),
            [],
        )
        .unwrap();
...
fn stage_platform_runtime(
    ...
    let executable = cef::build_util::linux::bundle(stage_root, target_path, "cinyuverse")?;
...
fn stage_platform_runtime(
    ...
    let executable = cef::build_util::win::bundle(stage_root, target_path, "cinyuverse")?;
    fs::remove_file(executable)?;
    let generated_manifest = stage_root.join("cinyuverse.exe.manifest");
