...
cef::wrap_browser_process_handler! {
    struct CinyuverseBrowserProcessHandler {
...
impl CefBootstrap {
    pub fn initialize(
        ...
        let process_handler = CinyuverseBrowserProcessHandler::new(scheduler);
        let mut application = CinyuverseCefApp::new(process_handler);
        let args = Args::new();
...
impl CefSession {
    ...
    fn create_browser(
        ...
        let mut client =
            CinyuverseClient::new(tab_id.clone(), self.runtime.clone(), self.registry.clone());
...
cef::wrap_dev_tools_message_observer! {
    struct CinyuverseDevToolsObserver {
        tab_id: BrowserTabId,
        runtime: Arc<BrowserRuntime>,
    }
...
cef::wrap_client! {
    ...
    impl Client {
        fn display_handler(&self) -> Option<DisplayHandler> {
            Some(CinyuverseDisplayHandler::new(self.tab_id.clone(), self.runtime.clone()))
        }

        fn download_handler(&self) -> Option<DownloadHandler> {
            Some(CinyuverseDownloadHandler::new(
                self.tab_id.clone(),
                self.runtime.clone(),
                self.registry.clone(),
            ))
        ...
        fn life_span_handler(&self) -> Option<LifeSpanHandler> {
            Some(CinyuverseLifeSpanHandler::new(
                self.tab_id.clone(),
                self.runtime.clone(),
                self.registry.clone(),
            ))
        ...
        fn load_handler(&self) -> Option<LoadHandler> {
            Some(CinyuverseLoadHandler::new(self.tab_id.clone(), self.runtime.clone()))
        }

        fn permission_handler(&self) -> Option<PermissionHandler> {
            Some(CinyuversePermissionHandler::new(
                self.tab_id.clone(),
                self.runtime.clone(),
                self.registry.clone(),
            ))
...
cef::wrap_download_handler! {
    struct CinyuverseDownloadHandler {
        tab_id: BrowserTabId,
        runtime: Arc<BrowserRuntime>,
        registry: Rc<RefCell<BrowserRegistry>>,
    }
...
cef::wrap_permission_handler! {
    struct CinyuversePermissionHandler {
        tab_id: BrowserTabId,
        runtime: Arc<BrowserRuntime>,
        registry: Rc<RefCell<BrowserRegistry>>,
    }
...
cef::wrap_task! {
    struct CinyuverseDestroyBrowserViewTask {
        browser: Browser,
    }
...
fn schedule_browser_view_destruction(browser: &Browser) {
    let mut task = CinyuverseDestroyBrowserViewTask::new(browser.clone());
...
cef::wrap_life_span_handler! {
    struct CinyuverseLifeSpanHandler {
        tab_id: BrowserTabId,
        runtime: Arc<BrowserRuntime>,
        registry: Rc<RefCell<BrowserRegistry>>,
    }
    ...
    impl LifeSpanHandler {
        ...
        fn on_after_created(&self, browser: Option<&mut Browser>) {
            ...
            let registration = browser.host().and_then(|host| {
                let mut observer = CinyuverseDevToolsObserver::new(
                    self.tab_id.clone(),
                    self.runtime.clone(),
                );
...
cef::wrap_display_handler! {
    struct CinyuverseDisplayHandler {
        tab_id: BrowserTabId,
        runtime: Arc<BrowserRuntime>,
    }
