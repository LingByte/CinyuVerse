    ...
    pub async fn cleanup_workspace(
        ...
        if repositories.is_empty() {
            ...
            if !WorkspaceManager::is_app_owned_workspace_dir(&workspace_dir) {
                tracing::warn!(
                    "Refusing to remove workspace directory outside Cinyuverse-owned storage: {}",
...
mod tests {
    ...
    fn temp_external_path(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("cinyuverse-external-{name}-{}", Uuid::new_v4()))
    }
    ...
    fn test_container(pool: SqlitePool) -> LocalContainerService {
        ...
        LocalContainerService {
            ...
            settings_path: std::env::temp_dir()
                .join(format!("cinyuverse-test-settings-{}.json", Uuid::new_v4())),
        }
    }
