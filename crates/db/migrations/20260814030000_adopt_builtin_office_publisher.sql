...
SELECT
    'cinyuverse', plugin_id, version, package_digest, source_kind, source_path,
    manifest_json, json_set(package_json, '$.publisher', 'cinyuverse'), created_at
FROM plugin_packages_v4
WHERE publisher = 'legacy.local'
  AND plugin_id = 'cinyuverse.office'
  AND source_kind = 'builtin';

UPDATE plugin_installations_v4
SET publisher = 'cinyuverse', updated_at = datetime('now','subsec')
WHERE plugin_id = 'cinyuverse.office'
  AND publisher = 'legacy.local'
  AND EXISTS (
      SELECT 1
      FROM plugin_packages_v4
      WHERE publisher = 'cinyuverse'
        AND plugin_id = 'cinyuverse.office'
