...
mod tests {
    ...
    fn repo_path_keeps_repo_subdirectory_for_managed_workspace_container() {
        let workspace = sample_workspace(
            Some("C:/Users/test/.cinyuverse-workspaces/ws-123"),
            true,
        ...
        assert_eq!(
            workspace.repo_path(&repo),
            Some(PathBuf::from(
                "C:/Users/test/.cinyuverse-workspaces/ws-123/repo-feature-a"
            ))
        );
    }
    ...
    fn container_ref_gitdir_path_is_normalized_to_worktree_root() {
        let root = std::env::temp_dir().join(format!("cinyuverse-workspace-{}", Uuid::new_v4()));
