    ...
    async fn legacy_builtin_office_adopts_canonical_publisher() {
        ...
        sqlx::query(
            "INSERT INTO plugin_packages_v4
             ...
             VALUES ('legacy.local', 'cinyuverse.office', '2.0.0', 'legacy:office', 'builtin',
                     '/tmp/office', '{}', '{\"id\":\"cinyuverse.office\"}', datetime('now'))",
        )
        .execute(&pool)
        .await
        ...
        sqlx::query(
            "INSERT INTO plugin_installations_v4
             ...
             VALUES ('cinyuverse.office', 'legacy.local', 'legacy:office',
        ...
        let publisher: String = sqlx::query_scalar(
            "SELECT publisher FROM plugin_installations_v4 WHERE plugin_id = 'cinyuverse.office'",
        )
        .fetch_one(&pool)
        .await
        ...
        let package_publisher: String = sqlx::query_scalar(
            "SELECT json_extract(package_json, '$.publisher')
             ...
             WHERE publisher = 'cinyuverse' AND plugin_id = 'cinyuverse.office'",
        )
        .fetch_one(&pool)
        .await
        ...
        let legacy_packages: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM plugin_packages_v4
             WHERE publisher = 'legacy.local' AND plugin_id = 'cinyuverse.office'",
        )
        .fetch_one(&pool)
        .await
        .expect("legacy package count");

        assert_eq!(publisher, "cinyuverse");
        assert_eq!(package_publisher, "cinyuverse");
...
fn sqlite_connect_options(database_path: &Path) -> SqliteConnectOptions {
    ...
    //
    // Use a filesystem path rather than interpolating `sqlite://{native path}`.
    // Windows paths contain `:` and `\`, which URL parsers treat as a host.
...
impl DBService {
    ...
    /// Open the canonical Cinyuverse database beneath an explicit data directory.
    ///
