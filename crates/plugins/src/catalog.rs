...
pub const DEFAULT_MARKETPLACE_ORIGIN: &str = "https://cinyuverse.xforever.xin";
...
const BUNDLED_TOPIC_CATEGORIES: &[(&str, &str)] = &[
    ("cinyuverse.office", "productivity"),
    ("cinyuverse.session-enhance", "productivity"),
    ("cinyuverse.multi-agent", "agent"),
    ("cinyuverse.workflow-creator", "workflow"),
    ("cinyuverse.plugin-development", "other"),
];
...
pub fn marketplace_origin() -> String {
    std::env::var("CINYUVERSE_MARKETPLACE_URL")
        .ok()
...
pub fn listing_from_package(package: &PluginPackage, offline: bool) -> CatalogListing {
    let owner = package
        .publisher
        .clone()
        .unwrap_or_else(|| "cinyuverse".to_owned());
...
pub fn origin_owner_name(origin: &str, fallback_id: &str) -> (String, String) {
    ...
    ("cinyuverse".to_owned(), fallback_id.to_owned())
}
...
mod tests {
    ...
    fn website_list_record_maps_to_catalog_listing() {
        let record: PublishedRecord = serde_json::from_str(
            r#"{
                ...
                "authorId": "cinyuverse",
                "name": "Drawio",
                "summary": "Drawio diagrams in Cinyuverse.",
                "version": "1.0.0",
                "category": "productivity",
                "sourceKind": "github",
                "repo": "https://github.com/LingByte/cinyuverse-drawio",
                "githubOwner": "LingByte",
                "githubRepo": "cinyuverse-drawio",
                "githubBranch": "main"
            }"#,
        )
        .unwrap();
        let listing = record.into_listing().unwrap();
        assert_eq!(listing.owner, "cinyuverse");
        ...
        assert_eq!(
            ...
            Some("https://github.com/LingByte/cinyuverse-drawio")
        );
    }
    ...
    fn published_catalog_keeps_non_official_plugins_in_community() {
        let drawio = CatalogListing {
            owner: "cinyuverse".into(),
            plugin_name: "drawio".into(),
            tag: "main".into(),
            version: "1.0.0".into(),
            display_name: "Drawio".into(),
            summary: "Drawio diagrams in Cinyuverse.".into(),
    ...
    fn github_tarball_uses_tar_gz_suffix() {
        assert_eq!(
            marketplace_archive_suffix(
                "https://codeload.github.com/LingByte/cinyuverse-drawio/tar.gz/main"
            ),
            "tar.gz"
        );
        assert_eq!(
            marketplace_archive_suffix("https://cinyuverse.xforever.xin/files/a.vxp"),
            "vxp"
        );
    }
    ...
    fn official_catalog_keeps_one_multi_agent_product() {
        let collaboration = listing(
            "collaboration",
            "Cinyuverse Collaboration",
            "让父 Agent 通过 cinyuverse-mcp 把工作委派给其它 Agent。",
        );
        ...
        let office = listing("cinyuverse.office", "办公套件", "Office files");
        ...
        assert_eq!(page.official.len(), 1);
        assert_eq!(page.official[0].plugin_name, "cinyuverse.multi-agent");
    }
