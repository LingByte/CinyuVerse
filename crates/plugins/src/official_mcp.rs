...
pub const DELEGATION_MCP_NAME: &str = "cinyuverse-delegation-mcp";

/// Host-injected / native-projected MCP identity for session enhancement.
pub const SESSION_MCP_NAME: &str = "cinyuverse-session-mcp";

/// Host-injected MCP identity for plugin development link requests.
pub const PLUGIN_DEV_MCP_NAME: &str = "cinyuverse-plugin-dev-mcp";

/// Host-injected / native-projected MCP identity for Workflow Creator.
pub const WORKFLOW_MCP_NAME: &str = "cinyuverse-workflow-mcp";
...
impl OfficialMcpRuntime {
    ...
    pub fn allow_workflow_mcp(&self) -> bool {
        self.bindings().iter().any(|binding| {
            binding.binary_id == "cinyuverse-workflow-mcp" || binding.product == "workflow"
        })
    }
...
pub fn host_family_product(spec: &Value) -> Option<&'static str> {
    ...
    match (managed.get("product").and_then(Value::as_str), binary_id) {
        ...
        (Some("workflow"), _) | (_, "cinyuverse-workflow-mcp") => Some("workflow"),
...
/// `--features` value consumed by `cinyuverse-mcp`.
...
/// `CINYUVERSE_CONVERSATION_ID`, and ACP agents still get per-session `session/new`.
...
mod tests {
    ...
    fn official_host_family_keeps_product_mcp_identity() {
        ...
        assert_eq!(
            projected_mcp_server_id("cinyuverse.multi-agent", "cinyuverse-delegation-mcp", &spec),
            DELEGATION_MCP_NAME
        );
        assert_eq!(
    ...
    fn host_family_stdio_spec_is_plugin_lifetime_not_session_scoped() {
        ...
        assert_eq!(spec["command"], "/opt/cinyuverse-mcp");
        let args = spec["args"]
            .as_array()
            .expect("args")
            .iter()
        ...
        assert_eq!(
            host_family_stdio_spec("/opt/cinyuverse-workflow-mcp", "workflow", "", None, None)["args"],
            json!([])
        );
    }
