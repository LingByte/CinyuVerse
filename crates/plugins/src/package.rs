...
impl PluginPackage {
    ...
    pub fn inspect(root: &Path, source_kind: PluginSourceKind) -> Result<Self, PluginError> {
        ...
        let cinyuverse_path = root.join(".cinyuverse-plugin/plugin.json");
        if !cinyuverse_path.is_file() {
            return Err(PluginError::invalid_manifest(format!(
                "missing `{}`",
                cinyuverse_path.display()
            )));
        }

        let manifest_text = fs::read_to_string(&cinyuverse_path)
        ...
        let mut formats = vec![PackageFormat::Cinyuverse];
    ...
    pub fn for_test(
        ...
        Self {
            ...
            publisher: Some("dev.cinyuverse.test".to_owned()),
            name: name.to_owned(),
            ...
            execution_root: None,
            formats: vec![PackageFormat::Cinyuverse],
        ...
        if relative.ends_with(".vxp")
            || matches!(
                ...
                "config.json"
                    ...
                    | ".cinyuverse-plugin/developer-link.json"
            )
        {
            continue;
        }
...
fn validate_v4_contract(contract: V4Contract<'_>) -> Result<(), PluginError> {
    ...
    if let Some(name) = engines
        .keys()
        .find(|name| !matches!(name.as_str(), "cinyuverse" | "pluginSdk"))
    {
        return Err(PluginError::invalid_manifest(format!(
