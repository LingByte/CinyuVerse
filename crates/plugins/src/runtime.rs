...
impl PluginWorkerRuntimeProvider {
            ...
            "native" => package
                ...
                .ok_or_else(|| {
                    PluginError::runtime_not_ready(
                        "cinyuverse-plugin-worker-native",
    ...
    pub async fn resolve_python(&self) -> Result<PathBuf, PluginError> {
        ...
        let lock = crate::plugin_worker_cpython_lock().ok_or_else(|| {
            PluginError::runtime_not_ready(
                "cinyuverse-plugin-worker-cpython",
                format!(
                    "Cinyuverse has no pinned Plugin Worker CPython artifact for {}-{}",
                    std::env::consts::OS,
                    std::env::consts::ARCH
                ),
            )
        ...
        let installation = GlobalRuntimeInstaller::new(&host)
            .install("cinyuverse.host", &runtime)
            .await?;
    ...
    pub async fn resolve(&self) -> Result<PathBuf, PluginError> {
        ...
        let runtime = plugin_worker_node_runtime().ok_or_else(|| {
            PluginError::runtime_not_ready(
                "cinyuverse-plugin-worker-node",
                format!(
                    "Cinyuverse has no pinned Plugin Worker Node.js artifact for {}-{}",
                    std::env::consts::OS,
                    std::env::consts::ARCH
                ),
            )
...
fn plugin_worker_node_runtime() -> Option<RuntimeContribution> {
    ...
    Some(RuntimeContribution {
        id: "cinyuverse-plugin-worker-node".to_owned(),
...
mod tests {
    ...
    async fn republishing_verified_runtime_clears_macos_quarantine() {
        ...
        tokio::fs::write(&executable, b"verified-runtime")
            .await
            .unwrap();
        xattr::set(&directory, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
        xattr::set(&executable, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
    ...
    fn worker_runtime_storage_clears_quarantine_from_owned_ancestors() {
        ...
        for path in [data.path(), plugins_root.as_path(), provider.root.as_path()] {
            xattr::set(path, "com.apple.quarantine", b"0081;0;Cinyuverse;").unwrap();
        }
    ...
    fn plugin_worker_node_catalog_is_pinned_for_the_current_supported_target() {
        ...
        assert_eq!(runtime.id, "cinyuverse-plugin-worker-node");
