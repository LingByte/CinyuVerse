...
fn linked_worker_package(root: &std::path::Path) -> PluginPackage {
    ...
    std::fs::write(
        ...
        r#"{
          ...
          "id":"dev.cinyuverse.drain","publisher":"dev.cinyuverse",
...
async fn update_publishes_the_exact_runtime_lock_for_the_candidate_digest() {
    ...
    let lock = control
        .runtime_for_plugin("dev.cinyuverse.drain", "fixture-cli")
        .await
        .unwrap()
...
async fn runtime_artifacts_are_content_addressed_and_locks_are_package_scoped() {
    ...
    let control = PluginControlPlane::new(Arc::new(SqlitePluginRegistry::new(pool.clone())));
    for id in ["dev.cinyuverse.one", "dev.cinyuverse.two"] {
        control
            .import(
    ...
    assert_eq!(inventory[0].referenced_plugins, vec!["dev.cinyuverse.one"]);
    assert_eq!(inventory[1].referenced_plugins, vec!["dev.cinyuverse.two"]);

    control.uninstall("dev.cinyuverse.one").await.unwrap();
    let inventory = control.runtime_inventory().await.unwrap();
    assert!(
        inventory
            .iter()
...
async fn rollback_republishes_the_retained_package_as_a_new_generation() {
    ...
    control
        ...
        .unwrap();
    control
        .set_enabled("dev.cinyuverse.rollback", true)
        .await
        .unwrap();
    ...
    assert!(
        control
            .rollback_available("dev.cinyuverse.rollback")
            .await
            .unwrap()
    );
...
async fn failed_update_candidate_preserves_published_package_and_generation() {
    ...
    control
        ...
        .unwrap();
    control
        .set_enabled("dev.cinyuverse.atomic-update", true)
        .await
        .unwrap();
    ...
    let current = control
        .plugin("dev.cinyuverse.atomic-update")
        .await
        .unwrap()
        .unwrap();
    ...
    .bind("dev.cinyuverse.atomic-update")
    .fetch_all(&pool)
    .await
    .unwrap();
    assert_eq!(
...
async fn package_update_invalidates_digest_scoped_grants_and_retains_rollback() {
    ...
    control
        ...
        .unwrap();
    control
        .grant_declared_permissions("dev.cinyuverse.scoped")
        .await
        .unwrap();
    assert!(
        control
            .capability_grants("dev.cinyuverse.scoped")
            .await
            .unwrap()
            .is_empty()
    );
...
async fn catalog_and_activation_survive_control_plane_restart() {
    ...
    first
        .set_enabled("dev.cinyuverse.persisted", true)
        .await
    ...
    assert_eq!(catalog[0].id(), "dev.cinyuverse.persisted");
