...
fn bundled_office_manifest_covers_pptx_docx_and_xlsx_actions() {
    ...
    assert_eq!(manifest.id.as_str(), "cinyuverse.office");
    assert_eq!(manifest.publisher.as_deref(), Some("cinyuverse"));
...
fn bundled_office_is_a_full_stack_portable_package() {
    ...
    assert_eq!(package.id.as_str(), "cinyuverse.office");
...
async fn office_action_resolves_skill_runtime_and_artifact_intent() {
    ...
    control
        ...
        .unwrap();
    control
        .activate_and_enable(&node, "cinyuverse.office", &[], Arc::new(EchoBroker))
        .await
        .unwrap();

    let action = control
        .resolve_action("cinyuverse.office", "create-document")
        .await
        .unwrap();
...
async fn failed_office_candidate_keeps_the_previous_generation_active() {
    ...
    let lease = manager.lease("cinyuverse.office").await.unwrap();
    ...
    manager.deactivate("cinyuverse.office").await.unwrap();
}
