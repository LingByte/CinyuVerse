...
fn write_linked_package(root: &std::path::Path, body: &str) {
    ...
    std::fs::create_dir_all(root.join(".cinyuverse-plugin")).unwrap();
    std::fs::write(
        root.join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.linked","publisher":"dev.cinyuverse","name":"Linked","version":"1.0.0",
          "readme":"README.md",
    ...
    control
        .record_runtime(
            ...
            RuntimeInstallation {
                ...
                installer: "test".into(),
                probe: Vec::new(),
                referenced_plugins: vec!["dev.cinyuverse.linked".into()],
            },
        )
        .await
        .unwrap();
    control.uninstall("dev.cinyuverse.linked").await.unwrap();
...
async fn replacement_cannot_take_over_another_publishers_plugin_id() {
    ...
    let mut original = PluginPackage::for_test(
        "dev.cinyuverse.identity",
        "Identity",
        "1.0.0",
        PluginSourceKind::Snapshot,
        root.path(),
    );
    original.publisher = Some("dev.cinyuverse".into());
    control
...
async fn uninstall_removes_membership_but_retains_runtime_artifacts() {
    ...
    control_plane
        .uninstall("dev.cinyuverse.remove")
        .await
        .expect("uninstall");

    assert!(
        control_plane
            .plugin("dev.cinyuverse.remove")
            .await
            .unwrap()
            .is_none()
    );
    ...
    control_plane
        .record_runtime(
            ...
            RuntimeInstallation {
                ...
                executable_path: "/opt/cinyuverse/shared/2/shared".into(),
                ownership: "managed".to_owned(),
...
async fn enabled_portable_action_resolves_from_the_unified_catalog() {
    ...
    control_plane
        ...
        .unwrap();
    control_plane
        .set_enabled("dev.cinyuverse.actions", true)
        .await
        .unwrap();

    let action = control_plane
        .resolve_action("dev.cinyuverse.actions", "review")
        .await
        .unwrap();
...
async fn activation_publishes_one_atomic_generation_for_app_and_agent_contributions() {
    ...
    control_plane
        .set_enabled("dev.cinyuverse.full-stack", true)
        .await
        .unwrap();
    ...
    assert_eq!(
        active
            .items
            .iter()
            .filter(|item| item.plugin_id == "dev.cinyuverse.full-stack")
            .map(|item| item.kind)
            .collect::<Vec<_>>(),
...
async fn file_opener_resolution_is_deterministic_and_disappears_when_disabled() {
    ...
    control_plane
        ...
        .unwrap();
    control_plane
        .set_enabled("dev.cinyuverse.preview", true)
        .await
        .unwrap();
    ...
    assert_eq!(resolved.plugin_id, "dev.cinyuverse.preview");
    assert_eq!(resolved.handler, "office.openPreview");

    control_plane
        .set_enabled("dev.cinyuverse.preview", false)
        .await
        .unwrap();
    assert!(
        control_plane
...
async fn file_opener_can_match_a_compound_filename_without_hijacking_json() {
    ...
    plugin.app.file_openers.push(FileOpenerContribution {
        ...
        extensions: Vec::new(),
        file_name_suffixes: vec![".cinyuverse-workflow.json".to_owned()],
        media_types: Vec::new(),
        priority: 100,
    ...
    control_plane
        ...
        .unwrap();
    control_plane
        .set_enabled("dev.cinyuverse.workflow", true)
        .await
        .unwrap();
    ...
    let resolved = control_plane
        .resolve_file_opener_for_file(Some("release.cinyuverse-workflow.json"), Some("json"), None)
        .await
        .unwrap()
...
async fn losing_a_required_plugin_unreads_dependents_without_clearing_enable_intent() {
    ...
    control_plane
        ...
        .unwrap();
    control_plane
        .set_enabled("dev.cinyuverse.base", true)
        .await
        .unwrap();
    control_plane
        .set_enabled("dev.cinyuverse.child", true)
        .await
        .unwrap();
    assert!(
        control_plane
            ...
            .items
            .iter()
            .any(|item| item.plugin_id == "dev.cinyuverse.child")
    );

    control_plane
        .set_enabled("dev.cinyuverse.base", false)
        .await
        .unwrap();

    let child = control_plane
        .plugin("dev.cinyuverse.child")
        .await
        .unwrap()
        .unwrap();
    ...
    assert!(
        control_plane
            ...
            .items
            .iter()
            .all(|item| item.plugin_id != "dev.cinyuverse.child")
    );
    assert!(
        control_plane
            .resolve_action("dev.cinyuverse.child", "missing")
            .await
            .is_err()
    );
