...
fn malicious_distribution_urls_fail_closed() {
    let source = include_str!("fixtures/office-presentation.cinyuverse-plugin.json");
    let resolver =
        ToolDependencyResolver::new(Platform::new(OperatingSystem::MacOs, Architecture::Arm64));
    ...
    for malicious in [
        ...
        let manifest = source.replace(
            "https://downloads.cinyuverse.dev/officecli/0.8.0/aarch64-apple-darwin/officecli",
            malicious,
        );
        let imported = PluginService::new()
