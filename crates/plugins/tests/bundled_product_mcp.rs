...
async fn replaced_collaboration_builtin_is_retired_into_multi_agent() {
    ...
    retired.description = Some("让父 Agent 通过 cinyuverse-mcp 把工作委派给其它 Agent。".into());
    ...
    control
        ...
        .unwrap();
    control
        .set_enabled("cinyuverse.collaboration", true)
        .await
        .unwrap();
    ...
    assert!(
        control
            .plugin("cinyuverse.collaboration")
            .await
            .unwrap()
            .is_none()
    );
...
fn bundled_multi_agent_plugin_is_the_delegation_gate() {
    ...
    assert_eq!(package.id.as_str(), "cinyuverse.multi-agent");
    assert_eq!(package.publisher.as_deref(), Some("cinyuverse"));
    ...
    assert_eq!(package.skills[0].id, "cinyuverse-multi-agent");
    let schema = package
        .config_schema
        .as_ref()
...
fn bundled_session_enhance_plugin_carries_four_tool_switches() {
    ...
    assert_eq!(package.id.as_str(), "cinyuverse.session-enhance");
...
async fn enabling_each_plugin_opens_only_its_mcp_gate() {
    ...
    control
        .set_enabled("cinyuverse.multi-agent", true)
        .await
        .unwrap();
    assert!(gate.allow_delegation_mcp());
    assert!(!gate.allow_session_mcp());
    ...
    control
        .set_enabled("cinyuverse.session-enhance", true)
        .await
        .unwrap();
    assert!(gate.allow_session_mcp());
    ...
    control
        .set_enabled("cinyuverse.multi-agent", false)
        .await
        .unwrap();
    assert!(!gate.allow_delegation_mcp());
    assert!(gate.allow_session_mcp());
...
fn bundled_plugin_development_is_skill_only() {
    ...
    assert_eq!(package.id.as_str(), "cinyuverse.plugin-development");
    assert_eq!(package.publisher.as_deref(), Some("cinyuverse"));
    assert_eq!(
        package.summary,
