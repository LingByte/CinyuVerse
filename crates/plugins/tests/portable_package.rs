...
fn v4_product_package_reads_readme_summary_content_and_mutable_root_config() {
    let root = tempfile::tempdir().expect("package root");
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.documents","publisher":"dev.cinyuverse","name":"Documents","version":"1.0.0",
          "readme":"README.md",
          "content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
    ...
    write(
        ...
        "---\nsummary: Create and review documents in Cinyuverse.\n---\n# Documents\n\nUse the included workflows.\n",
    );
    write(&root.path().join("config.json"), r#"{"preview":true}"#);
    write(
        &root.path().join(".cinyuverse-plugin/content.index.json"),
...
fn v4_product_package_requires_a_single_sentence_readme_summary() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.bad-summary","publisher":"dev.cinyuverse","name":"Bad summary","version":"1.0.0",
          "readme":"README.md","content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
...
fn snapshot_update_preserves_the_root_config_file() {
    ...
    for (root, version) in [(first.path(), "1.0.0"), (update.path(), "1.1.0")] {
        write(
            &root.join(".cinyuverse-plugin/plugin.json"),
            &format!(
                r#"{{
                  "manifestVersion":4,"apiVersion":"1.0",
                  "id":"dev.cinyuverse.config","publisher":"dev.cinyuverse","name":"Config","version":"{version}",
                  "readme":"README.md","content":{{"root":"contents","index":".cinyuverse-plugin/content.index.json"}},
                  "config":{{"schema":{{"type":"object","properties":{{"preview":{{"type":"boolean"}}}},"additionalProperties":false}}}},
                  "engines":{{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"}},
        ...
        write(
            &root.join(".cinyuverse-plugin/content.index.json"),
...
fn marketplace_install_copies_a_temp_package_into_durable_storage() {
    ...
    let storage = tempfile::tempdir().expect("installed packages");
    write(
        &source.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"drawio","publisher":"cinyuverse","name":"Drawio","version":"1.0.0",
          "readme":"README.md","content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
    ...
    write(
        ...
        "---\nsummary: Preview Drawio diagrams in Cinyuverse.\n---\n# Drawio\n",
    );
    write(&source.path().join("config.json"), "{}");
    write(
        &source.path().join(".cinyuverse-plugin/content.index.json"),
    ...
    for (root, version, properties, config) in schemas {
        write(
            ...
            &format!(
                r#"{{
                  "manifestVersion":4,"apiVersion":"1.0",
                  "id":"dev.cinyuverse.config-migrate","publisher":"dev.cinyuverse","name":"Config","version":"{version}",
                  "readme":"README.md","content":{{"root":"contents","index":".cinyuverse-plugin/content.index.json"}},
...
fn developer_link_executes_an_immutable_content_addressed_candidate() {
    ...
    write(
        &source.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "id":"dev.cinyuverse.frozen",
          "publisher":"dev.cinyuverse",
          "name":"Frozen",
          "version":"1.0.0"
        }"#,
    );
...
fn portable_package_auto_discovers_skills_and_preserves_unknown_fields() {
    ...
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "$schema": "cinyuverse-plugin/v3",
          "id": "dev.cinyuverse.example",
          "name": "Example",
          "version": "1.2.3",
    ...
    assert_eq!(package.id.as_str(), "dev.cinyuverse.example");
    assert_eq!(package.skills.len(), 1);
    assert_eq!(package.skills[0].path, "skills/example/SKILL.md");
    assert_eq!(package.formats, vec![PackageFormat::Cinyuverse]);
...
fn co_located_manifests_are_one_package_with_format_badges() {
    ...
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{"id":"dev.cinyuverse.shared","name":"Shared","version":"1.0.0"}"#,
    );
    write(
...
fn portable_package_requires_at_least_one_contribution() {
    ...
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{"id":"dev.cinyuverse.empty","name":"Empty","version":"1.0.0"}"#,
    );
...
fn v4_rejects_an_orphan_file_opener_without_a_preview_provider() {
    ...
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "$schema":"cinyuverse-plugin/v4",
          "manifestVersion":4,
          "apiVersion":"1.0",
          "id":"dev.cinyuverse.markdown-preview",
          "publisher":"dev.cinyuverse",
...
fn product_package_can_bind_a_file_opener_to_a_generic_artifact_editor_surface() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.diagram-editor","publisher":"dev.cinyuverse","name":"Diagram editor","version":"1.0.0",
          "readme":"README.md","content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
          "config":{"schema":{"type":"object","additionalProperties":false}},
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
          "entrypoints":{
    ...
    write(
        &root.path().join("README.md"),
        "---\nsummary: Edit diagrams in their Cinyuverse file tabs.\n---\n# Diagram editor\n",
    );
    write(&root.path().join("config.json"), "{}");
    write(
        &root.path().join(".cinyuverse-plugin/content.index.json"),
...
fn v4_rejects_shell_installers_before_install_or_trust_prompts() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.shell","publisher":"dev.cinyuverse","name":"Shell","version":"1.0.0",
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
...
fn v4_fails_closed_for_unknown_permissions_and_invalid_entrypoints() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.closed","publisher":"dev.cinyuverse","name":"Closed","version":"1.0.0",
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
...
fn v4_accepts_legacy_trust_tier_metadata_under_full_trust() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.native","publisher":"dev.cinyuverse","name":"Native","version":"1.0.0",
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
          "permissions":[{
...
fn v4_rejects_unknown_manifest_fields_before_install() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.unknown","publisher":"dev.cinyuverse","name":"Unknown","version":"1.0.0",
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
          "app.unknownSurface":{},
          "contributes":{"agent.skills":[{"id":"skill","kindVersion":1,"path":"skills/skill"}]}
        }"#,
...
fn v4_rejects_unknown_contribution_fields_before_install() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.unknown-contribution","publisher":"dev.cinyuverse",
...
fn snapshot_materialization_is_stable_and_developer_link_stays_external() {
    ...
    write(
        &source.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{"id":"dev.cinyuverse.snapshot","name":"Snapshot","version":"1.0.0"}"#,
    );
    write(
...
fn actions_and_commands_share_one_invocation_definition() {
    ...
    write(
        &root.path().join(".cinyuverse-plugin/plugin.json"),
        r#"{
          "id":"dev.cinyuverse.invoke","name":"Invoke","version":"1.0.0",
...
fn v4_rejects_plugin_kind_dependencies_with_a_stable_code() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.dep","publisher":"dev.cinyuverse","name":"Dep","version":"1.0.0",
          "readme":"README.md",
          "content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
          "dependencies":[{"kind":"plugin","id":"dev.cinyuverse.other"}],
          "integrations":[{"id":"documents","kind":"content.skill","resource":"contents/skills/documents"}]
        }"#,
    );
    write(
...
fn v4_rejects_conversation_timeline_card_slot_with_a_stable_code() {
    ...
    write(
        ...
        r#"{
          "manifestVersion":4,"apiVersion":"1.0",
          "id":"dev.cinyuverse.card","publisher":"dev.cinyuverse","name":"Card","version":"1.0.0",
          "readme":"README.md",
          "content":{"root":"contents","index":".cinyuverse-plugin/content.index.json"},
          "engines":{"cinyuverse":">=0.1.3 <1.0.0","pluginSdk":"^1.0.0"},
