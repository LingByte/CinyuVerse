...
fn import_office_action_manifest() {
    ...
    let manifest = include_str!("fixtures/office-presentation.cinyuverse-plugin.json");

    let imported = service
        .import_manifest(manifest, ManifestSource::Bundled)
        .expect("the bundled Office manifest should import");

    assert_eq!(imported.id.as_str(), "cinyuverse.office.presentation");
...
fn rejects_unknown_manifest_major() {
    let service = PluginService::new();
    let manifest = include_str!("fixtures/office-presentation.cinyuverse-plugin.json")
        .replace("cinyuverse-plugin/v2", "cinyuverse-plugin/v3");
...
fn rejects_legacy_install_commands_as_malicious_manifest_fields() {
    ...
    let manifest = include_str!("fixtures/office-presentation.cinyuverse-plugin.json").replace(
...
fn imports_optional_metadata_and_console_binding() {
    ...
    assert_eq!(imported.author.as_deref(), Some("Cinyuverse"));
    assert_eq!(
...
fn imports_action_without_artifact_provider_binding() {
    let mut manifest: serde_json::Value = serde_json::from_str(include_str!(
        "fixtures/office-presentation.cinyuverse-plugin.json"
    ))
    .expect("fixture JSON");
    manifest["actions"][0]
        .as_object_mut()
