...
use crate::{CONVERSATION_PROJECTION_VERSION, ConversationEventAppender, ConversationProjector};

const BUNDLE_VERSION: &str = "cinyuverse-conversation-bundle.v1";

#[derive(Debug, Error)]
pub enum ConversationBundleError {
    #[error("{0}")]
    NotFound(String),
    #[error("{0}")]
    BadRequest(String),
...
impl From<sqlx::Error> for ConversationBundleError {
    fn from(error: sqlx::Error) -> Self {
        Self::Internal(error.to_string())
    }
}

impl From<serde_json::Error> for ConversationBundleError {
    fn from(error: serde_json::Error) -> Self {
        Self::Internal(error.to_string())
    }
}

impl From<std::io::Error> for ConversationBundleError {
...
pub async fn export_conversation_bundle(
...
) -> Result<ConversationExportResult, ConversationBundleError> {
    let Some(summary) = DbConversationSummary::find_by_id(pool, conversation_id).await? else {
        return Err(ConversationBundleError::NotFound(format!(
    ...
    let manifest = ConversationBundleManifest {
        ...
        export_app_version: export_app_version.to_string(),
        exported_at: Utc::now(),
...
pub async fn import_conversation_bundle(
    ...
    workspace_id: Uuid,
) -> Result<ConversationImportResult, ConversationBundleError> {
    ...
    .await
    .map_err(|error| ConversationBundleError::Internal(error.to_string()))?;
    ...
    let _timeline = ConversationProjector::project(pool, conversation_id)
        .await
        .map_err(|error| ConversationBundleError::Internal(error.to_string()))?;
...
) -> Result<Uuid, ConversationBundleError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn conversation_bundle_export_contains_cinyuverse_tables() {
        let pool = migrated_pool().await;
        let (conversation_id, _) = seed_conversation(&pool).await;

        let exported = export_conversation_bundle(&pool, conversation_id, None, "test")
            .await
            .expect("export");
