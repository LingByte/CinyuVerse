...
mod tests {
    ...
    async fn ensure_workspace_creates_missing_branch_from_target_branch() {
        ...
        let workspace_dir = temp.path().join(".cinyuverse-workspaces").join("workspace");
...
pub enum WorkspaceError {
    ...
    NoRepositories,
    #[error("Unsafe workspace path outside Cinyuverse-owned storage: {0}")]
...
impl WorkspaceManager {
    const CUSTOM_WORKSPACE_DIR_NAME: &'static str = ".cinyuverse-workspaces";
    ...
    fn refuse_unsafe_cleanup(path: &Path, repos: &[Repo]) -> bool {
        if !Self::is_app_owned_workspace_dir(path) {
            warn!(
                "Refusing to clean workspace path outside Cinyuverse-owned workspace storage: {}",
                path.display()
            );
            return true;
        }
    ...
    fn validate_workspace_container_path(path: &Path) -> Result<(), WorkspaceError> {
        ...
        warn!(
            "Refusing to use workspace path outside Cinyuverse-owned workspace storage: {}",
            path.display()
        );
    ...
    async fn cleanup_orphans_in_directory(db: &Pool<Sqlite>, workspace_base_dir: &Path) {
        if !Self::is_app_owned_workspace_dir(workspace_base_dir) {
            warn!(
                "Skipping orphan cleanup for non Cinyuverse-owned workspace directory: {}",
    ...
    async fn cleanup_workspace_without_repos(workspace_dir: &Path) -> Result<(), WorkspaceError> {
        ...
        if !Self::is_app_owned_workspace_dir(workspace_dir) {
            warn!(
                "Refusing to remove orphan workspace outside Cinyuverse-owned storage: {}",
                workspace_dir.display()
            );
            return Ok(());
        }
