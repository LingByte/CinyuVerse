...
impl EditorConfig {
    ...
    fn ensure_extension_recommended(path: &Path) {
        ...
        const EXTENSION_ID: &str = "cinyuverse.cinyuverse";
            ...
            match std::fs::read_to_string(&extensions_file) {
                Ok(content) => match serde_json::from_str(&content) {
                    ...
                    Err(e) => {
                        ...
                        tracing::warn!(
                            ...
                            error = %e,
                            "extensions.json is not valid JSON; skipping Cinyuverse recommendation"
                        );
                        return;
                    }
                },
                Err(e) => {
                    tracing::warn!(
                        file = ?extensions_file,
                        error = %e,
                        "failed to read extensions.json; skipping Cinyuverse recommendation"
                    );
                    return;
                }
            }
