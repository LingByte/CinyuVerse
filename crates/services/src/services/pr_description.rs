...
mod tests {
    ...
    fn extracts_title_and_body_from_plain_json() {
        let raw = r#"{"Title":"Fix login timeout (Cinyuverse)","Body":"Users can sign in again."}"#;
        let draft = extract_pr_description(raw).expect("draft");
        assert_eq!(draft.title, "Fix login timeout (Cinyuverse)");
        assert_eq!(draft.body, "Users can sign in again.");
    }

    #[test]
    fn extracts_title_and_body_from_nested_payload() {
        let raw = r#"{"type":"assistant","content":"{\"Title\":\"Add retries (Cinyuverse)\",\"Body\":\"Retry failed requests.\"}"}"#;
        let draft = extract_pr_description(raw).expect("draft");
        assert_eq!(draft.title, "Add retries (Cinyuverse)");
