...
async fn execute_lifecycle_command(
    ...
    process
        ...
        .env("CINYUVERSE_PROJECT_ID", project_id.to_string())
        .env("CINYUVERSE_WORKSPACE_ID", workspace_id.to_string())
        .env("CINYUVERSE_WORKTREE_PATH", working_dir)
        .kill_on_drop(true);
    let output = process
        .output()
        .await
...
mod tests {
    ...
    async fn lifecycle_command_receives_worktree_context() {
        ...
        execute_lifecycle_command(
            r#"printf '%s|%s|%s' "$CINYUVERSE_PROJECT_ID" "$CINYUVERSE_WORKSPACE_ID" "$CINYUVERSE_WORKTREE_PATH" > lifecycle.txt"#,
            project_id,
            workspace_id,
            temp.path(),
        )
