...
pub fn build_rich_localized(
    ...
    match event {
        ...
        _ => RichMessage::info("Cinyuverse").with_title("Cinyuverse"),
    }
}
...
fn feishu_card(msg: &RichMessage) -> String {
    ...
    json!({
        ...
        "header": {
            ...
            "title": {
                "tag": "plain_text",
                "content": msg.title.clone().unwrap_or_else(|| "Cinyuverse".to_string()),
            },
        },
        "elements": elements,
    })
...
pub async fn ilink_send_text(
    ...
    let body = json!({
        "msg": {
            ...
            "client_id": format!("cinyuverse-{}", uuid::Uuid::new_v4()),
            "message_type": 2,
            "message_state": 2,
...
/// One-time migration of legacy plaintext-JSON channel tokens into `~/.cinyuverse/.env`
