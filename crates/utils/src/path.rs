...
/// tools). Clearing Cinyuverse application data must not remove those CLIs or the
...
pub fn managed_artifacts_directory(home_dir: &Path, _app_data_dir: &Path) -> PathBuf {
    home_dir.join(".local").join("share").join("cinyuverse")
}
...
pub fn get_cinyuverse_temp_dir() -> std::path::PathBuf {
...
fn directory_is_writable(path: &Path) -> bool {
    ...
    let probe = path.join(format!(".cinyuverse-write-{}", std::process::id()));
...
mod tests {
    ...
    fn test_make_path_relative_macos_private_alias() {
        ...
        let worktree = "/var/folders/zz/abc123/T/cinyuverse-dev/worktrees/vk-test";
    ...
    #[test]
    fn cinyuverse_temp_dir_is_writable_on_the_current_platform() {
        let path = get_cinyuverse_temp_dir();
