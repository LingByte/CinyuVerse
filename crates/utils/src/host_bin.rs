//! Locate Host-family sidecar binaries (`cinyuverse-mcp`, `cinyuverse-workflow-mcp`).
...
///
/// Order: `CINYUVERSE_MCP_BIN` / `CINYUVERSE_WORKFLOW_MCP_BIN` → current-exe directory
...
fn env_override(base: &str) -> Option<PathBuf> {
    let key = match base {
        "cinyuverse-mcp" => "CINYUVERSE_MCP_BIN",
        "cinyuverse-workflow-mcp" => "CINYUVERSE_WORKFLOW_MCP_BIN",
        _ => return None,
    };
...
mod tests {
    ...
    fn empty_stub_is_not_runnable() {
        ...
        let stub = dir.path().join("cinyuverse-mcp");
    ...
    fn search_skips_empty_sidecar_and_finds_profile_binary() {
        ...
        let macos = root
            .path()
            .join("cef-runtime/macos/app/cinyuverse.app/Contents/MacOS");
        fs::create_dir_all(&macos).unwrap();
        fs::write(macos.join("cinyuverse"), b"app").unwrap();
        fs::write(macos.join("cinyuverse-mcp"), []).unwrap();
    ...
    fn search_accepts_target_triple_sidecar_name() {
        let dir = tempfile::tempdir().unwrap();
        let sidecar = dir.path().join("cinyuverse-mcp-aarch64-apple-darwin");
        fs::write(&sidecar, b"sidecar").unwrap();
        let found = first_runnable_in(dir.path(), "cinyuverse-mcp", "cinyuverse-mcp").unwrap();
