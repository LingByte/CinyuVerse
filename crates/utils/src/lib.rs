...
mod tests {
    ...
    #[test]
    fn cache_dir_uses_the_cinyuverse_project_identity() {
        let path = cache_dir();
        let rendered = path.to_string_lossy();
        assert!(
            rendered.to_ascii_lowercase().contains("cinyuverse"),
            "cache directory should belong to Cinyuverse, got {rendered}"
        );
        assert!(
