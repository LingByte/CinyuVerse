...
pub const TUNNEL_INSTALL_URL: &str = "https://cinyuverse.xforever.xin/tunnel.sh";
...
mod tests {
    ...
    fn extract_relay_token_from_install_command() {
        assert_eq!(
            extract_relay_token(
                "curl -fsSL https://cinyuverse.xforever.xin/tunnel.sh | sh -s -- -t vbx_tun_e22cd3763db1436f847e709d79b3f9f9 -p 13630"
            )
            .as_deref(),
    ...
    fn install_command_uses_sudo_for_privileged_ports() {
        assert_eq!(
            ...
            "curl -fsSL https://cinyuverse.xforever.xin/tunnel.sh | sudo sh -s -- -t tok_abc -p 443"
        );
        assert_eq!(
            install_command("tok_abc", 8443),
            "curl -fsSL https://cinyuverse.xforever.xin/tunnel.sh | sh -s -- -t tok_abc -p 8443"
        );
    }
}
