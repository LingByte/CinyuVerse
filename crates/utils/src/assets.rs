...
/// Default Host data directory shared by Desktop release builds and `cinyuverse-server`.
///
/// Desktop debug builds still use the repo `dev_assets/` tree via [`asset_dir`].
pub fn default_host_data_dir() -> PathBuf {
    ProjectDirs::from("app", "cinyuverse", "cinyuverse")
        .map(|dirs| dirs.data_dir().to_path_buf())
        .or_else(|| dirs::data_dir().map(|path| path.join("cinyuverse")))
        .unwrap_or_else(|| PathBuf::from(".cinyuverse-data"))
}
...
/// Canonical Host data directory for Desktop and `cinyuverse-server`.
///
/// `CINYUVERSE_DATA_DIR` wins. Otherwise this is [`asset_dir`] so debug Desktop
/// keeps using `dev_assets/` and release builds share ProjectDirs with Server.
pub fn host_data_dir() -> PathBuf {
    if let Some(explicit) = std::env::var_os("CINYUVERSE_DATA_DIR") {
...
/// Settings live next to the Host database. `~/.cinyuverse/settings.json` is copied
/// once if the Host file does not exist yet.
pub fn settings_path() -> PathBuf {
    let dest = host_data_dir().join("settings.json");
    adopt_legacy_file(
        dirs::home_dir().map(|home| home.join(".cinyuverse").join("settings.json")),
        &dest,
    );
    dest
}
...
pub fn im_env_path() -> PathBuf {
    ...
    adopt_legacy_file(
        dirs::home_dir().map(|home| home.join(".cinyuverse").join(".env")),
        &dest,
    );
    dest
}
...
/// `com.cinyuverse.app`. Host product files now live in [`host_data_dir`].
pub fn tauri_app_data_dir_candidates() -> Vec<PathBuf> {
    let Some(data) = dirs::data_dir() else {
        return Vec::new();
    };
    let mut dirs = vec![
        data.join("com.cinyuverse.app"),
        data.join("Cinyuverse"),
        data.join("cinyuverse"),
    ];
    dirs.dedup();
    dirs
...
///
/// Desktop and `cinyuverse-server` must persist this next to the database so pairing
...
/// Materializes every bundled Cinyuverse plugin from application-owned bytes.
...
fn embedded_builtin_directories() -> Vec<String> {
    ...
    for path in BuiltinPluginAssets::iter() {
        if let Some(directory) = path
            .strip_suffix("/.cinyuverse-plugin/plugin.json")
...
fn existing_materialized_plugin_roots(data_root: &Path) -> BTreeMap<String, PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        for version in versions.flatten() {
            ...
            if !path.join(".cinyuverse-plugin/plugin.json").is_file() {
                continue;
            }
...
fn materialize_builtin_plugin(
    ...
    let manifest_path = format!("{directory}/.cinyuverse-plugin/plugin.json");
...
mod tests {
    ...
    fn discovers_builtin_packages_without_plugin_specific_host_code() {
        ...
        let mut ids = roots
            ...
            .map(|root| {
                let manifest: serde_json::Value = serde_json::from_slice(
                    &std::fs::read(root.join(".cinyuverse-plugin/plugin.json")).unwrap(),
                )
                .unwrap();
        ...
        assert_eq!(
            ...
            [
                ...
                "cinyuverse.session-enhance",
                "cinyuverse.workflow-creator",
            ]
        );
        let office = roots
            .iter()
            .find(|root| root.to_string_lossy().contains("cinyuverse.office"))
            .unwrap();
    ...
    fn keeps_already_materialized_packages_when_scanning_the_host_data_directory() {
        ...
        std::fs::create_dir_all(extra.join(".cinyuverse-plugin")).unwrap();
        std::fs::write(
            extra.join(".cinyuverse-plugin/plugin.json"),
    ...
    fn tauri_app_data_candidates_include_the_bundle_identifier() {
        ...
        assert!(
            files.iter().any(|path| {
                path.components()
                    .any(|component| component.as_os_str() == "com.cinyuverse.app")
                    && path
                        .file_name()
                        .is_some_and(|name| name == "agent-model-providers.json")
            }),
            "expected com.cinyuverse.app candidate, got {files:?}"
        );
    }
}
