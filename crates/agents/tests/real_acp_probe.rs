...
///
/// Example:
/// CINYUVERSE_REAL_ACP_COMMAND=/absolute/path/to/opencode
/// CINYUVERSE_REAL_ACP_ARGS='["acp"]'
/// CINYUVERSE_REAL_ACP_TIMEOUT_SECS=120 # optional for cold package installs
...
async fn installed_agent_completes_a_real_acp_initialize_handshake() {
    ...
    let args: Vec<String> = serde_json::from_str(
        &std::env::var("CINYUVERSE_REAL_ACP_ARGS").unwrap_or_else(|_| "[]".to_string()),
    )
    .expect("CINYUVERSE_REAL_ACP_ARGS must be a JSON string array");
    let timeout_seconds = std::env::var("CINYUVERSE_REAL_ACP_TIMEOUT_SECS")
        .map(|value| {
            value
                .parse::<u64>()
                .expect("CINYUVERSE_REAL_ACP_TIMEOUT_SECS must be a positive integer")
        })
        .unwrap_or(20);
    assert!(
        timeout_seconds > 0,
        "CINYUVERSE_REAL_ACP_TIMEOUT_SECS must be positive"
    );
