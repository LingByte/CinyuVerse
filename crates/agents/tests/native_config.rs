...
async fn native_config_preserves_unknown_fields_and_reports_auth_status() {
    ...
    let saved = provider
        .save(
            ...
            NativeConfigPatch {
                ...
                values: BTreeMap::from([(
                    "openai_api_key".to_string(),
                    Some("not-validated-by-cinyuverse".to_string()),
                )]),
            },
            false,
        )
    ...
    assert_eq!(persisted["OPENAI_API_KEY"], "not-validated-by-cinyuverse");
...
"#
    ...
    assert_eq!(
        config["model_providers"]["cinyuverse"]["supports_websockets"].as_bool(),
        Some(true)
    );
    assert_eq!(
    ...
    .unwrap();
    assert!(
        config["model_providers"]["cinyuverse"]
...
async fn advanced_codeg_parity_fields_write_their_native_shapes() {
    ...
    assert_eq!(
        grok_config["model"]["cinyuverse"]["api_backend"].as_str(),
        Some("responses")
    );
    assert_eq!(
...
async fn kimi_managed_model_is_normalized_to_the_native_schema() {
    ...
    assert_eq!(
        config["models"]["cinyuverse"]["model"].as_str(),
        Some("demo-model")
    );
    assert_eq!(
    ...
    assert_eq!(
        config["models"]["cinyuverse"]["capabilities"]
            .as_array()
            .unwrap(),
        &[
    ...
    assert_eq!(
        config["models"]["cinyuverse"]["support_efforts"]
            .as_array()
            .unwrap(),
        &[
    ...
    let config: toml::Value = toml::from_str(&config).unwrap();
    assert_eq!(
        config["models"]["cinyuverse"]["default_effort"].as_str(),
        Some("high")
    );
}
