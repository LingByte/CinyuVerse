...
mod tests {
    ...
    fn peels_the_use_tool_envelope_to_the_inner_mcp_name() {
        let peeled = unwrap_grok_use_tool(Some(&json!({
            "tool_name": "cinyuverse-delegation-mcp__delegate_to_agent",
            "tool_input": { "agent_type": "codex", "task": "introduce yourself" }
        })));
        assert_eq!(
            peeled,
            Some((
                "cinyuverse-delegation-mcp__delegate_to_agent".into(),
    ...
    fn strips_host_prefixes_from_mcp_tool_names() {
        assert_eq!(
            mcp_bare_tool_name("cinyuverse-delegation-mcp__delegate_to_agent"),
            "delegate_to_agent"
        );
        assert_eq!(
            mcp_bare_tool_name("mcp__cinyuverse-delegation-mcp__get_delegation_status"),
            "get_delegation_status"
        );
        assert_eq!(
            mcp_bare_tool_name("cinyuverse-delegation-mcp/cancel_delegation"),
            "cancel_delegation"
        );
    }
}
