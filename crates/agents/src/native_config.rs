...
fn finalize_grok_shape(
    ...
    for (field, path) in [
        ("grok_base_url", &["model", "cinyuverse", "base_url"][..]),
        ("grok_api_key", &["model", "cinyuverse", "api_key"][..]),
    ] {
    ...
    if value_at_path(document, &["model", "cinyuverse", "context_window"])
        .and_then(Value::as_i64)
        .is_some_and(|value| value <= 0)
    {
        remove_value_at_path(document, &["model", "cinyuverse", "context_window"]);
    }
    let custom_model = value_at_path(document, &["model", "cinyuverse", "model"])
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(str::to_string);
    if let Some(custom_model) = custom_model {
        set_value_at_path(
            document,
            &["model", "cinyuverse", "model"],
            Value::String(custom_model),
        )?;
        set_value_at_path(
            document,
            &["models", "default"],
            Value::String("cinyuverse".to_string()),
        )?;
...
fn finalize_kimi_shape(
    ...
    if value_at_path(document, &["providers", "cinyuverse", "env"])
    ...
    let model = value_at_path(document, &["models", "cinyuverse", "model"])
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(str::to_string);
    ...
    set_value_at_path(
        document,
        &["models", "cinyuverse", "model"],
        Value::String(model),
    )?;
    set_value_at_path(
        document,
        &["models", "cinyuverse", "provider"],
        Value::String("cinyuverse".to_string()),
    )?;
    set_value_at_path(
        document,
    ...
    let context = value_at_path(document, &["models", "cinyuverse", "max_context_size"])
        .and_then(Value::as_i64)
        .filter(|value| *value > 0)
        .unwrap_or(262_144);
    set_value_at_path(
        document,
        &["models", "cinyuverse", "max_context_size"],
        ...
        .is_some_and(|value| efforts.iter().any(|effort| effort == value))
    {
        set_value_at_path(
            document,
            &["models", "cinyuverse", "default_effort"],
            Value::String(default_effort.expect("checked above")),
        )?;
    } else {
        remove_value_at_path(document, &["models", "cinyuverse", "default_effort"]);
    }
    Ok(())
}
