//! Stable terminal command exposure for Cinyuverse-managed Agent runtimes.
...
const PROFILE_BLOCK_START: &str = "# >>> Cinyuverse managed Agent CLI >>>";
const PROFILE_BLOCK_END: &str = "# <<< Cinyuverse managed Agent CLI <<<";
const SHIM_MARKER_PREFIX: &str = "# Cinyuverse Agent CLI: ";
...
/// are visible after install. Does not write a Cinyuverse-owned shim.
...
fn shim_is_owned_by(contents: &[u8], agent_id: &AgentId) -> bool {
    ...
    let windows_marker = format!("rem Cinyuverse Agent CLI: {}", agent_id.as_str());
...
mod tests {
    ...
    fn windows_shim_binds_locked_runtime_without_changing_the_user_path() {
        ...
        let contents = render_shim(
            ShellFamily::Windows,
            &agent_id,
            Path::new(r"C:\Users\Dev%20\Cinyuverse\agents\claude.cmd"),
            &[PathBuf::from(r"C:\Users\Dev%20\Cinyuverse\node")],
        );

        assert!(contents.contains("set \"PATH=C:\\Users\\Dev%%20\\Cinyuverse\\node;%PATH%\"\r\n"));
        assert!(contents.contains("\"C:\\Users\\Dev%%20\\Cinyuverse\\agents\\claude.cmd\" %*\r\n"));
