...
/// make a plain `npm install -g` fail with `EEXIST` even when Cinyuverse is
...
/// for Cinyuverse-side `cmd.exe` probes, then a bare name.
...
fn write_probe_file(dir: &Path) -> bool {
    let probe = dir.join(format!(".cinyuverse-write-{}", std::process::id()));
