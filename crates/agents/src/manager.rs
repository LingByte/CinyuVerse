...
const HANDSHAKE_TIMEOUT_ENV: &str = "CINYUVERSE_ACP_SPAWN_HANDSHAKE_TIMEOUT_SECS";
const FULL_GATE_FIXTURE_PROMPT: &str = "__cinyuverse_agent_full_gate_fixture__";
...
// `CINYUVERSE_PROMPT_IDLE_TIMEOUT_SECS` fails a turn that produces no ACP traffic
...
const PROMPT_IDLE_TIMEOUT_ENV: &str = "CINYUVERSE_PROMPT_IDLE_TIMEOUT_SECS";
...
impl AgentConnectionRunner {
    ...
    async fn run_acp(
        ...
        // This is a Cinyuverse-side policy toggle, not an environment variable Pi
        ...
        // (or `OPENAI_BASE_URL=""`) leaked from the shell that launched Cinyuverse
            ...
            .connect_with(transport, |conn: ConnectionTo<Agent>| async move {
                ...
                let initialize = AcpAuthStatusAdapter::initialize(
                    ...
                    InitializeRequest::new(ProtocolVersion::LATEST)
                        ...
                        .client_info(Implementation::new("cinyuverse", env!("CARGO_PKG_VERSION"))),
                );
...
fn seed_pi_workspace_trust(
    ...
    let temporary_path = agent_dir.join(format!(".trust.json.cinyuverse-{}", uuid::Uuid::new_v4()));
...
/// classifier to decide. They must never silently upgrade Cinyuverse's independent
...
mod tests {
    ...
    fn test_launch_lock(agent_id: AgentId) -> SessionLaunchLock {
        SessionLaunchLock {
            agent_id,
            absolute_acp_program: PathBuf::from("/tmp/cinyuverse-test-acp"),
            args: Vec::new(),
    ...
    async fn live_manager_refuses_a_missing_working_directory() {
        ...
        let (_snapshot, ready_rx) = manager
            .register_connection(AgentConnectionLaunch {
                ...
                launch_lock,
                workspace_id: uuid::Uuid::new_v4(),
                working_dir: PathBuf::from("Cinyuverse"),
                additional_directories: Vec::new(),
                auto_approve_mode: AgentAutoApproveMode::Off,
                env: HashMap::new(),
            })
        ...
        assert!(message.contains("Cinyuverse"), "{message}");
    ...
    async fn session_auto_mode_does_not_upgrade_permission_interception() {
        ...
        // classifier. It must not become Cinyuverse's blanket auto-approval mode.
        assert_eq!(
