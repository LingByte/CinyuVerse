...
/// Every agent Cinyuverse manages. Order is used for stable scan/display output.
...
// ~/.cinyuverse/skills, deduped by name and grouped by prefix) and a "Skill 市场"
...
// ~/.cinyuverse/skills and mirrors it into every locally supported Agent adapter.
// ===========================================================================
...
fn cinyuverse_skills_dir() -> PathBuf {
    dirs::home_dir()
...
/// The agent-specific directory Cinyuverse writes skills into when hosting.
...
async fn scan_all_skills(custom_targets: &[CustomAgentSkillStorage]) -> Vec<LocalSkill> {
    ...
    let cinyuverse = cinyuverse_skills_dir();
    ...
    let cinyuverse_dir = SkillDir {
        scope: AgentSkillScope::Global,
        path: cinyuverse,
        read_only: false,
    };
    for item in list_skills_in_dir(&cinyuverse_dir, false).await {
        let entry = map.entry(item.id.clone()).or_default();
        entry.global = true;
        if entry.description.is_none() {
            entry.description = item.description.clone();
...
fn physical_path_key(path: &Path) -> PathBuf {
    ...
    // symlink to the Cinyuverse central store; following it would turn the
...
/// materializes a real copy in the global store (~/.cinyuverse/skills) and links
...
fn apply_hosting_with_layout(
    ...
    let cinyuverse = layout.store.join(skill_id);
    let (agent_src, agent_link) = if global {
        place_skill(src, &cinyuverse, false)?;
        (cinyuverse.clone(), link)
    } else {
...
/// not owned by that Plugin. Sources are first copied into a stable Cinyuverse
...
fn project_plugin_skills_with_layout(
    ...
    for (skill_id, skill_file) in skills {
        ...
        std::fs::write(stable.join(".cinyuverse-plugin-owner"), plugin_id)
...
fn owned_plugin_projection(path: &Path, plugin_id: &str) -> bool {
    std::fs::read_to_string(path.join(".cinyuverse-plugin-owner"))
...
fn staging_dir() -> PathBuf {
    std::env::temp_dir().join(format!("cinyuverse-skill-{}", uuid::Uuid::new_v4()))
}
...
async fn fetch_popular_skills(limit: usize) -> Result<Vec<SkillMarketItem>, SkillError> {
    let client = reqwest::Client::builder()
        .user_agent("Mozilla/5.0 cinyuverse-skills-market/1.0")
...
pub async fn search_skill_market(
    ...
    let client = reqwest::Client::builder()
        .user_agent("cinyuverse-skills-market/1.0")
        .timeout(std::time::Duration::from_secs(20))
        .build()
        .map_err(|e| SkillError::Other(e.to_string()))?;
    let response = client
...
mod tests {
    ...
    fn removing_a_plugin_deletes_only_its_owned_skill_projections() {
        ...
        assert!(
            !temp
                .path()
                .join("store/.plugins/dev.cinyuverse.research")
                .exists()
        );
        assert_eq!(
            std::fs::read_to_string(user_skill.join("SKILL.md")).unwrap(),
    ...
    fn custom_agent_skills_require_an_explicit_storage_declaration() {
        ...
        let directory = std::env::temp_dir().join("cinyuverse-custom-agent-skills");
        let storage = CustomAgentSkillStorage {
            agent_id: "local-reviewer".to_string(),
            shared_store: true,
            directory: Some(directory.clone()),
        };
        let workspace = std::env::temp_dir().join("cinyuverse-custom-agent-workspace");
    ...
    async fn saved_hermes_home_expands_tilde_for_skills() {
        let environment = HashMap::from([(
            ...
            "~/.hermes-cinyuverse-test".to_string(),
        )]);
        let paths = with_saved_agent_environment(environment, async {
