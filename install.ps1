# Install the Cinyuverse Host family (cinyuverse-server, cinyuverse-mcp, cinyuverse-workflow-mcp
...
#
#   irm https://raw.githubusercontent.com/LingByte/CinyuVerse/master/install.ps1 | iex
#
# Environment:
#   CINYUVERSE_VERSION      Install this version instead of the latest release.
#   CINYUVERSE_PLATFORM     Override platform detection (e.g. windows-x86_64).
#   CINYUVERSE_GITHUB_REPO        Source repository. Default LingByte/CinyuVerse.
#   CINYUVERSE_HOST_FAMILY_BASE   Override the download origin (no trailing slash).
#   CINYUVERSE_INSTALL_DIR        Where the `cinyuverse.cmd` launcher goes. Default %LOCALAPPDATA%\Cinyuverse\bin.
#   CINYUVERSE_PRINT_PLAN         Print the resolved platform and URLs, then exit.

$ErrorActionPreference = "Stop"

$Repo = if ($env:CINYUVERSE_GITHUB_REPO) { $env:CINYUVERSE_GITHUB_REPO } else { "LingByte/CinyuVerse" }
$CacheRoot = Join-Path $HOME ".cinyuverse" "host-family"
$InstallDir = if ($env:CINYUVERSE_INSTALL_DIR) { $env:CINYUVERSE_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA "Cinyuverse" "bin" }
...
function Get-Platform {
    if ($env:CINYUVERSE_PLATFORM) {
        return $env:CINYUVERSE_PLATFORM
    }
...
function Write-Launcher([string]$FamilyRoot) {
    ...
    $server = Join-Path $FamilyRoot "cinyuverse-server.exe"
    if (-not (Test-Path $server)) {
        $server = Join-Path $FamilyRoot "cinyuverse-server"
    }
    $cmd = Join-Path $InstallDir "cinyuverse.cmd"
...
if ($env:CINYUVERSE_VERSION) {
    $tag = if ($env:CINYUVERSE_VERSION.StartsWith("v")) { $env:CINYUVERSE_VERSION } else { "v$($env:CINYUVERSE_VERSION)" }
} else {
    $tag = Get-LatestTag
}
$version = $tag.TrimStart("v")
$archive = "Cinyuverse-$version-$platform-server.tar.gz"
$baseUrl = if ($env:CINYUVERSE_HOST_FAMILY_BASE) {
    $env:CINYUVERSE_HOST_FAMILY_BASE.TrimEnd("/")
} else {
    "https://github.com/$Repo/releases/download/$tag"
}
$familyRoot = Join-Path $CacheRoot $tag $platform "family"

if ($env:CINYUVERSE_PRINT_PLAN) {
...
try {
    if (Test-Path (Join-Path $familyRoot "SHA256SUMS")) {
        Write-Host "Cinyuverse Host family $tag is already installed for $platform."
        Assert-Sha256Sums $familyRoot
    } else {
        $temp = Join-Path ([IO.Path]::GetTempPath()) ("cinyuverse-install-" + [guid]::NewGuid().ToString("N"))
        New-Item -ItemType Directory -Path $temp | Out-Null
        Write-Host "Downloading Cinyuverse Host family $tag for $platform..."
    ...
    Write-Host ""
    Write-Host "Installed Cinyuverse Host family $tag to $familyRoot"
    Write-Host "Launcher: $launcher"

    $onPath = ($env:PATH -split ";" | ForEach-Object { $_.TrimEnd("\") }) -contains $InstallDir.TrimEnd("\")
    if ($onPath) {
        Write-Host ""
        Write-Host "Run ``cinyuverse`` to start the server on http://127.0.0.1:17891"
    } else {
        Write-Host ""
        Write-Host "Add $InstallDir to your PATH, then run ``cinyuverse``."
    }
} finally {
