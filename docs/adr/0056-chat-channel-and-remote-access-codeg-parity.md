...
Cinyuverse 的聊天通道和远程访问必须达到与 CodeG 同等的端到端可用性，并在
...
- 默认端口 `17891`，与 `cinyuverse-server` / Host 家族一致。
