...
2. 新建 Conversation 后输入 `&` 没有候选项，手打 `&Codex` 也不会变成 Mention Token。根因不是插件包缺配置，而是 Host Composer 把 `&` 绑在「当前 binding 已经投递过 `cinyuverse-delegation-mcp`」上。新 Conversation 在第一次 `session/new` 之前没有 binding，因此永远看不到 `&`。ADR-0057 要挡住的是插件启用前就已存在的旧会话，不是启用后新建的会话。
...
`&` 继续只表示「请父 LLM 考虑委派」，序列化为 `[&Name](cinyuverse://agent/<id>)`。不把 Agent 放进 `@` 面板。ADR-0031 否决用 `@` 做 Agent Mention 的决定仍然有效。
...
| 会话 | `[title](cinyuverse://conversation/<uuid>)` | 会话增强 `get_session_info` 已认识的 URI |
| 提交 | `[shortSha](cinyuverse://commit/<repoId>@<sha>)` | 提交身份；正文带短 SHA 与说明 |
| 指令 | `[#:name]([[tag:...]])` | 现有标签附录展开 |

会话 URI 使用已有的 `cinyuverse://conversation/<uuid>`，不另造 `cinyuverse://session/`。已保存的 `[#:…]` 与 `#` 草稿继续解析，不要求用户改历史。
