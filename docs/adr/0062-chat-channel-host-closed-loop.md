...
1. **出站不是 Host 职责。** 会话事件在 `ConversationEventPublisher` 提交后，
   只有桌面 `events.rs` 再调 `notify_conversation_event`。`cinyuverse-server` 用
...
IM，桌面 publisher 仍先发 row-op，再投递；`cinyuverse-server` 只投递。桌面事件循环
和命令收尾不再第二次投递。
...
- 桌面与 `cinyuverse-server` 对同一会话事件发出同一类 IM 通知。
