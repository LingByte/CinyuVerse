...
模型、模式、推理强度等 ACP 配置选项属于具体 Agent session，Cinyuverse 可以记忆用户选择
的 option id 与 value，但不能把它们写成 Agent 原生配置或视为永久能力。创建新会话
时，Cinyuverse 只有在 Agent 仍广告对应选项和值时才应用偏好；选项失效时使用 Agent
...
Session rebind 时，Cinyuverse 尝试沿用该 Conversation 原有的模型、模式与推理选项，
