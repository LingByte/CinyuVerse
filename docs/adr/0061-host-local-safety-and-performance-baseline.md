...
Cinyuverse 是本地优先的 Host。鉴权、配置文件机密和远程数据泄露不构成本机运行的默认
威胁模型。本 ADR 记录 2026-08-20 对 Host 家族（Desktop 与 `cinyuverse-server`）的静态
...
用户启用的 ACP Agent 与 Cinyuverse Plugin Package 与 Host 同权。仓库 setup / cleanup /
...
1. 用户启用的 ACP Agent、已安装或已启用的 Cinyuverse Plugin Package、用户编写的仓库
...
MCP 注入使用 Host 定位的 `cinyuverse-mcp` 二进制，这些保持。
...
同一条 Host 代码路径的 Desktop 与 `cinyuverse-server` 必须一起改（ADR-0033）。不得只在
桌面或只在 server 打补丁。
...
- 禁用仓库 hooks 后，用户若依赖 pre-commit 在 Cinyuverse 内运行，需要单独的产品决定；
...
- Headless `cinyuverse-server` 与桌面共用 Application Core；Git 与解压问题两边都有，
