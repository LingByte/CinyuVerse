...
# 通过 SSH 供给与接管远程 Cinyuverse Server

本决定为「把整个工作区搬到远端」补全最后一环：用户只有一台可 SSH 访问的服务器
（无公网 origin、无局域网可达、未安装 Cinyuverse）时，桌面端能够经 SSH **供给**
（安装、启动、升级）一个版本配套的 `cinyuverse-server`，经 SSH **隧道**建立可达，
...
扩展只是「供给器 + 隧道 + 指向切换」。Cinyuverse 的对应边界已由 ADR-0033 建立并
...
1. **共享应用核心与 Headless Server 已在。** `crates/server` 提供
   ...
   Application Core（ADR-0033）；`cinyuverse-server` 独立二进制存在，发行物形态已由
...
- **供给**：目标机器上没有 `cinyuverse-server` 时，没有任何自动化安装/启动路径；
...
SSH 只承担三件事：**供给通道**（投递与管理 `cinyuverse-server`）、**承载隧道**
...
127.0.0.1:<forwarded>  ── ssh -L 隧道 ──▶  远端 127.0.0.1:<port>
                                              │
                                        cinyuverse-server（loopback-only）
                                              │
                        Agent / worktree / 终端 / 插件内核（全在远端）
```

远端 `cinyuverse-server` 以 **SSH-only 模式**运行：只 bind loopback、不发布任何
...
**用户系统 SSH 环境**（`~/.ssh/config`、ssh-agent、密钥文件），Cinyuverse 不存储、
不生成、不管理 SSH 私钥（与 ADR-0060「使用用户环境」同一原则）。

连接一个 SSH Host Profile 时按以下幂等管线执行，每步可见、可重试、失败不留
半状态：

1. **探测**：`ssh` 执行探测脚本，识别 OS/arch、既有 `cinyuverse-server` 及其版本、
   数据目录占用（ADR-0054：被占用则只能作客户端，禁止起第二个 Host）。
2. **投递**：远端缺失或版本不配套时，下载（远端直连或经本地中转上传）与
   客户端**同 semver 家族**的 Cinyuverse Server 发行物，sha256 校验后解压到版本化
   目录（`~/.cinyuverse/server/<version>/`），原子切换 `current` 链接。
3. **启动**：以 SSH-only 模式启动 `cinyuverse-server`（loopback bind、指定数据
...
- **SSH 权限即管理员证明。** 能以 SSH 登录目标机器并读写数据目录的人，等价于
  ...
  `cinyuverse-server` 的邀请出示命令，取得一次性 pairing secret 并立即兑换为本
...
2. **SSH 凭据不归 Cinyuverse**：只调用系统 `ssh`（或库实现时读取系统配置与
...
**验收：** 一台仅有 SSH 访问、未装 Cinyuverse 的 Linux 服务器，从新建 Profile 到
...
- **建立在 ADR-0033 之上**：共享 Application Core、BackendTransport、Server
  ...
  供给类型与配套管线。
- **建立在 ADR-0054 之上**：Cinyuverse Server 发行物、Host 家族版本、Workstation
...
- **呼应 ADR-0060**：SSH 认证使用用户系统环境，Cinyuverse 不代管密钥。
