...
|---|---|---|---|
| 会话增强 | `cinyuverse.session-enhance` | 会话增强 | `cinyuverse-session-mcp` |
| 多智能体协同 | `cinyuverse.multi-agent` | 多智能体协同 | `cinyuverse-delegation-mcp` |

二者随 Cinyuverse 分发，`sourceKind=builtin`，可以停用，不能卸载。首次导入与 Office /
...
两个 MCP 互相独立，都作为客户端连接 `cinyuverse-server`，不走桌面进程内 UDS companion
...
Mention 仍只表示「请父 LLM 考虑委派」，序列化为 `[&Name](cinyuverse://agent/<id>)`。
UI 不得因出现 Mention 就显示子任务已启动。

### 8. 平台 `cinyuverse-mcp` 同一版本切干净

本决定落地后删除平台 companion 注入。未打开插件 = 没有对应工具，也没有 `&`。
旧会话保持到结束；新会话只走插件 MCP。不保留「没开插件就回退旧 cinyuverse-mcp」的
...
- ADR-0051 中「cinyuverse-mcp 暂时保留」和「这些插件必须用每会话短期票」被取代；
...
- Host 必须能把 **N 个** 已启用插件 STDIO 投递进 `session/new`，并同时做原生文件
  投影；今天只能注入一个 `cinyuverse-mcp`。
- Composer 读的是协同插件启用 + 本次会话是否已投递，不是 Agent 种类。
- 发行说明必须写明：升级后需在插件目录打开「会话增强」「多智能体协同」。

## Considered options

- **继续做平台内置 `cinyuverse-mcp`，只补设置页。** 否决。与 ADR-0051 和「从设置迁到
  插件」的产品方向冲突。
...
- **移植 CodeG 的自动化 / 待办 / 任务汇报工具。** 否决。Cinyuverse 已有独立领域。
