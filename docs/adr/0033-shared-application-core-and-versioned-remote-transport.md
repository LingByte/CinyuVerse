...
Cinyuverse 采用 Codeg 的多端总体方式：同一 React 应用通过 `BackendTransport` 访问同一套
Application Core；Tauri 桌面和 Axum Web Server 只是不同适配器。未来移动端作为
Cinyuverse Server 的薄客户端，不在移动设备上运行本地 Agent、Git worktree 或 Artifact
工具。
...
映射，不重复业务规则。独立 `cinyuverse-server` 组装数据库、Agent runtime、Delegation
...
P0/P1 的 Cinyuverse Server 是单一所有者、多台可信设备的本地优先服务，不是多用户或多租户
...
Cinyuverse Remote Protocol 从第一版起版本化并提供 capability negotiation：
...
Android 的所有实时与写操作都要求其 Server Profile 对应的 Cinyuverse Host 在线。Host 可以
...
P1 不接入 FCM、第三方推送或 Cinyuverse 云中继。前台连接可以实时更新；用户显式开启持续
...
- 可以参考和复用 Codeg 的 Transport、Axum adapter、attach/replay 以及 Office proxy
  实现，但 Cinyuverse 应优先使用自己的持久化 Conversation sequence，而不是只依赖内存
  replay buffer。
