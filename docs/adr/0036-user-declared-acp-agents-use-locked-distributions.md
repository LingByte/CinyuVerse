...
Cinyuverse 已经通过 Built-in Agent Profile 与 ACP 官方 Registry 支持开放的
...
统一 Agent 列表。这个交互值得采用，但 Cinyuverse 不能因此绕过已经建立的 Registry
freshness、冻结目标、安装锁和启动前校验边界。

## Decision

Cinyuverse 新增 **User-declared agent definition（用户声明 Agent 定义）** 作为第三种受控
