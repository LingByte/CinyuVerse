...
本决定把「插件优先、一切皆插件」从口号落成可验收的平台契约：定义 Cinyuverse 口径下
...
消费者）、内核最小化纪律」应吸收；其「agent loop 也是插件」不适用（Cinyuverse 是 ACP
编排壳，loop 在外部 Agent 内），其单进程热插拔组合方式不适用（Cinyuverse 是 Rust 内核 +
独立 Node Worker 多进程），其插件树式叠加组合被产品决定排除（见第 3 节原则 8）。

## Part I — 概念、原则与接管面

### 1. 「一切皆插件」的 Cinyuverse 定义

> Cinyuverse 基座只承诺一件事：**多 Agent 会话 + 隔离工作区 + 可信历史**。
...
9. **借鉴设计、控制依赖。** 语义可以来自 Cordis / Theia，实现落在 Cinyuverse 自己的
...
| 派生开发 | `cinyuverse-plugin fork`（源码派生 + `derivedFrom` 溯源元数据 + 新身份打包） | 新增 | Batch 6 |
...
2. Federation 面板：`app.tab` / `app.panel` / `app.kanban.view` /
   ...
   `@module-federation/vite` 模板（进 `cinyuverse-plugin init`）。每个插件面板包在
...
插件仓库自包含：标准 manifest、SDK 依赖、CI（`cinyuverse-plugin build / validate /
...
- **调试与诊断**：宿主提供插件运行诊断面（每插件 Worker 存活、最近崩溃、stdout/
  ...
  崩溃落错误边界并可一键重试；`cinyuverse-plugin doctor` 汇总工具链、SDK 版本与
  Host 兼容性。
- **测试**：单元层用 Testing 包的 fake broker 与 App harness（ADR-0046）；
  集成层 `cinyuverse-plugin test`；宿主旅程层 `cinyuverse plugin test --host` 覆盖该包声明
...
- **派生（fork）**：`cinyuverse-plugin fork <source>` 从已安装包快照或 GitHub 仓库
...
| 官网 `cinyuverse-site` Catalog | 官方分类展示能力等价插件；GitHub 地址上传流程（既有）承接阶段二 |
...
6. **DX 基线**：插件运行诊断面 UI（Worker 存活 / 日志流 / readiness 失败证据）；
   `cinyuverse-plugin doctor` 覆盖新 kind；六个 slot 的 init 模板与 Skill 旅程文档。
...
**验收：** `cinyuverse plugin test --host` 覆盖六个 slot 各一条旅程；参考插件从市场
...
3. Module Federation runtime 集成：本地插件产物 HTTP 端点、`loadRemote`、React
   singleton；`@module-federation/vite` 模板进 `cinyuverse-plugin init`（`panel` /
   `kanban-view` 模板）。
4. **`cinyuverse-plugin dev` 面板 HMR 模式**：宿主 `loadRemote` 指向 Vite dev 端点，
   改代码不重载宿主；退出回落构建产物。
...
首屏尤其不得出现可感知的加载退化；插件详情页标注「随 Cinyuverse 提供，默认启用，
...
1. `cinyuverse-plugin fork <source>`：从已安装包快照或 GitHub 仓库派生，强制新
...
- **姊妹篇 [ADR-0070](0070-ssh-provisioned-remote-server.md)**：「工作区搬到
  远端」是 L0 核心路线（SSH 供给的远程 Cinyuverse Server），不是插件路线；其中的
