...
Cinyuverse 在 ACP 连接处建立 `Protocol Session` seam，并提供两个 Adapter：
...
Cinyuverse 保留 CONTEXT 中的 Turn 产品语义：一次用户发起到 Agent 对该前台工作的应答
...
`messageId`、`toolCallId`、`planId`、`terminalId` 都按 opaque string 保存，Cinyuverse 不
...
acknowledgement。Cinyuverse 使用本地 prompt id 与当次 acknowledgement 建立用户 Turn 到
Agent `messageId` 的映射，避免 replay 或多客户端观察时重复用户消息。

V1 缺少 ID 的 Message、Thought 和单 Plan 使用带命名空间的本地合成 ID。合成 ID 只
用于 Cinyuverse 投影，标记 `identity_origin = synthesized_v1`，绝不发回 Agent，也不能被
...
4. 将 replay 结果按 Item ID 与 Cinyuverse 当前语义状态协调，只为缺失或变化的事实追加
...
创建、完成或取消一个 Cinyuverse 当前 Turn；历史 replay 也不能重新打开已经结束的
Permission/Elicitation。只有 replay barrier 之后的 live observation 可以推进新的前台
Turn 和待决交互。

Cinyuverse Event Log 仍是可见 Conversation 历史的权威。Agent replay 用于恢复 Agent Session
的 Item 状态和弥补缺失观察，不能覆盖用户明确标题、Cinyuverse Turn 终态、权限决定、文件
...
V1 Client Terminal 是 Agent 请求 Cinyuverse 创建、等待、终止和释放的本地进程资源；V2
Display Terminal 是 Agent 自己拥有、Cinyuverse 只显示的 Session Item。二者不能共享一个
含控制方法的 interface。
...
- Cinyuverse 产品自身的交互终端不因 ACP V2 而删除，但它不是 V2 ACP Client capability。

### 10. Client 工具统一通过有作用域的 MCP 暴露

V2 Agent 若需要读取文件、编辑未保存 buffer 或运行由 Client 提供的命令，Cinyuverse 通过
...
- MCP 工具调用仍经过 Cinyuverse 权限政策，不能因从 ACP Client RPC 改为 MCP 而扩大权限。
...
ACP Diff 是“Agent 报告的修改”，Cinyuverse checkpoint/git diff 是“工作区观测到的修改”。
...
- 本 ADR 不启用远程 ACP URL、HTTP 或 WebSocket Transport。ADR-0010 的本地 stdio
  启动边界保持不变；ADR-0033 的 Cinyuverse Remote Protocol 也不是 ACP Transport。
...
| ADR-0005       | 保留 Cinyuverse 复制事件的 Fork 语义；Agent `session/fork` 继续独立门控，不视为 V2 baseline                      |
...
- Cinyuverse 的核心围绕 Session 状态与 Item 事实，而不是某一版 ACP 方法，未来协议变化集中
  在 Adapter；
...
- Agent replay 与 Cinyuverse 权威历史需要 reconciliation module，不能直接转发通知；
...
- Cinyuverse 决定启用远程 ACP Transport；
- 有足够遥测支持讨论 V1 退役。
