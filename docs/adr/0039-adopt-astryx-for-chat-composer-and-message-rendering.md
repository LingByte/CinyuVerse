...
提供 150+ 组件与完整 Chat 组件族。经评估，其"纯文本 + 结构化 token"模型与 Cinyuverse
...
7. **用户消息气泡**：消息流中的用户消息统一由 `ChatMessage(sender="user")` +
   ...
   的受限语法渲染器，并以内联插件承接结构化 token。Cinyuverse 仅在主题层覆盖亮色
...
- **集成方式**：Astryx 为预编译 ESM + 预编译 CSS（`@astryxdesign/core/astryx.css`），
  无需 PostCSS/Babel 构建插件，`className` 可覆盖样式，不影响 Cinyuverse 现有
...
5. **阶段 4**：残留清理 —— `cmdk` 残留直接删除；Radix 封装**保留不替换**（用户决策
   2026-08-08：11 个 shadcn 封装工作正常且已用 Cinyuverse tokens，替换仅带来视觉变化
