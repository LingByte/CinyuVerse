...
`cinyuverse-mcp` 与 `cinyuverse-workflow-mcp` 是 Cinyuverse 官方产品 MCP。它们作为 Host 家族
...
- `cinyuverse-mcp` 与 Host 可执行文件同目录，沿用现有 sibling / `CINYUVERSE_MCP_BIN` 定位。
- `cinyuverse-workflow-mcp` 作为 `cinyuverse.workflow-creator` 的 managed Runtime，由
  Host 解析，不依赖用户 PATH。
...
| 产品 | 官方插件 | MCP | 生效条件 |
| --- | --- | --- | --- |
| 多智能体协同 | `cinyuverse.collaboration` | `cinyuverse-mcp` | 插件 Enabled，且 Agent 广告 `session/new.mcp_servers` |
| 工作流开发 | `cinyuverse.workflow-creator` | `cinyuverse-workflow-mcp` | 同上，且 Agent binding 包含该 Agent |
...
不得把 Workflow 工具并入 `cinyuverse-mcp`。Delegation Broker 与 listener 可以随
Host 常驻；没有协作插件时 companion 不会被注入，Broker 不会被 Agent 打到。

`cinyuverse.collaboration` 是随 Host 分发的 builtin 插件，默认 Disabled。它提供
协同 Skill 与 MCP 激活门，不要求 Worker。原生 Studio 与 Workflow Core 仍不
依赖 `cinyuverse.workflow-creator`；禁用该插件不删除源文件、版本、Automation 或
Run。
...
- 桌面与 Server 在安装 injector 前必须已有 Plugin control plane，并在 builtin
  导入后同步门状态。
- `cinyuverse.collaboration` 进入 `assets/plugins/`，走与 Office 相同的 materialize /
  import 路径。
- companion 注入新增稳定失败码 `official_product_mcp_disabled`。
- 本决定取代“平台几乎总是注入 `cinyuverse-mcp`”的隐含行为。

## Considered Options

- **继续把 `cinyuverse-mcp` 当无门控平台能力。** 否决。用户无法独立关闭协同而不
  伤害 Host。
