...
1. 设置 → 插件页副标题只说「通过可安装的插件为 Cinyuverse 增加新能力」，用户打开开关后
...
3. `cinyuverse-plugin init` 写出的 Node Worker 没有 stdio 循环，Host 启动即退出；`file-tab`
   实际是只读预览；测试用 `import.meta.url` 在 `cinyuverse-plugin test` 下会找不到文件。
4. 开发链接同时存在产品 CLI、Plugin Dev HTTP、开发工具对话框和开发 MCP。开发 MCP
   只返回 `pending_confirmation`，不能当调试面。
5. 产品官网 `~/Projects/cinyuverse-site` 已有 `/marketplace` 与
...
默认禁用，可以卸载。Host 家族仍按 ADR-0055 携带官方 MCP **二进制**（`cinyuverse-mcp`、
`cinyuverse-workflow-mcp`）；磁盘上有二进制不等于已安装插件，更不等于已注入会话。
...
`*.cinyuverse-workflow.json` 不走插件编辑面。

### 3. `init` 必须产出 Host 可运行的包

`cinyuverse-plugin init` 的完成标准是：对选用模板，`build` / `validate` / `test` 通过，
...
- `engines.cinyuverse` 为 `>=0.1.3 <1.0.0`，`engines.pluginSdk` 为 `^1.0.0`。
...
- Python 模板保持 `run_stdio_plugin_worker`；Rust 模板的清单 path 指向编译产物说明，
  并在 README 写明 `cinyuverse-plugin build` 不编译 Rust。
...
```text
cinyuverse-plugin init | build | validate | test | pack | toolchain
cinyuverse plugin add --dev .
cinyuverse plugin test --host
cinyuverse plugin pack
cinyuverse plugin add --profile | --web
```

`cinyuverse plugin add --dev .` 是链接本机目录的唯一产品入口：build、validate、经本机
Host token 调用 `plugin_control_import`（`developerLink`），尝试启用，并监视 digest。
Host 未运行时写入 `~/.cinyuverse/imports/links.jsonl`。

删除插件开发包的 `content.mcp`（`plugin_dev_link_request` / `plugin_dev_link_status`）、
根配置 `devMcp`、以及 `cinyuverse-mcp` 的 `plugin-dev` 产品面。不要用半截 MCP 当调试器。
Agent 按插件开发 Skill 直接跑 CLI。

`cinyuverse-plugin install --link` / `dev` / `doctor` 若仍实现，只能作为 `add --dev` 的
同一 Host import 的别名，不得再要求 `CINYUVERSE_PLUGIN_DEV_GRANT`。设置页「插件开发」
对话框不再展示 loopback endpoint 或索要凭据。需要该产品时，从市场官方分类安装
「插件开发」插件。

插件开发 Skill 必须写清完整开发与测试规范：定位本机 `cinyuverse-plugin toolchain`、
...
市场 Tab 接入 `https://cinyuverse.xforever.xin/marketplace`。Host 只消费官网提供的
...
`cinyuverse plugin add --profile` / `--web`、离线官方缓存。弹窗至少展示：owner、
...
产品命令是 `cinyuverse plugin test --host`。没有这条旅程，不得声称模板或官方包完成。

## Marketplace Catalog API

产品官网（`~/Projects/cinyuverse-site`）为 Host 增加只读 Catalog API，与现有提交/审核
