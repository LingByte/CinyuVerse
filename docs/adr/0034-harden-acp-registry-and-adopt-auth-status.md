...
`session_info_update`、`_meta` 传播及会话成本等能力，而 Cinyuverse 尚未完整使用。
ACP 的
...
认证状态查询，避免用 `session/new` 的不确定错误行为猜测认证状态。Cinyuverse 决定现在
...
1. Cinyuverse 默认以用户授予的本机权限运行；本 ADR 不引入 Agent 沙箱或权限收缩。
...
5. Cinyuverse 观察认证状态；账号动作按 ADR-0037 仅从 Built-in Agent Profile 白名单启动。
   API Key 仍由已适配 Agent 原生配置文件持有，可在本机明文保存；Cinyuverse 不验证
   API Key 的远程有效性。
...
Cinyuverse 的 Agent 管理以以下八条不可违反的原则为基础：
...
7. **用户配置写入必须原子、可比较、可恢复。** 任何失败都不能留下半套配置；并发
   修改必须向用户暴露冲突决策。
8. **草案协议只能通过兼容层进入核心。** 核心领域依赖 Cinyuverse 的稳定认证观察模型，
...
外部安装同样重新验证记录的内容证据，但 Cinyuverse 不删除或修改外部文件。
...
Cinyuverse 保存 initialize 返回的协议版本、agent info 和完整能力结构，至少包括：
...
`auth/status` 草案故意不报告认证方法，因此 Cinyuverse 不把 `authenticated: true` 自动翻译
为“已通过账号登录”：
...
2. **`session/list` 与 `session/delete`**：提供显式“导入 Agent 会话”流程和外部会话
   删除操作。ACP session 与 Cinyuverse Conversation 保持不同身份；列表不自动写入 Cinyuverse
   历史，删除也不级联删除 Cinyuverse 事件日志。
3. **`session_info_update`**：保存 Agent 侧标题/时间元数据，但 Cinyuverse 用户标题优先；
   Agent 更新不能覆盖用户明确命名。
...
- [ ] 恢复中断的 Queued/Running operation。
  ...
  - Verify：`cargo test -p cinyuverse agent_operation_recovery`。
