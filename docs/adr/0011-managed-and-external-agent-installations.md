...
Cinyuverse 将每个 Agent 安装明确记录为托管安装或外部安装。通用 Registry
Agent 及独立 ACP 适配器由 Cinyuverse 安装和管理，不再修改全局包，也不在每次启动时
临时下载执行；内置 Agent 的本地 Agent runtime 可以采用经绝对路径和兼容性校验的
现有系统安装，也可以由用户选择交给 Cinyuverse 托管安装。

托管安装成功后，Cinyuverse 将本地 Agent Runtime 的主命令发布为用户级稳定入口：
Unix 使用 `~/.local/bin`，Windows 使用同一用户目录并写入 User PATH。入口只是一层
由 Cinyuverse 标记所有权的 shim，实际制品仍保留在版本锁定的 Agent 私有目录；更新和
...
Node、uv、Python 等基础运行环境可以复用兼容的系统环境，也可以使用 Cinyuverse
维护的版本化共享运行环境。对于适配器型内置 Agent，ACP 适配器始终是托管安装，
并绑定到已经校验的确切 Agent runtime 路径。

基础运行环境采用“系统优先、托管兜底”：Cinyuverse 先通过跨平台 PATH 刷新解析 Node
...
环境不兼容直接中止，而是自动回退到对应平台上由 Cinyuverse 固定版本和校验和的共享运行
环境；托管 Node 与 npm 也必须分别通过有界版本检查。任何一种选择都不得修改用户的
系统安装，安装日志与预检查必须显示实际采用的环境来源和绝对路径。

Cinyuverse 为托管 Agent 发布终端命令时，稳定 shim 必须从当前 Installation lock 读取
...
因此，升级、修复和卸载只能修改 Cinyuverse 托管的产物；外部安装只能被使用和重新
校验，不能由 Cinyuverse 修改或删除。未来实现需要用作用域内、可审计的安装取代当前
全局 npm 安装策略，并在持久化安装记录中保存所有权。

Built-in Agent 在设置页打开时主动探测其官方本地 Runtime。只有候选绝对路径、
版本、完整性与 ACP 握手均通过，Cinyuverse 才自动将其接入为外部安装；探测过程不下载、
