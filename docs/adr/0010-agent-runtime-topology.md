...
“使用本地 Agent runtime”不等于每个 Agent 都必须在 ACP server 之外再有一个程序。Cinyuverse 区分原生 ACP agent（同一安装物同时提供 Agent runtime 与 ACP）和适配器型 ACP agent（ACP 桥接独立的本地 Agent runtime）。Built-in Agent Profile 声明拓扑，以及适配器是否自带可用的 vendor CLI。
...
- 普通 Registry Agent 若实际依赖 manifest 未声明的外部 runtime，预检失败后标记为不受支持；Cinyuverse 不为其补写专属适配。
...
- 首版仅启动由官方 Registry 分发、在本机运行的 ACP `stdio` 进程（Binary、npx、uvx）；
  ...
  的可验证绑定。
- Built-in Agent Profile 随 Cinyuverse 应用发布更新；不得从远端动态下载或覆盖其运行拓扑、
  检测规则、版本组合或完整性基线。
