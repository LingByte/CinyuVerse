...
Cinyuverse 将 **Plugin（插件）** 定义为用户可发现、可启用、可审计的一组外接能力，而
...
`ArtifactToolProvider` 是 Cinyuverse 拥有的运行时接口，用于把已验证工具接入统一的
...
文件系统持有；Cinyuverse 的 Artifact 记录保存路径、类型、内容哈希、生成者、关联
Conversation/Turn 和修订证据，事件日志记录 Artifact 生命周期引用，不复制或冒充
文件内容本身。

第三方无代码 manifest 只能引用 Cinyuverse 已知的 Provider 类型。允许第三方提供原生
...
插件启用成功后，Cinyuverse 必须让用户选择把插件内嵌 Skill 一键同步到所有 Agent，或
...
- Office Work 的首批内置动作至少覆盖创建/修改 PPTX、创建/修改 DOCX、分析/生成
  XLSX；具体模板可参考 Codeg Office skills，但采用 Cinyuverse 插件 manifest。
