...
Cinyuverse 只发行一个执行面：**Cinyuverse Host**。手机 App、IM 渠道、其它桌面 Cinyuverse
...
同一数据目录同一时刻只有一个 Host。Host 可以是 Cinyuverse.app 或 `cinyuverse-server`。
...
手机 Companion ──┐
IM Channel ──────┤
其它桌面 Workstation ┤── Remote Protocol / Chat channel ── Cinyuverse Host
本机桌面 ────────┘         （Desktop 或 cinyuverse-server）
```

打开本机 Cinyuverse.app 时，该进程就是 Local Host。只有需要被其它设备连接或无头
常驻时，才打开远程监听或改跑 `cinyuverse-server`。本机日常使用不强制 sidecar。
同一数据目录被 `cinyuverse-server` 占用时，本机桌面只能作为客户端连接，不能再
启动第二份 Host。
...
| 产物 | 形态 | 内容 |
| --- | --- | --- |
| Cinyuverse Desktop | `.dmg` / NSIS / `.msi` / `.AppImage` / `.deb` | 壳 + Host + 前端 + `cinyuverse-mcp` + 官方插件快照 |
| Cinyuverse Server | 跨平台目录 / 可选 Docker | `cinyuverse-server`、`cinyuverse-mcp`、`web/`、`plugins/bundled/` |
| Cinyuverse Companion | Android APK，后续 iOS | 薄客户端，不跑 Agent 或插件 |
...
其它电脑上的 Cinyuverse.app。近乎全接管 Host 上的工作：Project / Workspace /
...
授权发送者的入站命令，并投递无 secret 摘要。Cinyuverse 不发行这些 IM 的客户端。
...
- Chat channel 适配器属于 Host，随 `cinyuverse-server` 一起运行。
- 本阶段不把缺少 Apple / Windows 身份证书当作发布失败。

## Considered Options

- **本机也强制独立 `cinyuverse-server` sidecar。** 否决。增加端口、配对和双进程，
