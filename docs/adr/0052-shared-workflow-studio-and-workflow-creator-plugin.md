...
7. Workflow Creator 是官方、可选、随 Cinyuverse 分发的产品插件。原生 Studio 不依赖插件；插件禁用
   不删除源文件、版本、Automation 或 Run。
8. Plugin 包含 `cinyuverse-workflow-creator` Skill、`cinyuverse-workflow-mcp`、`workflow.definition` content、
   `*.cinyuverse-workflow.json` file opener 和 `artifact.editor` App surface，并且只消费公共 SDK。
