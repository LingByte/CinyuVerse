...
1. Cinyuverse Plugin 在产品界面中是一个不可再拆分的功能产品，不使用“平台扩展/Agent 扩展”分类。
...
9. Agent-native plugin 不复用 Cinyuverse 产品详情。它在对应 Agent 设置底部以列表加预览呈现，
   预览只包含 Skill、MCP、Runtime、Hook 与 Workflow，不展示 App extension、Cinyuverse capability、
...
- Agent-native plugin 仍在各 Agent 设置内管理，但不会影响 Cinyuverse Product Plugin 的统一展示。
- Agent-native 资源投影与 Cinyuverse Product Detail 使用不同 DTO/视图边界，避免任何一侧的术语或
  权限语义泄漏到另一侧。
