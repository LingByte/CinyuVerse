...
仓库：`~/Projects/cinyuverse-site`。不改审核模型（待审不上架）。
...
返回字段对齐并补 tag/digest。CORS：允许桌面与 `cinyuverse-server` 所在的回环与用户配置
的 Host 来源读取公开目录。
...
`cinyuverse plugin add --web <github>` 写入 `sourceKind=github`。
`cinyuverse plugin add --web https://cinyuverse.xforever.xin/marketplace/<owner>/<name>`
写入 `sourceKind=marketplace`。`plugin update` 只跟随来源锁。

### 3.4 官方包不再预装

- 停止启动时 `materialize_builtin_plugins` 自动 import 到 catalog。
- 官方五件套在官网以 `category=official` 上架（owner 建议 `cinyuverse`）。
- Host 家族可继续打包快照目录，仅供离线市场官方分类安装。
- 迁移：已有 builtin 记录改为官方市场来源锁（owner=`cinyuverse`，name=现 plugin id，
...
- **官网与 Host 域名。** 用户指定 `https://cinyuverse.xforever.xin/marketplace`。实现时以
