...
Cinyuverse 的默认 Agent 档案与 Codeg 当前默认集合对齐：Claude Code、Codex、Gemini、
...
本 ADR 取代 ADR-0012、ADR-0021 与 ADR-0035 中“认证只能只读显示、Cinyuverse 不得启动
登录或注销”的限制，但保留安装和认证相互独立、后台不得自动触发交互式认证的原则。

用户在“设置 → Agent”明确点击后，Cinyuverse 可以执行两类档案动作：
...
持有；Cinyuverse 不截获标准输入、不读取终端输出，也不把交互内容写入数据库或诊断记录。
...
官方凭据文件存在。为与上述固定 Codeg 基线保持一致，Cinyuverse 仅在 API 模式且不存在真实
凭据时写入固定的本地门禁标记；该标记不是访问令牌、不会传给模型端点、不会覆盖真实
Kimi 凭据，切回订阅登录时也只删除由 Cinyuverse 写入且值完全匹配的标记。真实 OAuth Token
仍只由 Kimi 官方 CLI 管理。

OpenCode 的 Provider 连接是已适配原生配置，不是任意文件写入。Cinyuverse 可结构化读写
...
`content` 保留文档完整结构（键名、嵌套与数组），所有值一律替换为固定掩码，使前端
悬停时能看到有意义的脱敏结构而非全量点号；无法无歧义脱敏（如未解析或 YAML 中值可能
出现在键与标签中）时返回空内容。前端可对这份已脱敏预览默认模糊，并在悬停或键盘聚焦
时解除模糊；完整凭据文件不得进入前端 DOM 或状态。
...
`uv` 时，Cinyuverse 从 Astral 官方发布页下载与 Codeg 对齐的固定版本，按六个平台固定的
...
并行工具、搜索、描述与基础指令，未覆盖字段继续继承完整官方模板。Cinyuverse 生成的 catalog
...
Cinyuverse 按 Codex 的路径规则解析绝对路径、`~/` 与 `CODEX_HOME` 相对路径，在 8 MiB
...
Cinyuverse 生成自己的 catalog 与 sidecar。
...
`PI_ACP_TRUST_WORKSPACE=0` 可关闭该行为，且此 Cinyuverse 策略变量不会传给子进程。
外部安装探测可临时使用这些 Pi Runtime 设置完成握手，但持久化安装锁不得冻结它们；启动
时当前设置必须覆盖旧锁，用户清空字段时也必须清除旧锁或父进程继承的同名变量。

Claude Code、Codex 与 Gemini 支持 Cinyuverse 本地 Model Provider 预设。预设保存名称、Agent
类型、端点、模型映射和凭据，绑定时分别投影到三个 Agent 的官方原生配置；Claude 支持
八项结构化模型映射，Codex 标准预设只替换官方配置里的 provider 字段
（`model_provider = "cinyuverse"`、`[model_providers.cinyuverse]`、`auth.json` 的
`OPENAI_API_KEY`、顶层 `model`），不读、不写、不删除 `model_catalog_json` 或外部
catalog 文件；Gemini 使用单模型映射。更新已绑定预设会重新投影。首次绑定前由 Cinyuverse
