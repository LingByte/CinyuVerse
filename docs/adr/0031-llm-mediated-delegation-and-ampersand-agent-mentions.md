...
Cinyuverse 的首个完整多 Agent 协作模式采用 Codeg 已验证的 LLM-mediated delegation：
父 Agent 通过按会话注入的 `cinyuverse-mcp` companion 调用异步委派工具，主进程内
...
现有 `crates/delegation`、`crates/delegation-proto`、`crates/cinyuverse-mcp` 和
`src-tauri/src/delegation` 是继续完善的基础。目标是行为对齐 Codeg，而不是把其巨型
Broker 文件整体复制进 Cinyuverse；现有模块边界可以替换，只要公开行为与不变量成立。
...
- 使用 `@` 作为 Agent Mention：否决。Cinyuverse 选择 `&` 区分 Agent 与其它引用语法。
