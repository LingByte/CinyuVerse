...
凭据或绑定 Workspace。现有 `cinyuverse-mcp` 是平台内置的多 Agent 协作 companion；把 Workflow 工具
...
`cinyuverse-mcp` 退场已由 [ADR-0057](0057-session-enhance-and-multi-agent-plugins.md) 落地。

## Decision

1. 每个产品 Plugin 独立拥有 MCP identity、tool catalog、protocol compatibility、Runtime 和 Agent
   binding。Workflow Creator 使用 `cinyuverse-workflow-mcp`；未来协作插件使用独立 MCP。
...
- 平台 `cinyuverse-mcp` 在 ADR-0057 落地的同一版本删除注入；Workflow 交付不依赖该迁移。
- Full Trust 允许 Runtime 执行，但动态身份与 generation lifecycle 仍由 Host 管理以保证正确性。

## Considered options

- **扩展现有 `cinyuverse-mcp`。** 否决。它把多 Agent 协作和 Workflow Creator 变成不可独立演进的单一
  产品。
