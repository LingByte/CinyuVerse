...
先纠正一个先前比较里的错误结论：Cinyuverse 的 Office 预览**不是**一次性渲染。
`.cinyuverse-plugin/plugin.json` 的 `office-preview` 声明的进程是
...
**其一，技能是手抄的。** Cinyuverse 在 `contents/skills/` 下手写了三份 SKILL.md
...
判据是「还有没有人在看」，Cinyuverse 的判据是「起来多久了」。
...
**构建期而非运行期**，这是与 Codeg 的必要分歧：Cinyuverse 的插件内容是内容寻址
锁定的（`content.index.json` + `package.lock.json`），运行期改写 `contents/`
会让锁失效。Codeg 没有这层锁，所以它能在运行期同步。Cinyuverse 不能为了对齐做法
而破坏自己的完整性保证。
...
连接断开。Codeg 用 SSE 连接数做这件事，Cinyuverse 用租约续期，判据等价而不依赖
预览内容协议。
...
Codeg 保留了 `officecli_render_html` 作为 watch 之外的路径。Cinyuverse **不加**：
