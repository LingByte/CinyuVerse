...
原 ADR 的前提"Cinyuverse 无 ACP `session/fork` 线路调用"经复核**不准确**：Cinyuverse 依赖的
...
让 agent **在服务端真正分叉出保留上下文的新会话 S2**，Cinyuverse 侧再建兄弟 Conversation
...
（下述）用于 Cinyuverse 侧兄弟会话的可见历史 + 绑定 S2；(5) agent 未广告 fork 时按下述
导入语义降级并明示。**下方"复制事件到分叉点"仍是 Cinyuverse 侧会话历史的构建方式，不变。**
