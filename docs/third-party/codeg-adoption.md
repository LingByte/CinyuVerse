...
## Cinyuverse targets and adaptations
...
`crates/server/src/preview_proxy.rs`. Cinyuverse accepts only Artifact Service
...
The resulting Rust is substantially reorganized for Cinyuverse's ports-and-adapters
...
Cinyuverse reimplemented the observable behavior within its existing
`delegation`, `delegation-proto`, and `cinyuverse-mcp` crates. Adopted behavior
...
`@Agent`/`codeg://agent` convention to Cinyuverse's structured `&Agent` and
`cinyuverse://agent` contract. Cinyuverse-specific changes include open `AgentId`,
...
Cinyuverse reimplemented the observable scheduling behavior in the
...
templates. Cinyuverse-specific changes include a versioned canonical
...
ports, migrations, and tests were rewritten around Cinyuverse's event-sourced
...
Cinyuverse adopted the observable transport separation and attach lifecycle:
...
delivery. Cinyuverse-specific changes replace Codeg's arbitrary string dispatch
...
No Codeg Web router or Axum server file was copied byte-for-byte. Cinyuverse adapted
the observable authenticated headless-server boundary, one WebSocket carrying
multiple durable subscriptions, and static SPA hosting. Cinyuverse-specific changes
...
actual `cinyuverse-server`, serve the built static tree, authenticate, exercise SPA
...
are generated from Cinyuverse's own `remote-protocol` Rust DTOs.
...
both archives by SHA-256, and does not package either tool with Cinyuverse. This adds
...
Cinyuverse adopted the complete default Agent catalog and the observable settings
...
environment editor and install-log/diagnostics dialog. Cinyuverse exposes equivalent
...
Cinyuverse retains its own open `AgentId`, managed-install lock,
...
The implementation is reorganized around Cinyuverse's `agents`, `services`, `db`, and
...
- `cargo test -p cinyuverse office`
- `cargo test -p delegation-proto`
- `cargo test -p delegation`
- `cargo test -p cinyuverse-mcp`
...
- `cargo test -p cinyuverse remote_desktop`
...
- `cargo test -p cinyuverse --features qa-mode agent_management::tests`
- `cd frontend && pnpm exec vitest run src/pages/settings/Agent*.test.tsx`

## Apache-2.0 obligations

Cinyuverse preserves the upstream license and attribution in
`THIRD_PARTY_NOTICES.md`. A complete copy of Apache License 2.0 is in
`docs/third-party/licenses/Apache-2.0.txt`. Modified files carry Cinyuverse-specific
