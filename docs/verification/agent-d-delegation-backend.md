...
- Pre-change baselines:
  ...
  - `cargo test -p cinyuverse-mcp`: 10 passed
...
|---|---|---|
| Capability injection | `companion_injection_follows_capability` did not compile because the injector accepted Agent identity rather than capabilities | `cargo test -p cinyuverse companion_injection_follows_capability --lib` |
...
| Mention schema | `tools/list` omitted AgentMention and `cinyuverse://agent` semantics | `cargo test -p cinyuverse-mcp tool_schema_explains_agent_mentions` |
...
| Session info | `sessions` exposed no tool and listener had no resolver | `cargo test -p cinyuverse-mcp session_info_feature_is_independent`; `cargo test -p delegation session_info_resolves_only_with_its_feature_service --lib` |
...
| Long transcript stream | A single assistant message over 10,000 deltas lost its first chunks | `cargo test -p cinyuverse compact_transcript_does_not_cut_a_long_streamed_message --lib` |
...
- Unix UDS integration:
  `cinyuverse-mcp::tests::call_broker_round_trips_over_uds`.
- Windows named-pipe integration is compiled and run on Windows:
  `cinyuverse-mcp::tests::call_broker_round_trips_over_named_pipe`.
