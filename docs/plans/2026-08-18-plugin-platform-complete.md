# Cinyuverse 插件平台完整实现设计

| 字段 | 值 |
| --- | --- |
| 文档标题 | Cinyuverse 插件平台：使「除核心服务与页面骨架外皆为插件」成为可交付系统 |
| 作者 | Cinyuverse maintainers |
| 日期 | 2026-08-18 |
...
Cinyuverse 已经具备 v4 产品包、candidate-first 激活、Full Trust、Office 参考插件、linked-dev 与 CLI 的纵向切片，但公共扩展面仍然是封闭枚举，Host Broker 只有两条 RPC，Worker 只能跑 Node ESM，官方产品 MCP 靠插件 ID 特判，Agent 绑定写死 13 个内置身份。用户无法把「描述需求 → Agent 写插件 → 调试接入 → 启用」走通；第三方也无法把除文件预览之外的产品面真正插进 Host。
...
3. 官方会话增强、多智能体协同、Workflow Creator 声明 `content.mcp` + 一等字段 `managedRuntime.hostFamilyBinary`，进程仍是 Host 家族 `cinyuverse-mcp` / `cinyuverse-workflow-mcp`。删除 `OfficialProductMcpGate`、`project_official_product_mcp`、按 ID 注入。不写 JS `session.mjs`。
...
- 用户可通过插件高度定制 Cinyuverse：扩展前端、后端，以及 Cinyuverse 管理的全部 Agent。
...
- 不把 Agent-native Codex/Claude 插件自动翻译成 Cinyuverse App contribution。
- 不在 v4 Full Trust API 上静默恢复 sandbox / 权限弹窗。
- 不让 Rust/Python GUI 跑进 App iframe。
- 不保留 PluginAction 作为公共或运行时概念。
- 不把 Cinyuverse git checkout 当作产品开发路径。
...
| K9 | **官方 MCP 进程模型：`managedRuntime.kind = hostFamilyBinary`。** 进程是 `locate_cinyuverse_mcp_binary()` / sibling `cinyuverse-workflow-mcp`。执行权威仍在 Application Core。包只声明 binaryId、args 模板、`hostScopes`、config→feature 映射。Desktop 与 `cinyuverse-server` 共用**一份** Registry 注入实现。删除 `extra_stdio_servers`。删除 workflow-creator 的 JS `dist/mcp/workflow-control.mjs`，不再物化。 | 不把会话/委派重写成 JS Worker；不双注入。 |
...
| K18 | **SDK/CLI/Skill 随 Host 家族，落在 ADR-0054 树下 `sdk/` 与 `bin/cinyuverse-plugin`。** Locator 优先 `cinyuverse plugin toolchain`（PATH / sibling），不依赖 MCP，不搜 `CONTEXT.md`。 | 无会话也能定位。 |
| K19 | **`cinyuverse.plugin-development` 不可卸载。Skill 默认启用并投影。`plugin.dev.*` MCP 默认关闭，直到用户开启开发会话。** 这是对 ADR-0047 的唯一例外，且只覆盖 Skill。 | 发现 vs 执行分离。 |
| K20 | **`plugin.dev.link` 必须先经用户确认 + 与 Marketplace 相同的 Full Trust 文案。** Grant 绑定 (conversation_id, publisher, plugin_id, source_digest)。工具结果不含 token。CLI 读 Host 数据目录 grant 文件。删除产品路径上的 `CINYUVERSE_PLUGIN_DEV_TOKEN`。 | 否则是 Agent→Full Trust RCE。 |
...
| K25 | **`/plugins/:id` 配置 Tab 挂 `plugin.detail.panel`。** 删除 `agentDefaults` 特判的同一 merge 必须交付 `cinyuverse.multi-agent` 的 panel。通用表单复用已有 `frontend/src/components/rjsf`。删除 `PluginsSettings.tsx` 产品面。 | ADR-0057。 |
...
| B14 | locator 找 `CONTEXT.md` | 用户项目找不到工具链 | ADR-0054 树 + `cinyuverse plugin toolchain`。 |
...
| Core 对象 | 职责 |
| --- | --- |
| Host 进程 | Desktop 或 `cinyuverse-server`，独占数据目录 |
...
flowchart TB
  subgraph Authoring["作者 / Agent"]
    Skill["开发 Skill 默认投影"]
    CLI["cinyuverse-plugin / cinyuverse plugin toolchain"]
    DevMCP["plugin.dev MCP 默认关"]
    Skill --> CLI
    DevMCP -->|"用户确认后 link"| CLI
  end

  subgraph Host["Cinyuverse Host"]
...
accept.fileNameSuffixes: [".cinyuverse-workflow.json"]
...
4. **删除特判与交付 `cinyuverse.multi-agent` 的 `plugin.detail.panel` 是同一 merge**（PR-P12）。panel 负责子智能体 ACP mode/config 探测 UI（ADR-0057）。session-enhance 四个布尔可用通用表单，不强制 panel。
5. `PluginsSettings.tsx` 产品插件面删除。Agent-native 入口只留「设置 → Agent」底部。产品路由只有 `/plugins` 与 `/plugins/:id`。

测试：`cinyuverse.multi-agent` 启用后配置 Tab 能改 `depthLimit` 与某个 Agent 的 `agentDefaults`，且 `config.json` 被原子写回。
...
CompiledPluginPackage.entrypoints.worker.runtime
  node    → lock cinyuverse-plugin-worker-node @ 22.22.3 + target + sha256（已有）
  python  → lock cinyuverse-plugin-worker-cpython @ 3.12.11 + target + sha256（PR-P04 写入真实哈希）
...
| 包 | `@cinyuverse/plugin-sdk` 子路径 `/worker` `/app` `/testing` `/protocol` `/stdio` `/host` `/isolated` |
| 打包 | 现有 esbuild → `dist/worker.mjs`；纯 Node 复制 ESM |
| Isolated | `@cinyuverse/plugin-sdk/isolated` 导出同一 worker，不导出 Node `fs` 助手。运行靠 OS sandbox |
...
| 项 | 规格 |
| --- | --- |
| 公共 API | `cinyuverse_plugin.define_plugin_worker(setup)`；`registrar.handle`；`environment.host.call`；`environment.log`；`registrar.on_dispose`；`PluginSdkError(code, message)` |
| 同步 vs async | setup/handler 可为 sync 或 `async def`。stdio 循环用 **asyncio**（`asyncio.run`）。sync handler 在默认 executor 跑 |
| 包布局 | `sdk/python/pyproject.toml`，包名 `cinyuverse-plugin`，`src/cinyuverse_plugin/{__init__,stdio,worker,host,testing,isolated}.py` |
| Isolated | `from cinyuverse_plugin.isolated import define_plugin_worker`，文档声明 fs/net/subprocess 会被 OS 拒绝 |
| 打包 | `cinyuverse-plugin build` 识别 `runtime/worker.py` 或 `[tool.cinyuverse.plugin]`。Vendor **仅纯 Python wheel 或 py3-none-any**。带 native 扩展（manylinux / macOS / win_amd64）→ `build` 失败 `python_native_wheel_forbidden`。site 写入 `dist/python-site/`，生成 `dist/python-site.lock.json`（name、version、sha256）计入 package digest |
| Lock | 见 §6.4.1。`PYTHONSAFEPATH=1` `PYTHONNOUSERSITE=1` `PYTHONPATH={lock}/lib:{package}/dist/python-site` |
| CI | `{cpython}/python -m cinyuverse_plugin.protocol_fixtures packages/plugin-contract/fixtures/protocol` |
...
identity：`cinyuverse-plugin-worker-cpython + 3.12.11 + target + sha256:{archive}`。

#### 6.5 Rust implementer spec

| 项 | 规格 |
| --- | --- |
| crate | `cinyuverse-plugin-sdk`，MSRV **1.85**（与 workspace nightly 兼容的稳定下限写在 crate；CI 用仓库 `rust-toolchain.toml`） |
...
| 打包 | `cinyuverse-plugin build` 调 `cargo build --release --target <triple> --locked`。产物 `dist/worker-<triple>`（Windows `.exe`）。`.vxp` 可含多个 triple；Host 只 exec 当前 triple |
...
| CI | `cargo test -p cinyuverse-plugin-sdk protocol_fixtures` 读同一 `*.jsonl` |
...
env:  继承用户环境 + NO_COLOR=1 + CINYUVERSE_PACKAGE_CLASS=full-trust
```
...
env:  继承用户环境 + PYTHONSAFEPATH=1 + PYTHONNOUSERSITE=1
      ...
      + CINYUVERSE_PACKAGE_CLASS=full-trust
```
...
env:  继承用户环境 + CINYUVERSE_PACKAGE_CLASS=full-trust
```
...
```
env:  只保留
      CINYUVERSE_PLUGIN_ID, CINYUVERSE_GENERATION, CINYUVERSE_PACKAGE_CLASS=isolated,
      CINYUVERSE_PROTOCOL=1.1, LANG, TZ,
...
无 `PATH` 指向用户工具。无 `HOME` 指向真家目录（可指 `plugin-data`）。无 `CINYUVERSE_SERVER_URL` 除非该 Isolated 包另有已授权 Host API（官方 MCP 永不进 Isolated Worker）。
...
`contents/mcps/workflow-control.json`：`binaryId: "cinyuverse-workflow-mcp"`，`argsTemplate: []`，`hostScopes: ["workflow.*"]`。删除今日 `entrypoint: "dist/mcp/workflow-control.mjs"` 与包内 `dist/mcp/workflow-control.mjs` / `runtime/mcp-server.mjs`。P18 **不**再对该 JS 做 Node materialize。进程只 exec Host sibling `cinyuverse-workflow-mcp`。

`binaryId` 解析：

| binaryId | 定位 |
| --- | --- |
| `cinyuverse-mcp` | 现 `locate_cinyuverse_mcp_binary()`：`CINYUVERSE_MCP_BIN` 或 sibling `cinyuverse-mcp[.exe]` |
| `cinyuverse-workflow-mcp` | 现 `locate_named_sibling("cinyuverse-workflow-mcp")` |

未知 `binaryId` → contribution incompatible。第三方包 **禁止** `hostFamilyBinary`（仅 `publisher=cinyuverse` 的 builtin 可声明）。第三方继续用 in-package `entrypoint` + Host 语言 lock。
...
injected_stdio_servers(context):
  ...
  for each published content.mcp
      ...
      and All-agents binding includes this agent:
        resolve binaryId → locate_cinyuverse_mcp_binary() | locate_named_sibling("cinyuverse-workflow-mcp")
...
删除 `COLLABORATION_PLUGIN_ID`：启动时若 `plugin_installations` 仍有 `cinyuverse.collaboration`，一次性改写为 `cinyuverse.multi-agent`（已有则丢弃旧行）。
...
`apply_official_product_runtime` 里按 `cinyuverse.multi-agent` 读 `depthLimit` 的逻辑改为：找 `hostScopes` 含 `delegation.*` 的已启用 MCP 插件的 `config.json`。

**`crates/server/src/product_mcp.rs` `/internal/companion`**：

今日用 `Authorization: Bearer` 对 `gate.session_token()` / `gate.delegation_token()`，再按 `x-cinyuverse-product` 铸全开 `TokenPermissions`。替换为：

1. Bearer 必须是 §7.5 的 scoped `cinyuverse.plugin-mcp` token（`plugin_id` + `mcp_id` + `generation` + `scopes` + `conversation_id`）。
2. `x-cinyuverse-product` 必须与该 token 的 MCP contribution `product` 一致（`session` / `delegation`）。Workflow 官方二进制不走此 HTTP companion 面。
...
Office 幸存身份（`assets/plugins/office/.cinyuverse-plugin/plugin.json`）：
...
| Agent | 投影位置 | 格式 |
| --- | --- | --- |
| `claude_code` | `~/.claude/settings.json` 的 `hooks` 中 Cinyuverse 拥有块 `cinyuverse.plugin.<pluginId>` | Claude hook JSON |
| 其他 `hooks_project=false` | 不投影 | contribution `incompatible` |

冲突：同一 event 多个插件按 `(pluginId, id)` 稳定序全部注入，不覆盖。卸载只删 `cinyuverse.plugin.*` 块。
...
#### 9.1 `cinyuverse.plugin-development`

- builtin，不可卸载。
- **Skill 默认启用并投影**到所有 `skills_project` Agent。
- **`content.mcp`（`plugin.dev.*`）默认关闭。** 用户在 `/plugins/cinyuverse.plugin-development` 点「开始开发会话」或 CLI `cinyuverse plugin dev-session start` 才启用 MCP，并展示与 Marketplace 相同的 Full Trust 文案。关闭开发会话即停 MCP、作废 grant。
- Skill 第一步：**不依赖 MCP**。写明执行 `cinyuverse plugin toolchain`（见 §9.2）。MCP 仅用于确认后的 `plugin_dev_link_request`。

删除产品路径 `locate_toolchain.py`。仓库贡献者用 `pnpm --filter @cinyuverse/plugin-cli`，不进 Skill。
...
cinyuverse-server
cinyuverse-mcp
cinyuverse-workflow-mcp
cinyuverse-plugin          # CLI 入口：#/bin/sh 转调 Host 托管 Node lock + sdk/js/plugin-cli
web/
plugins/bundled/
sdk/
  plugin-contract/
  js/                 # @cinyuverse/plugin-sdk 与 @cinyuverse/plugin-cli 的 packed tgz + 解包
  python/             # cinyuverse-plugin wheel
  rust/               # cinyuverse-plugin-sdk .crate
```

**Desktop**：

- macOS：`Cinyuverse.app/Contents/Resources/{sdk,plugins/bundled}`；`Contents/MacOS/{cinyuverse,cinyuverse-mcp,cinyuverse-workflow-mcp,cinyuverse-plugin}`
- Windows：安装根 `resources/` 与 `cinyuverse-plugin.exe` sibling
- Linux：`/usr/lib/cinyuverse/` 或 AppImage `usr/lib/cinyuverse/`

体积：SDK 源/包约 15–25 MiB；**不含** CPython/Node lock（lock 在用户数据目录按需）。codesign：Desktop 现有 ad-hoc/正式签名覆盖 `MacOS/` 新二进制；`sdk/` 作资源。

`cinyuverse plugin toolchain`：查 `current_exe()` sibling，再查 `CINYUVERSE_HOST_ROOT`，再查 PATH。**不需要**活会话或 MCP。输出 §9.2 JSON（hostVersion、cli、contract、js、python、rust、templates）。Skill 与 Agent 都先跑这个命令。
...
8. CLI **禁止**扫描 grant 目录。`cinyuverse-plugin dev` 只在 **Host 本机**运行（Host console 终端，或该 Host 上 Agent 的 shell）。它调用 Application Core `plugin.dev.grant_for_cwd`：入参 `{workspaceId, sourceDigest}`，返回 `{endpoint, grantId}`；Host 再把 token 注入该 CLI 子进程环境。Companion / Workstation 上的 `cinyuverse-plugin dev` 必须失败，稳定码 `dev_link_host_only`。删除 `--token` 与 `CINYUVERSE_PLUGIN_DEV_TOKEN`。
...
`plugin_logs`：每插件 20 MiB 或 10_000 条，滚动删。Redaction：匹配 `token`、`authorization`、`secret`、`password`、`CINYUVERSE_SERVER_TOKEN`、`CINYUVERSE_PLUGIN_DEV_GRANT` 的 key 与 `Bearer ` 前缀；替换为 `[redacted]`。

Worker stderr 捕获入日志。App bridge `log` 同源。

`DoctorReport` 无 `grants` 字段。CLI `cinyuverse-plugin doctor` 调 Application Core `plugin.doctor`（Dev Protocol `/doctor` 改为该 Core 的 HTTP 适配），与 `/plugins/:id` 开发者抽屉同一 JSON。Remote 也走 `plugin.doctor`。
...
> 安装「{name}」即允许发布者 {publisher} 的代码使用这台电脑，权限与 Cinyuverse 相同。请确认你信任该发布者。

#### 11.2 Isolated SDK（四语言）

| 语言 | 入口 |
| --- | --- |
| TS/Node | `@cinyuverse/plugin-sdk/isolated` |
| Python | `cinyuverse_plugin.isolated` |
| Rust | `cinyuverse-plugin-sdk` feature `isolated` |
...
**Windows**：`CreateAppContainerProfile("cinyuverse.plugin.isolated.<pluginId>")`。无 grant 时不加 `INTERNET_CLIENT`。文件 ACE 同路径规则。Job：`KILL_ON_JOB_CLOSE` + 256 MiB。
...
v1 **不**运营公共 HTTPS index，也没有 `plugins.cinyuverse.dev` 或等价官方目录。本列车不把公开托管 index 列为工作项。

1. Host 发行物带 `plugins/index/official.v1.json` + `official.v1.json.minisig`，用 **Cinyuverse 发行 Ed25519 公钥**（内置于 Host，与 updater 同一密钥族或独立 `MARKETPLACE_ROOT_PUBKEY` 常量）。这是随安装包分发的静态文件，不是网络服务。
2. index schema：`packages[]` = publisher、id、name、summary、class、version、digest、sig（publisher 对 digest 的签）、updatedAt。随发行物的 index 只列 `publisher=cinyuverse` 的 builtin 镜像（可选）。
...
8. `cinyuverse-plugin pack` 用作者本机 publisher 私钥写 `signature.json`。密钥管理：用户文件，不是 device token。
...
| `CINYUVERSE_PLUGIN_DEV_TOKEN` 产品路径 | P24 |
| `--template agent` | P25 |
...
- **官方 MCP 改写成包内 session.mjs**：否决。执行权威与 `cinyuverse-mcp` 二进制已存在；重写会回归会话工具。选 `hostFamilyBinary`。
...
| Workflow Creator 文件 Tab / Studio | P19 同提交注册 renderer | `bundled_workflow_creator` + 打开 `.cinyuverse-workflow.json` |
...
发行说明：官方 MCP 改 Registry；旧会话需 rebind；开发用 `cinyuverse plugin toolchain`；Marketplace Full Trust 文案；未 TOFU 第三方只能 Isolated。
...
- `assets/plugins/*/.cinyuverse-plugin/plugin.json`
...
- **影响**：`crates/plugin-sdk`（cinyuverse-plugin-sdk）；WorkerHost native；CLI cargo build
- **依赖**：P03
...
- **影响**：`plugin_control.rs`；server 鉴权 `cinyuverse.plugin-mcp`
- **依赖**：P09
...
- **依赖**：P12、**P16**、P17
- **说明**：§7.2–7.4。`injected_stdio_servers` 含 workflow；`extra_stdio_servers` 永久 `[]`。HTTP companion 验 scoped token + featureMap。Cutover：反馈开关、委派、单次 workflow 注入（manager 列表里 `cinyuverse-workflow-mcp` 恰好一条）。无 JS session/workflow MCP 物化。
...
### PR-P23a — ADR-0054 安装树 + `cinyuverse plugin toolchain`

- **影响**：打包脚本；Desktop/Server 布局；`cinyuverse-plugin` sibling；文档 ADR-0054 补遗
- **依赖**：P01
...
- **影响**：Core `plugin.dev.grant_for_cwd`；`plugin_dev_grants`；会话事件 `plugin.dev.link`；MCP `plugin_dev_link_status`；CLI 只在 Host 调 Core；删 token 复制 UI 与 `CINYUVERSE_PLUGIN_DEV_TOKEN`
- **依赖**：P17、P23b
