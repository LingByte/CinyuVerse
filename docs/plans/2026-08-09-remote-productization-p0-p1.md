# Cinyuverse 远程产品化 P0/P1 改进计划

**状态：** 决策完成，尚未实施。

**日期：** 2026-08-09。

**对照基线：** Cinyuverse `e13d2a4366e2d03f2abc4e523be91cfd626792f9`；Codeg
...
**目标：** 保留 Cinyuverse 已有的 Application Core、版本化 Remote Protocol、持久事件
...
P0 完成后，用户可以在桌面 Cinyuverse 中保存多个 Server Profile，通过一次性配对建立长期
设备信任，在独立 Server-bound window 中连接桌面或 Headless Cinyuverse Host，并完成以下
...
1. `cinyuverse-server` 有可验证的 Docker/Compose 与跨平台独立发行物；
2. Android 原生伴随端可以连接在线桌面或 Headless Host，查看、对话、审批和只读检查
   结果；
3. 桌面 Conversation 成为可拖拽、可分组、可持久恢复的一等 Dockview panel。

P0/P1 均不把 Cinyuverse 变成云服务、多用户协作平台或远程 ACP endpoint。Cinyuverse Host 仍在
...
| Headless Host      | `crates/server`/`cinyuverse-server` 提供 HTTP、WebSocket、配对、撤销、离线缓存与静态 Web UI                                | 缺少正式容器、签名发行物和完整部署生命周期                                  |
...
| Codeg 已有或更成熟的部分                                 | Cinyuverse 当前状态                                                        | Cinyuverse 采用方式                                                                              |
| -------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
...
Cinyuverse 不复制 Codeg 的明文 Profile token、大型路由业务边界或宽松网络默认值。Cinyuverse 已有
...
- Cinyuverse Host 可以是桌面应用或 Headless Server；同一数据目录同一时刻只有一个 Host。
...
- Compose 示例包含只读 image、持久数据卷、健康检查、graceful stop、资源限制示例与升级
  前备份步骤；
- Windows x64、macOS arm64/x64、Linux amd64/arm64 的独立 `cinyuverse-server`；
...
- FCM、第三方推送、Cinyuverse 云中继或应用被系统彻底停止后的保证送达；
...
- `CONTEXT.md` 维护 Server owner、Cinyuverse Host、Server Profile、Server-bound window、Remote
...
- Kubernetes、多节点、HA、Cinyuverse 托管云、内置公网 TLS；
- FCM/第三方/Cinyuverse 云推送；
- iOS 客户端；
