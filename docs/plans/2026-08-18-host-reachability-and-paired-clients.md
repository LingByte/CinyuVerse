...
产品仓：`~/Projects/cinyuverse-remote-android`。
...
本机控制台（Host 进程上的运维面）
        ...
        │  出示配对邀请
        ▼
   Cinyuverse Host ── Remote Protocol ──┬── Companion（Android，随后 iOS）
                                   ├── Workstation（其它电脑 Cinyuverse.app）
...
Cinyuverse 只吸收「多档案 + 扫一下进 Host」，并补上他们缺的：短时邀请、Host 身份、
...
- 能探则探；探不到允许保存，状态为未验证。
- Cinyuverse 不代配官方 App / `cloudflared`。
...
- Cinyuverse.app：粘贴邀请为主，有相机可扫。打开 **Server-bound window** 绑该
  Host 身份。
...
3. 邀请 DTO 与 `cinyuverse-pairing:1` payload；生成端不再出「无 origin 的配对码」
...
钉死的 FRP 发行物走与 Agent 安装相同的校验和策略。不引入 Cinyuverse 云中继。
...
- Cinyuverse 官方 frps / Tailscale tailnet / 云中继。
- 默认 Funnel 或未确认的公网暴露。
