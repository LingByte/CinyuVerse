...
**目标：** 在不降低 V1 兼容性、不破坏既有 Conversation 历史的前提下，为 Cinyuverse
...
- 由 Cinyuverse 发起 `auth/login`、`auth/logout`、浏览器/设备码登录；
...
| MCP/delegation  | Session 注入 builder、`cinyuverse-mcp`、Broker                     | 保留 token/路径/生命周期策略                   | 版本化 descriptor；V2 仅 stdio/HTTP 和显式 type     |
...
- replay 顺序保存为 Agent session order，但不会重写 Cinyuverse event sequence；
...
- [ ] Cinyuverse 不因 V2 authMethods 增加登录/注销动作。
...
本计划的“完成”是：Cinyuverse 已拥有可维护、可关闭、通过真实 Agent 验证的 V2 支持，V1
