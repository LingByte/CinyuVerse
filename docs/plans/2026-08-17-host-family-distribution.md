...
展开 scopes；`cinyuverse-mcp` 只在 `cinyuverse.collaboration` 启用后注入。

- [x] ADR-0054 / ADR-0055 与本计划
- [x] `DevicePermissionPreset` 与配对展开
- [x] `cinyuverse.collaboration` builtin 插件
- [x] 官方 MCP 门接入桌面与 `cinyuverse-server` injector
- [x] `scripts/package-host-family.js` 与无身份证书的 Host 家族 workflow
- [x] CONTEXT / README / headless 部署文档

## P1

- [x] Chat inbound 由 Host 启动（`server::start_chat_inbound`），`cinyuverse-server` 可收 Telegram
- [x] `cinyuverse.workflow-creator` builtin，启用后注入 `cinyuverse-workflow-mcp`
...
- [x] `npx cinyuverse` 下载 GitHub Host family tarball，先验 sidecar SHA-256 和内层 `SHA256SUMS`，再启动 `cinyuverse-server`
...
# desktop injector
cargo test -p cinyuverse companion_injection
