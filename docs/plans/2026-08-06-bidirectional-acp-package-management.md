...
- [x] 为「内置 Agent 更新目标 = Profile 锁版本」与「`acp_adapter` 被
  ...
  - RED：无测试覆盖当前行为。
  - GREEN：仅增加测试，不改变产品行为。
  - Verify：`cargo test -p cinyuverse install_version_override` /
      `cargo test -p cinyuverse builtin_update_target`.
...
- [x] `resolve_install_plan` 的 BuiltInProfile 分支：存在 fresh snapshot 且
  ...
  - Verify：`cargo test -p cinyuverse builtin_update_uses_registry`.
- [x] 组件版本组合一致性：agent_runtime 与 acp_adapter 目标必须来自同一
      snapshot，禁止混搭。
  - RED：构造 snapshot 内 runtime 与 adapter 版本不匹配时计划被接受。
  - GREEN：planning 拒绝不一致组合。
  - Verify：`cargo test -p cinyuverse builtin_update_component_pairing`.
- [x] `apply_custom_version_override` 纳入 `acp_adapter`（npx/binary 规则与
  ...
  - Verify：`cargo test -p cinyuverse install_version_override_acp_adapter`.
...
- [ ] 内置 Agent 组件安装到版本锁定私有目录，shim 所有权标记与原子切换保留；
  ...
  - Verify：`cargo test -p cinyuverse managed_install_scoped_dir`.
