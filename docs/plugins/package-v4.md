# Cinyuverse Plugin Product Package v4

## 1. 用户模型

Cinyuverse Plugin 是一个可安装、可启停、可配置的产品功能单元。用户不需要理解这个功能由
...
`.cinyuverse-plugin/content.index.json` 是 CLI 初始化、作者维护、build 严格校验、签名覆盖且由 Host
再次校验的结构化目录：
...
`integrations` 是插件内容/代码与 Cinyuverse 内核扩展点的机器映射，不是用户详情页章节：
...
`cinyuverse-plugin pack` 产生确定性 `.vxp`。发布包包含 README、config 初始值、contents、depends、
dist 和 `.cinyuverse-plugin` 元数据，不包含作者源码、source map、developer-link、绝对路径或时间戳。
...
Cinyuverse Office 必须只依赖公开包契约和 SDK。它的 README summary 是列表入口；Word/Excel/
