...
> 执行信任与权限相关步骤已由 ADR-0048 取代。当前实现为 Full Trust；本文中 sandbox、grant、
...
- ADR-0046、v4 schema、canonical protocol/error catalog；
...
- native Agent plugin projection 与 Cinyuverse Plugin inventory 分离。
...
- core 搜索到 `cinyuverse.office` 是否只在 fixture/migration/docs？产品分派中必须为零。
