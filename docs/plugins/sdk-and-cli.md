...
| Package                      | 用途                                                      |
| ---------------------------- | --------------------------------------------------------- |
| `@cinyuverse/plugin-sdk/worker`   | Worker lifecycle、handler 注册与 Capability Broker client |
| `@cinyuverse/plugin-sdk/app`      | Full Trust App bridge 与 surface lifecycle                |
| `@cinyuverse/plugin-sdk/testing`  | generation、Worker 与 App harness                         |
| `@cinyuverse/plugin-sdk/protocol` | wire DTO、JSON value 与 context 类型                      |
| `@cinyuverse/plugin-cli`          | build、validate、test、linked install、dev 与确定性 pack  |
...
当前 CLI 命令与预期结果（以 `cinyuverse-plugin --help` 为准）：

```text
cinyuverse-plugin init [dir] [--publisher id] [--template full|app|agent]
cinyuverse-plugin validate [dir] [--json]
cinyuverse-plugin build [dir]
cinyuverse-plugin test [dir]
cinyuverse-plugin dev [dir] --host <loopback-url> --token <dev-token>
cinyuverse-plugin install --link [dir] --host <loopback-url> --token <dev-token>
cinyuverse-plugin uninstall [dir] [--delete-data] --host <loopback-url> --token <dev-token>
cinyuverse-plugin pack [dir] [--output file.vxp]
cinyuverse-plugin doctor [dir] --host <loopback-url> --token <dev-token>
```

普通 `.vxp` 产品安装仍由 Cinyuverse UI 完成。registry publish、签名和独立
...
在 Cinyuverse monorepo 外验证尚未发布的 SDK 时，按 Skill 用 locator 输出替换本地 SDK dependency，
...
- App reload 仍经过 generation lifecycle，不直接注入 Cinyuverse React tree。
...
```ts
import { createWorkerHarness } from '@cinyuverse/plugin-sdk/testing';
...
projection 仍需要 Cinyuverse integration suite，不能由 fake green 代替。

## 8. Cinyuverse 插件开发指南 Skill 的契约
...
4. 使用 `cinyuverse plugin init/validate/test/build`，不手写生成 metadata；
