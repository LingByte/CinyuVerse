# Cinyuverse Office reference plugin

Cinyuverse Office 是公共 Plugin v4 契约的首个 reference package。源码是 git 子仓库
`assets/plugins/office`（`https://github.com/LingByte/cinyuverse-plugin-office`）。它必须仅使用公开 SDK、
integration、Capability Broker 和 Runtime dependency；Cinyuverse 核心不得认识 Office 插件 ID、
...
Office 与所有 Cinyuverse 产品插件一样采用 Full Trust：启用后 Worker、App 与 OfficeCLI 拥有当前用户
