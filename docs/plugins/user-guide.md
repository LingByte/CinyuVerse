...
Cinyuverse 自己的会话日志、Agent 连接、工作区隔离和窗口布局仍由 Host 负责。插件往这些面上挂能力，不替代它们。
...
### `cinyuverse plugin add`

本机已安装 Desktop 或 `cinyuverse-server` 时，CLI 会发现 Host 并写入同一套控制面。Host 没在跑时，发布包进入 `~/.cinyuverse/imports`，链接开发目录进入 `~/.cinyuverse/imports/links.jsonl`，下次启动再导入。

```bash
cinyuverse plugin add --web https://github.com/LingByte/cinyuverse-plugin-office#v1.0.0 -y
cinyuverse plugin add --web github:LingByte/cinyuverse-plugin-office#v1.0.0 -y
cinyuverse plugin add --profile ~/plugins/search.vxp
cinyuverse plugin add --dev ~/Projects/office
cinyuverse plugin list
cinyuverse plugin update acme.search --ref v1.3.0
cinyuverse plugin remove acme.search
cinyuverse plugin remove acme.search --delete-data
...
桌面且具备 `plugin.write` 时，目录页可以「添加插件」，也可以把 `.vxp` 拖到页面上。只接受带 `.cinyuverse-plugin/plugin.json` 并且通过校验的包。
...
`cinyuverse plugin add --dev` 与 `cinyuverse-plugin install --link` 都把同一份源目录登记为 linked development。Cinyuverse 跟着那个目录走，不会删你的源码。
...
Workflow Creator 启用后，可以打开 `*.cinyuverse-workflow.json` 进编辑页，也可以让 Agent 走它的 Skill 和 MCP。源文件仍是创作真相。发布才会生成不可变版本。Automation 钉在某个已发布版本上，不会跟着草稿走。
...
```bash
cinyuverse plugin remove <插件ID>
cinyuverse plugin remove <插件ID> --delete-data
```
...
链接开发插件卸的是 Cinyuverse 这一侧的引用。你的开发目录还在。
