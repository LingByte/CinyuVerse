...
把 Codex/Claude Code 原生插件的全部能力自动翻译成 Cinyuverse App contribution。
...
- 严格 v2 manifest 与宽容 v3 manifest 同时描述 `cinyuverse.office`；
...
Worker 与 Runtime 永远运行在拥有数据目录的 Cinyuverse Host。远程客户端不能把 Host 插件下载
...
ADR-0048 将插件执行模型改为 Full Trust。Broker 是 Runtime、Artifact、conversation 与 app lifecycle
...
首个实现使用 Cinyuverse 管理的 JavaScript runtime；protocol 保持语言无关，使其他 Worker adapter
...
插件提供 descriptor，Cinyuverse 使用自身组件和 design tokens 渲染。v4 第一批稳定 surface：
...
- package 代码与 Cinyuverse 运行在同一用户信任级别；
...
Plugin 模块服务于 Cinyuverse Product Plugin，保留全局设置侧栏并采用“单列目录 → 独立详情”结构：
...
列出 Cinyuverse Package 的 App/Runtime 生命周期；若多格式 source 与 Cinyuverse Package 相关联，可以
显示可追溯链接，但仍是两个独立启停状态。
...
9. Native Agent plugin 与 Cinyuverse Product Plugin 保持独立 lifecycle，不互相替代启停状态。
