...
查工具链路径执行 `cinyuverse-plugin toolchain`（仓库里是 `node packages/plugin-cli/dist/cli.js toolchain`）。它会打印 Host 版本、CLI、contract、JS / Python / Rust SDK 路径和模板名。没有 `cinyuverse plugin toolchain`。链接正在跑的 Host 用 `cinyuverse plugin add --dev .`。

## 你在开发什么

你在做一个产品包，不是再搭一套应用运行时。框架是已经打开的 Cinyuverse。包有一个身份、一份 README、一份根配置、一份 `contents/`，以及一组写死的 integrations。
...
`.cinyuverse-plugin/content.index.json` 由作者维护、`build` 严查、Host 再验。UI 不扫描磁盘猜内容。
...
```json
{
  "$schema": "https://schemas.cinyuverse.dev/plugin/v4/plugin.schema.json",
  "manifestVersion": 4,
  "apiVersion": "1.0",
  "id": "notes",
  "publisher": "you",
  ...
  "name": "Notes",
  "readme": "README.md",
  "engines": { "cinyuverse": ">=0.1.3 <1.0.0", "pluginSdk": "^1.0.0" },
  "content": {
    "root": "contents",
    "index": ".cinyuverse-plugin/content.index.json"
  },
  "config": {
    "schema": {
      "type": "object",
...
```bash
cinyuverse-plugin validate
cinyuverse-plugin validate --json
cinyuverse-plugin build
cinyuverse-plugin test
cinyuverse-plugin test --host
cinyuverse-plugin install --link .
cinyuverse plugin add --dev .
cinyuverse-plugin doctor
cinyuverse-plugin pack
cinyuverse-plugin pack --output dist/notes.vxp
cinyuverse-plugin uninstall
cinyuverse-plugin uninstall --delete-data
```

`cinyuverse-plugin install` 只支持 `--link`。产品 CLI：`cinyuverse plugin add --dev .` 会 build、validate、导入为 developer link，并尝试启用。源码变化后重新 build；Host 按 digest 发布候选代。

普通用户装发布物走 `cinyuverse plugin add --profile file.vxp`、`--web <git>#tag`、GitHub Release `.vxp`（带 SHA-256 时校验）或桌面拖入 `.vxp`。`cinyuverse plugin list` / `update` / `remove` / `gc-runtimes` 操作同一 Host catalog。`cinyuverse plugin test --host` 与 `cinyuverse-plugin test --host` 对着真 Host 走装、Skill 热更新、卸载。

`dev` 会先 build，再 link，再监视源码。digest 变了才重载。重载走候选代，失败则上一完整代仍对外可见。

`pack` 产出确定性 `.vxp`，并打印 `sha256:` 摘要。发布物带 README、config 初值、contents、depends、dist 和 `.cinyuverse-plugin` 元数据。不带 `runtime/` 源码、source map、`.git`、`node_modules`、开发链接文件。
...
5. `build`，看 `dist/`。
6. `validate` 和 `test`。
7. 打开 Desktop 或 `cinyuverse serve`，执行 `cinyuverse plugin add --dev .`。
8. 在「设置 → 插件」里启用，走一遍用户能看见的路径。
9. 要分发就 `pack`，用 `cinyuverse plugin add --profile file.vxp` 或把 `.vxp` 拖进去。
...
- `CINYUVERSE_PLUGIN_API_VERSION` 为 `"1.0"`
- `CINYUVERSE_PLUGIN_PROTOCOL_VERSION` 为 `"1.1"`

### definePluginWorker

```ts
import { definePluginWorker } from '@cinyuverse/plugin-sdk/worker';
...
```ts
import { createWorkerHarness, createGenerationHarness } from '@cinyuverse/plugin-sdk/testing';
...
```ts
import { runStdioPluginWorker } from '@cinyuverse/plugin-sdk/stdio';
...
包名 `cinyuverse-plugin`，源码在 `sdk/python`。需要 Python 3.11+。Host 锁的 Isolated/托管解释器是 CPython 3.12.11（python-build-standalone）。

```python
from cinyuverse_plugin import define_plugin_worker, run_stdio_plugin_worker
...
```python
from cinyuverse_plugin import create_worker_harness, create_generation_harness, MemoryHostClient
```

## Rust SDK

crate 名 `cinyuverse-plugin-sdk`，路径 `crates/plugin-sdk`。MSRV 1.85。stdio 用 tokio current-thread。

```rust
use cinyuverse_plugin_sdk::{
...
更新失败时，界面和 doctor 应仍指向上一完整代。不要靠杀掉 Cinyuverse 当回滚。
...
```bash
cinyuverse-plugin pack --output notes.vxp
```
...
Agent 原生插件和 Cinyuverse 产品包可以同目录共存，执行权和信任仍分开。
