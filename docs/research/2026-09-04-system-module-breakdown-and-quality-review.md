...
Cinyuverse 是一个 local-first 的 Tauri v2 桌面应用，用于编排多个 AI coding agent（Claude Code、Codex、OpenCode 等），核心能力包括 git-worktree 隔离工作区、事件溯源会话、多智能体委派、插件平台、工作流/自动化、远程访问（Headless Server + 配对设备）。
...
| `crates/cinyuverse-mcp` | 1.4k 行 | 委派 companion MCP 独立二进制（刻意只依赖 delegation-proto） |
...
| `crates/cinyuverse-workflow-mcp` | 72 行 | 内嵌 Node MCP 脚本 launcher |
...
2. **依赖方向经过刻意治理**：`conversations` 从 `db` 拆出打破反向依赖；`cinyuverse-mcp`/`review` 二进制刻意轻依赖；事件类型（agents）/折叠（conversations）/SQL（db）三分。
