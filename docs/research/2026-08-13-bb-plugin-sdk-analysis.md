...
5. **它统一了 App + Agent，但没有声明式外部 Runtime 模型。** manifest 没有
   ...
   程序，但宿主无法仅凭 manifest 安装、锁定、诊断或卸载它。因此 bb 是 Cinyuverse
...
执行时注册。对 Cinyuverse 而言，这意味着若希望安装确认页在执行第三方代码前就能准确展示
...
其中 `fileOpener` 最接近 Cinyuverse Office 预览问题：插件可为扩展名注册 viewer/editor，用户
...
隔离。对 Cinyuverse 这类本地 IDE/Agent 宿主，若开放广泛第三方生态，原样采用 bb 的全信任
...
2. **不要只在代码执行后才知道贡献。** Cinyuverse 的 install review、远程 Host 管理和
...
3. **不要遗漏 Runtime requirement。** bb 对普通 npm code dependency 管理很好，但没有
   ...
   这正是 Cinyuverse 现行 Plugin control plane 可以保留并深化的差异化能力。
