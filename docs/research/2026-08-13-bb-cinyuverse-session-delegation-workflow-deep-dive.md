# BB 与 Cinyuverse：会话、委派与工作流深度对比

> 日期：2026-08-13
> 基线：BB `aefe3ea49ef7d7236905893a5feaa5986a929872`；Cinyuverse 当前工作区
> 续篇：[`2026-08-12-bb-agent-cinyuverse-comparison.md`](./2026-08-12-bb-agent-cinyuverse-comparison.md)

## 结论先行

Cinyuverse 在这两个方向上都不是“从零开始”：它已经拥有可靠的 Conversation/Turn
事件语义、持久化委派子会话、取消传播、自动化租约、版本证据和每次运行的工作树隔离。
真正的差距在控制面，而不是执行底座：

1. **会话与委派**：Cinyuverse 能让模型异步委派一个一次性任务，但尚未把
   `spawn / wait / tell / steer / queue / cancel` 做成统一、持久、可由用户、CLI、
   SDK、MCP 和工作流共同调用的会话原语。
2. **工作流**：Cinyuverse 的 Automation 是“可靠地定时启动一个 Turn”；BB Workflow
...
Cinyuverse 的 `crates/delegation/src/steering.rs` 负责 companion 的反馈、提问和 session
...
Cinyuverse Automation 当前做得好的部分应保留：版本化 `TurnLaunchSpec`、owner lease、原子
...
| 维度       | BB Workflow                                            | Cinyuverse Automation               | 应补能力                                                     |
| ---------- | ------------------------------------------------------ | ------------------------------ | ------------------------------------------------------------ |
...
Cinyuverse 应要求步骤声明副作用等级：`read_only`、`idempotent`、`mutating_unknown`。只有在
...
Cinyuverse 最值得保留的是它已经统一的 Conversation/Turn 事实、终态、版本证据和安全隔离；
