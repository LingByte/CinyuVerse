...
- `cargo test -p application --test command_contract` 曾被占用中的 `cargo build --bin cinyuverse` 文件锁打断。随后 `cargo test -p server --lib` 已跑通 `conversation_detail_uses_open_path_snake_case_keys` 与 `create_project_session_payload_accepts_*`（`named-defect-retrace.log`）。**不以契约扫描绿作为完成门。**
