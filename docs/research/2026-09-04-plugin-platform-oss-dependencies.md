...
对象/类实例，跨插件服务调用就是方法调用。Cinyuverse 的既定模型是**每插件一个独立 Node
...
- **每个 Worker 内各跑一棵 Cordis 树**：进程边界仍在，跨插件服务依旧要走宿主 RPC，
  ...
  reload）与 Cinyuverse 已有的激活代（Activation Generation）模型重叠冲突。
- **只用它的类型与调度器、把服务代理成 RPC stub**：等于自己重写 Cordis 的核心
  （服务解析器必须换成跨进程实现），引依赖已无意义。

**但 Phase E 需要的语义，Cordis 是现成最好的参照。** 它验证了一套与 Cinyuverse 词表
...
在 Cordis 里就是「插件注册一个新服务 + 其他插件 inject 它」，映射到 Cinyuverse 即
...
Tauri webview 场景（非 CDN、本地文件）不构成障碍：Cinyuverse 宿主已有本地 HTTP 服务
...
- **宿主侧**：Cinyuverse 前端是 Vite 构建。纯 runtime 用法不要求宿主改造构建管线——
  `createInstance` 后 `registerShared` 把宿主的 react / react-dom /
  `@cinyuverse/plugin-sdk`（以及受控暴露的 dockview API）注册为 shared singleton，再
...
- **插件侧**：`@cinyuverse/plugin-sdk` 的构建模板（`cinyuverse-plugin` CLI）加上
...
Cinyuverse 的面板由 dockview 布局驱动、无路由概念；它不提供共享依赖协商（靠 import map
外置）。维护状态使其直接出局。
...
**匹配分析。** 价值主张与 Cinyuverse 诉求**正好相反**：qiankun 的全部复杂度花在把子应用
...
**匹配分析。** 采用 Piral 意味着 Cinyuverse 前端壳重写为 Piral instance、插件改写为
pilet、市场对接其 feed 协议——三者 Cinyuverse 都已有自建等价物（dockview 壳、v4 包、
...
`<script type="importmap">`（或 es-module-shims 垫片）把 react、`@cinyuverse/plugin-sdk`
...
1. **架构互斥。** Theia 的前后端各持一个**全局 DI 容器**，扩展在构建期被收进同一
   应用产物（CLI 收集模块统一 webpack 打包）——这是「编译期组装的单体」，与 Cinyuverse
   「运行期安装、独立 Worker 进程、激活代原子切换」的动态插件模型不同层。采用
   Theia 等于放弃 Rust 宿主（`Deployment` trait、事件溯源会话核心、ACP runtime）
   换一个 Node 后端，重写成本覆盖全部三层。
2. **Electron 绑定。** Theia 桌面形态是 Electron；Cinyuverse 是 Tauri + Rust。
3. **产品面不重叠。** Cinyuverse 的核心域（Conversation 事件溯源、Workspace worktree、
   Agent 编排）在 Theia 里没有对应物，能复用的只有 workbench 表层，而这层 Cinyuverse
   已由 dockview 承担。

**可借鉴的贡献点模型设计**（落 Phase B/E 的扩展点注册表设计）：

- **「接口即扩展点」+ 集中收集**：核心定义贡献接口，注册表通过 ContributionProvider
  统一收集与迭代——对应 Cinyuverse 的做法是 manifest 声明贡献 + Rust 控制面注册表按
...
- **贡献接口稳定、rebind 是逃生舱**：Theia 实践明确「扩展走贡献点，rebind 视为
  ...
  布道内容）。映射到 Cinyuverse：稳定面收敛（ADR-0067 Phase 4）之外不给插件提供
  「覆盖宿主实现」的通道。
...
**匹配分析。** 它与 dockview-react 是同位竞争（docking/布局层），Cinyuverse 已深度使用
...
文件级 copyleft，作为未修改的 crate 依赖对 Cinyuverse 无传染义务（修改其源文件才需
开源该文件）。
...
- **与既定模型合缝。** `Dependencies::Unavailable` + 自定义不可用原因类型 `M`
  可以直接表达「平台不支持」「digest 缺失」「来源锁不允许」等 Cinyuverse 特有约束；
...
- **semver 解析配套用 [`semver` crate](https://crates.io/crates/semver)**（dtolnay，
  ...
  区间解析、`engines.cinyuverse` 校验用它；注意它实现的是 **cargo 语义**的区间（`^`
...
- **[extism](https://github.com/extism/extism)**（BSD-3，v1.21.0 / 2026-03-26，
  ...
  Cinyuverse 已在 ADR-0048 决定 Full Trust + Node Worker + TS SDK：采用 extism 等于
...
**匹配分析。** 与自建市场相比是净负收益：Cinyuverse 的包格式是 `.vxp`（非 VSIX）、
目录 API 已按 ADR-0067 Phase 2 定义为六类只读接口 + 四元组身份 + digest + 来源锁，
官网（cinyuverse-site）已承载发布流程。引入 Open VSX 意味着运维一套
...
蓝本，二者都应转写进 Cinyuverse 自己的 manifest + Rust 控制面，而不是作为运行时依赖
进入 Worker 或前端。
