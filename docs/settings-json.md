...
Cinyuverse-owned settings are stored in the user-editable
`~/.cinyuverse/settings.json` document. The application reads the relevant section
...
Cinyuverse writes the document atomically and preserves sibling sections when one
page saves.
...
receive `CINYUVERSE_PROJECT_ID`, `CINYUVERSE_WORKSPACE_ID`, and `CINYUVERSE_WORKTREE_PATH` in
...
files owned by each agent; Cinyuverse does not duplicate or overwrite those
