...
- Verification: `cargo check -p cinyuverse`; `pnpm run check`; `pnpm run lint`.
...
- Verification: `cargo test -p cinyuverse provider_runtime --lib`; `cargo fmt --check`; `rg -n "include!" src-tauri/src/commands/provider_runtime`; `cargo check -p cinyuverse`; `pnpm run check`; `pnpm run lint`.
...
- Verification: `cargo test -p utils command_output_detail --lib`; `cargo test -p utils process --lib`; `cargo fmt --check`; `cargo check -p cinyuverse`; `pnpm run check`; `pnpm run lint`.
...
- Behavior lock: red `cargo test -p cinyuverse preview_extraction_failure_message --lib` first failed on the missing helper; after implementation, file-preview tests prove stderr detail is preferred, stdout detail is used when stderr is empty, and empty output keeps the generic preview failure message.
- Verification: `cargo test -p cinyuverse preview_extraction_failure_message --lib`; `cargo test -p cinyuverse file_tree --lib`; `cargo test -p utils command_output_detail --lib`; `cargo check -p cinyuverse`; `cargo fmt --check`; `pnpm run check`; `pnpm run lint`.
...
- Behavior lock: pre-edit and post-edit `cargo test -p cinyuverse parses_opencode_models_from_stdout_lines --lib` passed with the same expected parsed model list.
- Verification: `cargo test -p cinyuverse parses_opencode_models_from_stdout_lines --lib`; `cargo check -p cinyuverse`; `cargo fmt --check`; `pnpm run check`; `pnpm run lint`; `git diff --check`.
...
- Behavior lock: red `cargo test -p cinyuverse mcp_servers --lib` first failed with an empty-path subtraction overflow panic; after implementation, tests prove empty paths return an MCP path error and non-object intermediate values are replaced before writing server maps.
- Verification: `cargo test -p cinyuverse mcp_servers --lib`; `cargo check -p cinyuverse`; `cargo fmt --check`; `pnpm run check`; `pnpm run lint`; `git diff --check`.
