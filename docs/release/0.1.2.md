# Cinyuverse Desktop 0.1.2

Cinyuverse 0.1.2 完成 IAE（Integrated Agent Environment）产品方向升级，将 Agent 接入、并行开发环境和 Vibe Coding 配套工具集中到统一桌面平台。
...
发布包可能尚未完成 Apple 公证。系统提示“App 已损坏”时，请先确认安装包来自 Cinyuverse 官方 Releases，然后执行：

```bash
xattr -dr com.apple.quarantine /Applications/Cinyuverse.app
```

权限不足时可以在同一命令前添加 `sudo`。该命令只需要作用于 Cinyuverse 应用路径，无需关闭系统全局安全检查。
