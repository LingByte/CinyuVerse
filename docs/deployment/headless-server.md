...
`cinyuverse-server` is a headless composition root over the same Application Core,
SQLite data directory, Agent runtime, Delegation companion, Plugin/Artifact
services, and Automation Engine used by desktop Cinyuverse. It does not load Tauri.

## Distribution status

P0 ships a Host family directory: `cinyuverse-server`, sibling `cinyuverse-mcp`,
`cinyuverse-workflow-mcp`, production `web/`, and `plugins/bundled/`. Assemble it with:

```bash
node scripts/package-host-family.js \
  --server target/release/cinyuverse-server \
  --mcp target/release/cinyuverse-mcp \
  --workflow-mcp target/release/cinyuverse-workflow-mcp \
  --web frontend/dist \
...
`npx cinyuverse`, `install.sh`, and `install.ps1` download
`Cinyuverse-${VERSION}-{linux-x86_64,linux-aarch64,darwin-aarch64,windows-x86_64,windows-aarch64}-server.tar.gz`
from the matching GitHub Release, verify the sidecar `.sha256` and the inner
`SHA256SUMS`, then run `cinyuverse-server` with `CINYUVERSE_STATIC_ROOT` set to `web/`.
...
`npx cinyuverse serve --local`. Replace the token with `npx cinyuverse serve --rotate-token`.
...
Plain `npx cinyuverse` stays on loopback. Docker Compose still publishes
...
```bash
npx cinyuverse list
npx cinyuverse list --refresh
npx cinyuverse install claude_code --yes
```
...
the confirmation prompt. The remote control command `npx cinyuverse agent list`
...
`cinyuverse-server`, `cinyuverse-mcp`, and `web/`. See `server::apply_host_upgrade`.
...
- Publish signed, checksummed standalone `cinyuverse-server` binaries for Windows x64,
...
|---|---|---|
| `CINYUVERSE_DATA_DIR` | platform Cinyuverse data directory | One desktop/Server owner at a time runs Automation for this directory. |
| `CINYUVERSE_SERVER_LISTEN` | `127.0.0.1:17891` | Non-loopback is rejected unless LAN exposure is acknowledged. |
| `CINYUVERSE_SERVER_ALLOW_LAN` | unset | Set to `1` only with an external TLS/auth boundary and reviewed network policy. |
| `CINYUVERSE_SERVER_TOKEN` | generated once | Prefer generated credentials. Supplied values must be at least 32 bytes with at least 12 distinct byte values; use a secret manager, never a URL or command argument. |
| `CINYUVERSE_STATIC_ROOT` | unset | Point to the production `frontend/dist` tree. |
| `CINYUVERSE_SERVER_ALLOWED_ORIGINS` | empty | Comma-separated exact browser origins; same-origin requests need no entry. |
