...
Cinyuverse 桌面端集成了 `tauri-plugin-updater`：在 **设置 → 系统 → 应用更新** 检查更新，
下载并安装签名的发布产物后一键重启。
...
- 更新源与公钥在 [tauri.conf.json](../src-tauri/tauri.conf.json) 的 `plugins.updater`：
  - `endpoints`：默认指向 `https://github.com/LingByte/CinyuVerse/releases/latest/download/latest.json`。
  - `pubkey`：签名验证公钥。
...
1. 生成密钥对（私钥务必保密）：
   ```bash
   pnpm tauri signer generate -w ~/.cinyuverse-updater.key
   ```
2. 把生成的**公钥**内容粘贴到 `tauri.conf.json` 的 `plugins.updater.pubkey`。
3. 构建/发布时提供私钥（作为 CI secret，勿提交）：
   ```bash
   export TAURI_SIGNING_PRIVATE_KEY="$(cat ~/.cinyuverse-updater.key)"
