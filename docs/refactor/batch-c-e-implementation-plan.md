...
3. `chat_channel.rs`（1642 行）投递/WebSocket/持久化并入 `crates/services` 的 `chat_delivery` 模块；密钥迁至 `~/.cinyuverse/.env`（明文、权限 0600、**只收 IM 渠道密钥**；迁移时读旧 JSON 写入 .env 后删旧文件）。按 ADR-0004 有意明文，不改钥匙串。
