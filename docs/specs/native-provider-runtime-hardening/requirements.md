...
2. As a user reopening Cinyuverse, I want provider runtime history and normalized conversation state to remain inspectable, so prior native turns are not limited to in-memory events.
...
4. As a user interrupting a native-provider turn, I want Cinyuverse to stop the whole owned runtime tree, so helper processes do not survive after cancellation.
...
7. WHEN a native turn is interrupted THEN Cinyuverse SHALL terminate the owned process tree or provider-native turn where available, and SHALL mark the associated execution process as killed.
...
- Backend focused tests: `cargo test -p cinyuverse provider_runtime --lib`
...
- Keep existing `cargo test -p cinyuverse provider_runtime --lib` green after each slice.
