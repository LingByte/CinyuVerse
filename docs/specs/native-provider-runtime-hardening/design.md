...
- After fallback policy: `cargo test -p cinyuverse provider_runtime --lib`
- After normalized event adapter introduction: `cargo test -p cinyuverse provider_runtime --lib`
- After bridge runner extraction: `cargo test -p cinyuverse provider_runtime --lib` and `pnpm run backend:check`
- After DB persistence: `pnpm run prepare-db`, `pnpm run prepare-db:check`, `cargo test -p cinyuverse provider_runtime --lib`
