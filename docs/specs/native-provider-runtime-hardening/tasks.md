...
- [x] Task: Add tests proving native provider send-turn does not silently ACP fallback by default.
  ...
  - Verify: `cargo test -p cinyuverse provider_runtime --lib`
...
- [x] Task: Move Claude SDK turn execution to the bridge runner.
  ...
  - Verify: `cargo test -p cinyuverse provider_runtime --lib`; `pnpm run backend:check`
...
- [ ] Task: If Option A, add provider runtime event persistence.
  ...
  - Verify: `pnpm run prepare-db`; `pnpm run prepare-db:check`; `cargo test -p cinyuverse provider_runtime --lib`
...
- [x] Task: If Option B, make live-only raw history explicit.
  ...
  - Verify: `cargo test -p cinyuverse provider_runtime --lib`; `pnpm run generate-types:check` if shared types change
...
- [x] Task: Route bridge interrupts through registry-owned process termination.
  ...
  - Verify: `cargo test -p cinyuverse provider_runtime --lib`; manual desktop interrupt check when implementing visible behavior
...
- [x] Run `cargo test -p cinyuverse provider_runtime --lib`
