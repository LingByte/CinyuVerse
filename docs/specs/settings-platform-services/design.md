...
This change is implemented as a settings-platform expansion inside the existing Cinyuverse Tauri app:
...
- Persistence:
  ...
  - feature-specific settings should use new command modules and either SQLite tables, `app_metadata`-style key/value storage, or Cinyuverse-managed JSON files depending on existing local patterns;
  - secrets use secure token commands when possible.

The implementation uses Cinyuverse's current SQLx/db crate patterns.
...
- Cinyuverse-managed MCP/skills directories.
...
- command prefix,
- event filters,
- event forwarding from Cinyuverse ACP/workspace events.
...
Implement version-control settings using Cinyuverse error/style conventions:
