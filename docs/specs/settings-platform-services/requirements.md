...
1. Work happens in `C:\Users\Administrator\Documents\Projects\Cinyuverse-settings-platform-services` on branch `feature/settings-platform-services`, based on local `master`.
2. Existing Cinyuverse architecture and `DESIGN.md` are the implementation references for settings IA, backup/restore, network proxy, web service, chat channels, and version-control settings.
3. Cinyuverse remains a Tauri desktop app first. Web-service settings should expose a local service and credentials, but the implementation must fit Cinyuverse's existing React Router + Tauri command architecture.
...
Converge Cinyuverse settings into a broader platform settings surface, matching the Tahoe design system while adding the user-requested settings capabilities:
...
- Export a portable Cinyuverse backup file, default extension `.cinyuversebak`.
- Backup should include:
  - app config,
  - executor profiles,
  - SQLite application data,
  - locally hosted skills/MCP registries under Cinyuverse-managed directories,
...
- Saved proxy should be used by backend HTTP clients where Cinyuverse owns the client.
...
- User can configure IM bot channels using the shared Cinyuverse channel model:
  - Telegram,
  - Lark/Feishu,
...
- Do not migrate Cinyuverse away from its existing React Router architecture.
- Do not replace Cinyuverse's ACP-native agent settings model.
- Do not introduce UI that violates Cinyuverse Tahoe design tokens.
