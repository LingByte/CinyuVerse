...
crates/
  ...
  server/             Axum/static UI/WebSocket/cinyuverse-server binary
...
```json
{
  "$schema": "cinyuverse-plugin/v2",
  "id": "cinyuverse.office.presentation",
  "version": "1.0.0",
...
Composer AgentMention
  ...
  → 序列化为 [&Name](cinyuverse://agent/<stable-kind>)
...
`supports_cinyuverse_companion(feature)`。注入逻辑只读能力，不判断 `AgentKind ==
...
| Delegation tool schema | `src-tauri/src/acp/delegation/tool_schema.json` | 适配 `&Mention` 和 Cinyuverse AgentKind |
| Broker race/cache behavior | `src-tauri/src/acp/delegation/broker.rs` | 行为/测试对齐，保留 Cinyuverse 模块化 crate |
...
任何 copy/adapt 必须满足 Apache-2.0 第 4 节；若 Cinyuverse 发布包尚无第三方 notices，
...
- Packaging smoke：OfficeCLI/`cinyuverse-mcp` sidecar、`cinyuverse-server` 静态资源与跨平台路径。
