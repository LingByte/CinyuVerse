...
- **RED**：对 fake Agent capabilities 构造支持/不支持 `session/new.mcp_servers` 两例，
  断言前者收到 cinyuverse-mcp，后者返回 `delegation_parent_unsupported`；测试不检查
  AgentKind。
...
- **Verify**：`cargo test -p cinyuverse-mcp tool_schema_explains_agent_mentions`
...
- **Verify**：`cargo test -p cinyuverse-mcp && cargo test -p delegation`
...
## M5 — cinyuverse-server、WebTransport 与 Web 功能
...
- **Depends**：T5.3–T5.7。
- **RED**：从生产静态资源启动 `cinyuverse-server`，Playwright 旅程无法完成登录、创建 Turn、
...
**M6 Gate**：远程协议文档足以让独立客户端只依赖 Server，不读取 Cinyuverse SQLite 或本机
