...
1. Cinyuverse 仍以 Tauri 桌面端为首要交付面；Web 使用同一 React UI，移动端本阶段不开发。
2. 用户启用内置 Office 插件或首次执行其动作，视为同意 Cinyuverse 自动安装被明确展示的
...
5. 本阶段多 Agent 只有 LLM-mediated delegation；确定性 Graph/Workflow
   Engineering 另行决策。
6. 同一 Cinyuverse 数据目录可由桌面或 `cinyuverse-server` 打开，但 Automation Engine、
...
把 Cinyuverse 从“桌面会话客户端 + 若干功能入口”扩展为统一能力平台：
...
- **PLG-004**：工具依赖安装到 Cinyuverse 托管的版本目录，保存来源、精确版本、哈希、平台、
  安装时间和当前指针；不得修改用户全局包。
...
- **PLG-009**：插件/工具卸载只删除 Cinyuverse 所有的产物；仍被运行租约引用的版本延迟清理。
...
- **DEL-001**：父 Agent 通过按会话注入的 `cinyuverse-mcp` 暴露
...
- **WEB-003**：独立 `cinyuverse-server` 能初始化与桌面相同的数据库、Application Core、
...
- **WEB-010**：桌面可以选择连接远程 Cinyuverse Server，事件按窗口/Server profile 隔离。
...
1. 用户从内置 Office 插件点击“创建 PPT”，Cinyuverse 在缺少 OfficeCLI 时完成可验证的
