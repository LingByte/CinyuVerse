...
> 状态：**Phase 4 - Implement 进行中** ｜ ✅ M1（DB）+ ✅ M2（broker/listener）+ ✅ M3（companion）｜ **58 测试绿**（db 13 + delegation 32 + proto 3 + cinyuverse-mcp 10）｜ 下一步 M4（接线，核心集成门）｜ 2026-06-14
>
...
- [ ] **T3.1 `cinyuverse-mcp` crate（stdio MCP server 骨架）**
  - Acceptance：新 `[[bin]]` crate；解析 `--parent-connection-id/--socket-path/--token/--parent-pid/--features`；stdin 逐行 dispatch `initialize`/`tools/list`/`tools/call`/`notifications/cancelled`；`tool_schema.json` 定义 5 个工具，agent_type enum 使用 Cinyuverse 序列化名。
  - Verify：CLI 冒烟（管道喂 JSON-RPC：initialize 握手、tools/list 返回 5 工具）+ dispatch 单测。
  - Files：root `Cargo.toml`(member)、`crates/cinyuverse-mcp/{Cargo.toml,src/main.rs,tool_schema.json}`（transport 复用 proto）。
...
- [ ] **T3.2 companion transport client + 取消 + parent-pid 看门狗**
  ...
  - Files：`crates/cinyuverse-mcp/src/{main.rs,client.rs,inflight.rs}`。
...
- [ ] **T4.4 MCP 注入（session/new 的 mcp_servers，仅 ClaudeCode）**
  - Acceptance：`new_acp_session` 在父=ClaudeCode 且功能开启时，向 `NewSessionRequest.mcp_servers` 追加 `McpServerStdio("cinyuverse-mcp", …)`（带 token/socket/features args）；非 ClaudeCode 不受影响；token 在此 mint+注册。
...
- [ ] **T5.1 status/cancel 工具端到端**
  ...
  - Files：`crates/delegation/src/listener.rs`、`crates/cinyuverse-mcp/src/main.rs`（status/cancel 分支）。
...
- [ ] **T6.1 check_user_feedback（队列 + 至少一次投递）**
  ...
  - Files：`crates/delegation/src/{listener.rs,feedback.rs}`、`crates/cinyuverse-mcp/src/main.rs`。
...
- [ ] **T6.3 `--features` 门控端到端**
  ...
  - Files：`crates/agents/src/manager.rs`(注入决策)、`crates/cinyuverse-mcp/src/main.rs`(门控)。
...
- [ ] **T8.1 sidecar 打包**
  - Acceptance：`tauri.conf.json` `externalBin` 注册 `cinyuverse-mcp`；多平台命名 `cinyuverse-mcp-<target-triple>`；dev 期 `CINYUVERSE_MCP_BIN` 覆盖；`locate_cinyuverse_mcp_binary` 三级查找（env→同级→PATH）。
