...
让 Cinyuverse 的**父智能体（v1 限 ClaudeCode）的 LLM 能够自主把自包含子任务委派给独立的子智能体会话**，子任务异步在后台执行，父智能体可并行 fan-out 多个、轮询/等待/取消，并在结果回来后继续推进。同时让父智能体能在运行中**实时接收用户插话（steering）**与**向用户提阻塞式多选问题**。
...
**决定性事实**（已核验，影响整体架构）：Cinyuverse 当时**没有任何「app 自实现工具暴露给 agent LLM」的能力**。前端的 `ask_question`/`feedback_check`/`task_create` 卡片只是按工具名识别 agent 原生工具（见 [NormalizedConversation tools](../../../frontend/src/components/NormalizedConversation/tools)），后端对 MCP 完全被动（只读写用户配置文件，spawn 时零注入）。
...
ClaudeCode LLM ─ ToolUse(delegate_to_agent / get_delegation_status / cancel_delegation
        ...
        │ stdio (MCP)
        ▼
   cinyuverse-mcp（companion 二进制，每次父会话启动一个）
...
crates/cinyuverse-mcp/                   # companion 二进制（最小依赖；依赖 delegation 的 transport 模块）
...
src-tauri/src/delegation/           # 把 broker 接到真实 Cinyuverse（trait 具体实现 + 注入 + 命令）
  mod.rs
  ...
  injection.rs          # 把 cinyuverse-mcp 写进 ClaudeCode 的 MCP 配置文件 + token 注册/吊销
...
>
> 当前 Cinyuverse 在 [manager.rs:951](../../../crates/agents/src/manager.rs#L951) 调用 `NewSessionRequest::new(cwd)` 时尚未填充 `mcp_servers`。无需 cwd 级 `.mcp.json`；`session/new` 的 `mcpServers` 天然按会话隔离，可规避陈旧 token、共享文件并发和 teardown 清理问题。

spawn ClaudeCode 的 `new_acp_session` 路径（[manager.rs:945-962](../../../crates/agents/src/manager.rs#L945)）增强为：

1. 用 `locate_cinyuverse_mcp_binary()` 定位 companion（env 覆盖 `CINYUVERSE_MCP_BIN` → 可执行同级 → PATH）。
...
3. 构造一条 companion server（仅当父 agent 是 ClaudeCode 且至少一项功能开启时）：
   ```rust
   let companion = McpServerStdio::new("cinyuverse-mcp", cinyuverse_mcp_bin)
...
- stop_reason 映射：Cinyuverse `PromptFinished.stop_reason: Option<String>`（`"end_turn"`/`"cancelled"`/…）映射到 `DelegationError`（child_refusal/child_max_tokens/child_empty/…），需建立字符串 → 错误码映射表（见 §11）。
- 缓存常量：每 parent 512MB FIFO；单结果文本截断 256KiB；status 预览 2KiB。

### 4.6 ConnectionSpawner 实现（接 AgentRuntime）

`spawner_impl.rs` 把 `ConnectionSpawner` trait 接到 Cinyuverse：
...
见 §4.2。新增：`crates/delegation/`、`crates/cinyuverse-mcp/`、`src-tauri/src/delegation/`、`frontend/src/features/delegation/`、一条 DB 迁移。改动：`crates/agents/src/manager.rs`（spawn 注入钩子）、`src-tauri/tauri.conf.json`（sidecar）、`ToolCallCard.tsx`（卡片分发）、`shared/types.ts`（生成）。
...
3. ~~**ClaudeCode 配置隔离**~~ ✅ **已解决**：ACP `session/new` 的 `mcp_servers` 参数按会话天然隔离，Cinyuverse 的 `agent-client-protocol` 0.11.1 已支持。无需 cwd 级 `.mcp.json`。见 §4.3。
4. **companion 二进制构建/分发**：新 crate `[[bin]]` + `tauri.conf.json` `externalBin` sidecar；多平台命名（`cinyuverse-mcp-<target-triple>`）与 dev 时 `CINYUVERSE_MCP_BIN` 覆盖路径。
...
3. **companion**：`crates/cinyuverse-mcp` stdio MCP server + tool_schema + transport client；sidecar 打包。
