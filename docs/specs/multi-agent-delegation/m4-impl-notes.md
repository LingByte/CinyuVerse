...
`cargo build -p cinyuverse` 通过 → `cargo test --workspace` → `pnpm tauri dev` 手验：ClaudeCode `delegate_to_agent` → 子 session DB 落行（parent 链对）→ 结果回父 LLM。
