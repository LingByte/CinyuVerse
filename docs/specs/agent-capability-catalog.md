...
Cinyuverse currently obtains session configuration choices through four competing
...
only useful if Cinyuverse can apply the exact advertised value to the agent.
...
- FR-1: Cinyuverse MUST persist one `CapabilityCatalog` snapshot for the active
...
- FR-3: Cinyuverse MUST refresh a catalog in the background after app startup,
...
- FR-5: A catalog whose fingerprint does not match the active agent
  configuration MUST be treated as unavailable for choice selection; Cinyuverse
...
- FR-6: If no matching catalog is available, both UI surfaces MUST render
  ...
  browser storage.
- FR-7: Cinyuverse MUST preserve raw agent model IDs and raw choice values for
...
- FR-9: Fast MUST be represented as a capability, not a hard-coded Codex
  ...
  control, Cinyuverse MUST expose it in the same session-control UI and apply its
  advertised value. When it does not advertise such a control, Cinyuverse MUST NOT
...
Then Cinyuverse writes that exact advertised option/value pair.
...
- OS-1: Maintaining a Cinyuverse-owned global model list. Account eligibility
