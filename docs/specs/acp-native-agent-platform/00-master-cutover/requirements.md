...
Replace Cinyuverse's old agent execution model with a single ACP-native agent platform.
Users should experience agents as installable runtime plugins, while Cinyuverse owns a
...
4. Existing Cinyuverse workspace, Git, file tree, preview, project rail, and settings
...
1. As a user, I want every supported coding agent to appear as an installable
   ...
   not require rebuilding Cinyuverse's core runtime.
...
1. WHEN Cinyuverse starts a live coding-agent session THEN it SHALL create or attach an
...
4. WHEN an agent requests permission THEN Cinyuverse SHALL surface the ACP permission
