...
5. **The Agent owns persistent configuration.** Cinyuverse edits only profile-known
...
- Generic Agents use only Registry-declared Binary/npx/uvx distributions.
  ...
  user confirmation.
- Managed Runtime components are installed in Cinyuverse-owned versioned locations.
  Built-in Runtime may automatically attach a verified external installation;
  Cinyuverse never modifies or removes external components. Built-in Profiles and
  Cinyuverse-managed base Runtime require expected SHA-256; Registry Binary uses
...
- Only Built-in Profiles with known file schemas expose persistent configuration
  fields. Cinyuverse reads latest state, writes user-submitted known fields
