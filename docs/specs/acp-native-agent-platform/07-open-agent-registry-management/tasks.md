...
- [x] E1. Add typed management commands and management events.
  - RED: `cargo test -p cinyuverse agent_management_commands_serialize_snapshots_and_errors`
  ...
  - Verify: `cargo test -p cinyuverse agent_management`; `pnpm run generate-types:check`.
...
- [x] G2. Run integration, visual, migration and real-agent release gates.
  ...
  - Verify:
    ...
    - `cargo test -p api-types && cargo test -p db && cargo test -p agents && cargo test -p cinyuverse`
