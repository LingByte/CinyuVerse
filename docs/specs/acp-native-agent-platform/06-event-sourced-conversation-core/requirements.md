...
1. Cinyuverse conversation history must be independent from each Agent's own transcript
...
4. The current `sessions.id` may be reused as the Cinyuverse conversation id during
   implementation, but its meaning must change: it identifies a Cinyuverse-owned
...
renderable from Cinyuverse-owned data:
...
- Cinyuverse exports a portable conversation bundle containing metadata, bindings,
  turns, events, projections, attachments, checkpoints, and capability snapshots.
- Cinyuverse bundle import restores renderable conversations without requiring any
  Agent transcript file.
- Agent transcript import remains available only as an importer that converts
  external history into Cinyuverse-owned conversations/events.
...
- Always:
  ...
  - treat Cinyuverse event log as canonical;
...
- Removing or corrupting the external Agent transcript file does not remove the
  Cinyuverse conversation timeline.
...
same: Cinyuverse event data, not Agent transcript files, must be canonical.
