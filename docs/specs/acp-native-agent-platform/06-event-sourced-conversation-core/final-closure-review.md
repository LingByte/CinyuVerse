...
The core conversation path has moved to a Cinyuverse-owned event log:
...
- R8 Import and Export: Cinyuverse bundle import/export and explicit transcript-to-
...
cargo test -p cinyuverse conversation_start_turn_maps_agent_blocks_to_input_blocks
cargo test -p cinyuverse conversation_detail_projection_uses_event_log
cargo test -p cinyuverse conversation_event_paging_returns_sequence_cursor
cargo test -p cinyuverse conversation_bundle_export_contains_cinyuverse_tables
cargo test -p cinyuverse conversation_bundle_import_restores_renderable_events
cargo test -p cinyuverse history_import_to_conversation_events
cargo test -p cinyuverse failed_prompt_emits_terminal_event
cargo test -p cinyuverse conversation_checkpoint_file_changes
