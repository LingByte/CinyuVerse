...
```text
Cinyuverse-owned event log and projector
...
conversation_imports
  id
  source = cinyuverse_bundle | agent_transcript
  source_agent
  external_session_id
  bundle_version
  raw_source_path
...
pub enum ConversationEvent {
    ...
    // ⚠️ 2026-07-04 P0-2 已移除：ForkSupportUpdated（从未在生产发射；Cinyuverse 无真 fork）。
...
- if a new ACP session is created for an existing Cinyuverse conversation, persist it
  as a new binding event and keep the existing Cinyuverse timeline intact.
...
```text
cinyuverse-conversation-bundle/
  manifest.json
  conversations.json
  bindings.json
  turns.json
...
Export reads from Cinyuverse tables only. Agent transcript files are not required.

### Import

Cinyuverse bundle import:
