...
2. WHEN an agent is installed or updated THEN Cinyuverse SHALL execute the registry
   distribution plan for Npx, Binary, Uvx, or System command entries.
3. WHEN config/auth is probed THEN Cinyuverse SHALL report agent-specific status
...
5. WHEN history is imported THEN Cinyuverse SHALL parse each agent's native history
