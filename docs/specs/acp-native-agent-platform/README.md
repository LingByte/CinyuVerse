...
Replace Cinyuverse's current agent execution architecture with a single ACP-native
...
- `06-event-sourced-conversation-core/`: breaking refactor that makes Cinyuverse's
...
Cinyuverse conversation history is rebuilt from Cinyuverse-owned events. Agent transcript
