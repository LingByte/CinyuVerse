...
3. Message channels and Web service have persisted configuration and basic
   commands, but are not fully bound to real Cinyuverse conversation events and
   session actions.
...
actions bypass real Cinyuverse conversations. Web service is only a health/status
...
- Bind message channels and Web service to real Cinyuverse conversation/session
...
3. Replace inbound `send_task(...)`:
   - Store a bridge from external sender to a real Cinyuverse conversation, not just
...
- A desktop-created conversation emits channel notifications from real
  `ConversationEvent`s.
- An inbound chat `task` command creates or continues a real Cinyuverse conversation
