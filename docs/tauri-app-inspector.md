...
Cinyuverse 可以对当前工作区中的 Tauri v2 源码项目安装一个仅在 debug
构建中启用的元素检查器。检查器基于开源
[`tauri-plugin-redline`](https://github.com/twiced-technology-gmbh/redline-plugin-tauri)
（MIT）实现，并增加了 Cinyuverse 的项目内回传桥。

## 首次启用

1. 在 Cinyuverse 中打开包含 `src-tauri/` 的源码项目。
2. 点击右侧竖向工具栏中的“扫描元素”按钮。
3. Cinyuverse 会修改当前工作区副本：
   - 在 `src-tauri/Cargo.toml` 中加入 Redline 和 Cinyuverse 伴生插件；
   - 在 Tauri Builder 中以 `cfg!(debug_assertions)` 注册插件；
   - 为一个 JSON capability 加入 `redline:default` 和
     `cinyuverse-inspector:default`；
   - 生成 `.cinyuverse/tauri-plugin-cinyuverse-inspector/` 回传桥。
...
1. 保持目标 Tauri App 和对应的 Cinyuverse 会话同时打开。
2. 再次点击 Cinyuverse 右侧工具栏中的“扫描元素”按钮。
...
6. Cinyuverse 会自动接收 selector、元素 HTML、计算样式、坐标和修改说明，
   并把元素 chip 插入当前 Agent 输入框。补充一句整体要求后即可发送。

Redline 仍会保留它原有的 JSON 下载行为，便于故障时手工恢复；Cinyuverse
不需要用户选择或上传这个 JSON。

## 范围和安全性

- 只支持 Cinyuverse 当前工作区中的源码项目，不会尝试附加任意已安装 App。
- 插件注册放在 `debug_assertions` 分支，release 运行时不会启用覆盖层。
- 回传只写入项目自身的 `.cinyuverse/inspector/inbox/`，Cinyuverse 后端根据
