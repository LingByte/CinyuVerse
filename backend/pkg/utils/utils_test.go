...
// ===== NormalizeTrunkNumberVoiceDialogWs =====

func TestNormalizeTrunkNumberVoiceDialogWs(t *testing.T) {
	tests := []struct {
		input   string
		wantErr bool
	}{
	...
	for _, tt := range tests {
		_, err := NormalizeTrunkNumberVoiceDialogWs(tt.input)
		if (err != nil) != tt.wantErr {
			t.Errorf("NormalizeTrunkNumberVoiceDialogWs(%q) err=%v, wantErr=%v", tt.input, err, tt.wantErr)
		}
	}
}
...
func TestNormalizeTenantSlug(t *testing.T) {
	...
	for _, tt := range tests {
		got := NormalizeTenantSlug(tt.input)
		if got != tt.want {
			t.Errorf("NormalizeTenantSlug(%q) = %q, want %q", tt.input, got, tt.want)
		}
	}
}
...
// ===== NormalizeTransferAgentBriefText =====

func TestNormalizeTransferAgentBriefText(t *testing.T) {
	// Empty
	got, err := NormalizeTransferAgentBriefText("")
	if err != nil || got != "" {
		t.Fatalf("empty: got=%q, err=%v", got, err)
	}

	// Valid
	got, err = NormalizeTransferAgentBriefText("Hello {{Name}}")
	if err != nil {
		t.Fatalf("valid: err=%v", err)
	}
	if got != "Hello {{Name}}" {
	...
	_, err = NormalizeTransferAgentBriefText(string(long))
	if err == nil {
		t.Fatal("too long: should return error")
	}

	// Newline
	_, err = NormalizeTransferAgentBriefText("line1\nline2")
	if err == nil {
		t.Fatal("newline: should return error")
	}

	// Tab
	_, err = NormalizeTransferAgentBriefText("col1\tcol2")
	if err == nil {
		t.Fatal("tab: should return error")
	}
}
