...
# Design System: Cinyuverse Tahoe

`DESIGN.md` is the single current source of truth for frontend design in
Cinyuverse. Older audits, generated `.impeccable` output, and files with "legacy" in
their names are migration references only.

**Creative North Star: "The Developer Flight Deck, built as a good Mac citizen."**

Cinyuverse is a local engineering control surface for coordinating many agent sessions, worktrees, terminals, previews, and diffs without losing context. The density and calm of a flight deck stay; what changes is the finish — Cinyuverse should now read as a **native macOS Tahoe app**, not a web dashboard wrapped in a window.
...
This is the section that makes Cinyuverse feel like Tahoe rather than a web app. Decide which layer a surface belongs to before styling it.
...
Cinyuverse uses native SF Pro on macOS. Windows and Linux use bundled Inter
...
The **Composer Shell** (grounded, always-available action surface) and the **Agent bar** (icon-only, hover reveals the name, accent-filled when selected) are Cinyuverse's signatures. Keep their surrounding controls quieter than the primary input and the selected agent.
