...
#
#   curl -fsSL https://raw.githubusercontent.com/LingByte/CinyuVerse/master/install.sh | sh
#
# Environment:
#   CINYUVERSE_VERSION      Install this version instead of the latest release.
#   CINYUVERSE_PLATFORM     Override platform detection (e.g. linux-x86_64).
...
#   CINYUVERSE_INSTALL_DIR        Where the `cinyuverse` launcher goes. Default ~/.local/bin.
#   CINYUVERSE_PRINT_PLAN         Print the resolved platform and URLs, then exit.

set -eu

DEFAULT_REPO="LingByte/CinyuVerse"
REPO="${CINYUVERSE_GITHUB_REPO:-$DEFAULT_REPO}"
CACHE_ROOT="${HOME}/.cinyuverse/host-family"
INSTALL_DIR="${CINYUVERSE_INSTALL_DIR:-${HOME}/.local/bin}"
...
detect_platform() {
    if [ -n "${CINYUVERSE_PLATFORM:-}" ]; then
        printf '%s' "$CINYUVERSE_PLATFORM"
        return
    fi
...
verify_sha256sums() {
    root="$1"
    ( cd "$root" && grep -v '  SHA256SUMS$' SHA256SUMS > .cinyuverse-verify-list || true )
    if [ ! -s "${root}/.cinyuverse-verify-list" ]; then
        rm -f "${root}/.cinyuverse-verify-list"
        fail "SHA256SUMS in the archive contained no checksums"
    fi
    if command -v sha256sum >/dev/null 2>&1; then
        ( cd "$root" && sha256sum --quiet --check .cinyuverse-verify-list ) ||
            fail "the extracted Host family failed per-file verification"
    else
        ( cd "$root" && shasum -a 256 --status --check .cinyuverse-verify-list ) ||
            fail "the extracted Host family failed per-file verification"
    fi
    rm -f "${root}/.cinyuverse-verify-list"
}
...
write_launcher() {
    family_root="$1"
    mkdir -p "$INSTALL_DIR"
    launcher="${INSTALL_DIR}/cinyuverse"
    cat > "$launcher" <<LAUNCHER
#!/usr/bin/env sh
exec "${family_root}/cinyuverse-server" "\$@"
LAUNCHER
    chmod +x "$launcher"
    printf '%s' "$launcher"
}
...
main() {
    ...
    if [ -n "${CINYUVERSE_VERSION:-}" ]; then
        tag="v${CINYUVERSE_VERSION#v}"
    else
        tag=$(latest_tag)
    ...
    if [ -n "${CINYUVERSE_HOST_FAMILY_BASE:-}" ]; then
        base_url="${CINYUVERSE_HOST_FAMILY_BASE%/}"
    else
        base_url="https://github.com/${REPO}/releases/download/${tag}"
    fi
    family_root="${CACHE_ROOT}/${tag}/${platform}/family"

    if [ -n "${CINYUVERSE_PRINT_PLAN:-}" ]; then
        printf 'platform=%s\n' "$platform"
        printf 'tag=%s\n' "$tag"
        printf 'archive=%s\n' "$archive"
    ...
    if [ -f "${family_root}/SHA256SUMS" ]; then
        printf 'Cinyuverse Host family %s is already installed for %s.\n' "$tag" "$platform"
        verify_sha256sums "$family_root"
    else
        TEMP_DIR=$(mktemp -d)
        printf 'Downloading Cinyuverse Host family %s for %s...\n' "$tag" "$platform"
    ...
    chmod +x "${family_root}/cinyuverse-server" "${family_root}/cinyuverse-mcp" \
        "${family_root}/cinyuverse-workflow-mcp" 2>/dev/null || true

    launcher=$(write_launcher "$family_root")
    printf '\nInstalled Cinyuverse Host family %s to %s\n' "$tag" "$family_root"
    printf 'Launcher: %s\n' "$launcher"

    case ":${PATH}:" in
        *":${INSTALL_DIR}:"*) printf '\nRun `cinyuverse` to start the server on http://127.0.0.1:17891\n' ;;
        *) printf '\nAdd %s to your PATH, then run `cinyuverse`:\n  export PATH="%s:$PATH"\n' \
