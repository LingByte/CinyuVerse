...
`npx cinyuverse` launches the Host family `cinyuverse-server` (not the Tauri desktop app).
It downloads the GitHub Release tarball for this platform, verifies the
sidecar SHA-256 and the inner `SHA256SUMS`, then starts `cinyuverse-server` with
`CINYUVERSE_STATIC_ROOT` pointed at the packaged `web/` tree.
...
`npx cinyuverse` starts the Host on loopback. `npx cinyuverse serve` opens the Web UI on
...
Pin a tag with `CINYUVERSE_HOST_FAMILY_TAG=v0.1.3`. Point at an already extracted
directory with `CINYUVERSE_HOST_FAMILY_DIR`.

Package a validated Cinyuverse product plugin from its project directory:
...
```bash
cinyuverse plugin add --web https://github.com/LingByte/cinyuverse-plugin-office#v1.0.0 -y
cinyuverse plugin add --profile ./search.vxp
cinyuverse plugin add --dev .
cinyuverse plugin list
cinyuverse plugin remove acme.search
```

## What is Cinyuverse?

Cinyuverse is a modern project management tool designed specifically for developers. It helps you organize your coding projects with kanban-style task management while providing powerful integrations with git repositories and AI coding agents.
...
Cinyuverse provides a complete project management experience with these key capabilities:
...
Cinyuverse supports customization through its configuration system:
