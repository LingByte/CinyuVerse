...
const HELP = `Usage: cinyuverse plugin pack [dir] [--output file.vxp]
       cinyuverse plugin add --web <git-or-url[#ref]> [--plugin ID] [--yes]
       cinyuverse plugin add --profile <file.vxp|archive> [--plugin ID] [--yes]
       cinyuverse plugin add --dev <dir> [--yes] [--detach]
       cinyuverse plugin publish [dir|file.vxp] [--owner USER] [--password PASS] [--show-tree]
       cinyuverse plugin publish --web <github-owner/repo[#tag]> [--owner USER] [--password PASS]
       cinyuverse plugin list [--json]
       cinyuverse plugin update <id> [--ref tag] [--yes]
       cinyuverse plugin remove <id> [--yes] [--delete-data]
       cinyuverse plugin gc-runtimes
       cinyuverse plugin test --host [dir]
...
publish Submit a packed .vxp or GitHub repository to the official marketplace review queue.
        ...
        --owner USER    Marketplace username (or CINYUVERSE_MARKET_OWNER)
        --password PASS Marketplace password (or CINYUVERSE_MARKET_PASSWORD)
...
async function removePlugin(args) {
  ...
  if (!pluginId) {
    throw new Error("Usage: cinyuverse plugin remove <plugin-id> [--yes] [--delete-data]");
  }
  const host = await requireRunningHost();
  const catalog = await hostCall(host, "plugin_control_catalog", {});
  const plugins = Array.isArray(catalog?.plugins) ? catalog.plugins : [];
...
async function requireRunningHost() {
  ...
  if (!host.token || !(await hostAccepts(host.url, host.token))) {
    throw new Error("No running Cinyuverse Host. Start Desktop or `cinyuverse serve`.");
  }
  return host;
}
...
async function addWebPlugin(source, flags) {
  ...
  const staging = fs.mkdtempSync(path.join(os.tmpdir(), "cinyuverse-plugin-add-"));
  try {
...
async function installUnpackedSnapshot(unpacked, flags, lock = {}) {
  ...
  if (roots.length === 0) {
    throw new Error("No Cinyuverse plugin package found (.cinyuverse-plugin/plugin.json).");
  }
  ...
  log(
    "No running Host accepted the install. Cinyuverse desktop or server will import this .vxp on next launch.",
  );
}
...
async function addDevPlugin(source, flags) {
  ...
  if (!fs.existsSync(path.join(root, ".cinyuverse-plugin", "plugin.json"))) {
    throw new Error("No Cinyuverse plugin package found (.cinyuverse-plugin/plugin.json).");
  }
...
function marketplaceOrigin(flag) {
  return String(flag || process.env.CINYUVERSE_MARKETPLACE_URL || "https://cinyuverse.xforever.xin").replace(
    /\/+$/,
    "",
  );
}
...
async function publishPlugin(args) {
  const parsed = parseArgs(args);
  const owner = parsed.flags.owner || process.env.CINYUVERSE_MARKET_OWNER || "";
  const password = parsed.flags.password || process.env.CINYUVERSE_MARKET_PASSWORD || "";
  if (!owner || !password) {
    throw new Error(
      "publish requires --owner and --password, or CINYUVERSE_MARKET_OWNER and CINYUVERSE_MARKET_PASSWORD.",
    );
  }
...
function readPluginIdentity(root) {
  const manifest = JSON.parse(
    fs.readFileSync(path.join(root, ".cinyuverse-plugin", "plugin.json"), "utf8"),
  );
  return {
...
function importsDirectory() {
  return path.join(os.homedir(), ".cinyuverse", "imports");
}
...
function parseAddArgs(args) {
  ...
  if (!source) {
    throw new Error(
      "Usage: cinyuverse plugin add --web <url> | --profile <file> | --dev <dir> [--yes]",
    );
  }
...
async function updatePlugin(args) {
  ...
  const pluginId = parsed.positional[0];
  if (!pluginId) {
    throw new Error("Usage: cinyuverse plugin update <plugin-id> [--ref tag] [--yes]");
  }
  const host = await requireRunningHost();
  const catalog = await hostCall(host, "plugin_control_catalog", {});
...
async function testAgainstHost(args) {
  ...
  if (!parsed.flags.host) {
    throw new Error("Usage: cinyuverse plugin test --host [dir]");
  }
  const root = path.resolve(parsed.positional[0] || ".");
  await ensureValidPlugin(root);
  const identity = readPluginIdentity(root);
...
function findPluginRoots(dir, depth = 0, found = []) {
  if (fs.existsSync(path.join(dir, ".cinyuverse-plugin", "plugin.json"))) {
    found.push(dir);
    return found;
  }
...
function hostDataDirs() {
  ...
  const dirs = [];
  if (process.env.CINYUVERSE_DATA_DIR) dirs.push(process.env.CINYUVERSE_DATA_DIR);
  if (process.platform === "darwin") {
    dirs.push(
      path.join(home, "Library", "Application Support", "com.cinyuverse.app"),
      path.join(home, "Library", "Application Support", "cinyuverse"),
      path.join(home, "Library", "Application Support", "app.cinyuverse.cinyuverse"),
    );
  } else if (process.platform === "win32") {
    const appData = process.env.APPDATA || path.join(home, "AppData", "Roaming");
    dirs.push(path.join(appData, "com.cinyuverse.app"), path.join(appData, "cinyuverse"));
  } else {
    const dataHome = process.env.XDG_DATA_HOME || path.join(home, ".local", "share");
    dirs.push(
      path.join(dataHome, "com.cinyuverse.app"),
      path.join(dataHome, "cinyuverse"),
    );
  }
  dirs.push(path.join(home, ".cinyuverse-data"), path.join(home, ".cinyuverse"));
  return dirs;
}
...
function discoverHost() {
  const envUrl = (process.env.CINYUVERSE_URL || "").replace(/\/+$/, "");
  const envToken = (process.env.CINYUVERSE_TOKEN || "").trim();
  let token = envToken;
  let port = 17891;
  if (!token) {
  ...
  try {
    const response = await fetch(`${url}/api/v1/call/plugin_control_catalog`, {
      ...
      headers: {
        ...
        "x-cinyuverse-protocol-version": "1.0",
      },
...
async function load(file) {
  ...
  if (!fs.existsSync(modulePath)) {
    throw new Error(
      "Cinyuverse Plugin CLI is unavailable; reinstall or update Cinyuverse.",
    );
  }
