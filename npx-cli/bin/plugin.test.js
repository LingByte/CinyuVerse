...
test("cinyuverse plugin pack creates a validated VXP package", () => {
  const root = mkdtempSync(join(tmpdir(), "cinyuverse-cli-plugin-"));
  ...
  try {
    ...
    const result = spawnSync(
      ...
      {
        encoding: "utf8",
        env: { ...process.env, CINYUVERSE_LOCAL: "1" },
      },
    );
...
test("plugin add extracts a vxp whose entries have no unix mode bits", () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "cinyuverse-plugin-lock-"));
  const dest = mkdtempSync(join(tmpdir(), "cinyuverse-plugin-out-"));
  try {
    writeFixture(root);
...
test("plugin add discovers a local Host token without CINYUVERSE_TOKEN", () => {
  const { discoverHost } = require("./plugin");
  const dir = mkdtempSync(join(tmpdir(), "cinyuverse-host-"));
  const prevDir = process.env.CINYUVERSE_DATA_DIR;
  const prevToken = process.env.CINYUVERSE_TOKEN;
  const prevUrl = process.env.CINYUVERSE_URL;
  try {
    delete process.env.CINYUVERSE_TOKEN;
    delete process.env.CINYUVERSE_URL;
    process.env.CINYUVERSE_DATA_DIR = dir;
    ...
    if (prevDir === undefined) delete process.env.CINYUVERSE_DATA_DIR;
    else process.env.CINYUVERSE_DATA_DIR = prevDir;
    if (prevToken === undefined) delete process.env.CINYUVERSE_TOKEN;
    else process.env.CINYUVERSE_TOKEN = prevToken;
    if (prevUrl === undefined) delete process.env.CINYUVERSE_URL;
    else process.env.CINYUVERSE_URL = prevUrl;
...
test("plugin add resolves marketplace and GitHub sources", async () => {
  ...
  const root = mkdtempSync(join(tmpdir(), "cinyuverse-plugin-find-"));
  try {
    writeFixture(root);
...
test("plugin add parses --web, --profile, and --dev", () => {
  ...
  const git = parseGitSource("https://github.com/LingByte/cinyuverse-plugin-office");
  assert.equal(git.url, "https://github.com/LingByte/cinyuverse-plugin-office.git");
  assert.equal(git.ref, undefined);
  const branch = parseGitSource(
    "https://github.com/LingByte/cinyuverse-plugin-office/tree/main",
  );
  assert.equal(branch.ref, "main");
  const tagged = parseGitSource(
    "https://github.com/LingByte/cinyuverse-plugin-office#v1.2.0",
  );
  assert.equal(tagged.ref, "v1.2.0");
  const spec = parseGitSource("github:LingByte/cinyuverse-plugin-office#v1.2.0");
  assert.equal(spec.url, "https://github.com/LingByte/cinyuverse-plugin-office.git");
  assert.equal(spec.ref, "v1.2.0");
  const shorthandTag = parseGitSource("LingByte/cinyuverse-plugin-office#v1.2.0");
  assert.equal(shorthandTag.ref, "v1.2.0");
  const commit = parseGitSource(
    "https://github.com/LingByte/cinyuverse-plugin-office/commit/abcdef1",
  );
  assert.equal(commit.ref, "abcdef1");
  const archive = parseGitSource("https://example.com/search.vxp");
  assert.equal(archive, null);
  const root = mkdtempSync(join(tmpdir(), "cinyuverse-plugin-infer-"));
  try {
    writeFixture(root);
...
test("plugin list formats catalog rows", () => {
  ...
  assert.match(table, /cinyuverse\.office\s+3\.0\.0\s+builtin\s+off/);
...
test("plugin publish parses credentials and marketplace origin", () => {
  ...
  assert.equal(marketplaceOrigin(), "https://cinyuverse.xforever.xin");
  assert.equal(
...
function writeFixture(root) {
  mkdirSync(join(root, ".cinyuverse-plugin"), { recursive: true });
  mkdirSync(join(root, "contents", "skills", "test"), { recursive: true });
  writeFileSync(
    join(root, ".cinyuverse-plugin", "plugin.json"),
    JSON.stringify({
      manifestVersion: 4,
      apiVersion: "1.0",
      id: "test",
      ...
      name: "Test",
      readme: "README.md",
      engines: { cinyuverse: ">=0.1.3", pluginSdk: "^1.0.0" },
      content: {
        root: "contents",
  ...
  writeFileSync(
    join(root, ".cinyuverse-plugin", "content.index.json"),
    JSON.stringify({
      schemaVersion: 1,
      items: [
        {
  ...
  writeFileSync(
    join(root, "README.md"),
    "---\nsummary: Test the Cinyuverse plugin pack command.\n---\n# Test\n",
  );
