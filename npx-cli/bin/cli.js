...
async function main() {
  ...
  console.error(`Fetching Cinyuverse Host family ${familyTag()} for ${platformDir}...`);
  let family;
  try {
  ...
  const binName = isMcpMode ? "cinyuverse-mcp" : "cinyuverse-server";
  ...
  if (!isMcpMode) {
    ...
    if (fs.existsSync(staticRoot)) {
      env.CINYUVERSE_STATIC_ROOT = staticRoot;
    }
  }
  ...
  if (!isMcpMode && !isHostUtility) {
    console.error(`Starting Cinyuverse Host ${CLI_VERSION} (${family.source})...`);
  }
...
main().catch((err) => {
  ...
  if (process.env.CINYUVERSE_DEBUG) {
