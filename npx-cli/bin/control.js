...
async function call(command, args, flags) {
  const baseUrl = (process.env.CINYUVERSE_URL || 'http://127.0.0.1:17891').replace(/\/+$/, '');
  const token = process.env.CINYUVERSE_TOKEN;
  if (!token) throw new Error('CINYUVERSE_TOKEN is required');
