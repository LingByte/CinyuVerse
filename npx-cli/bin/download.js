...
const DEFAULT_GITHUB_REPO = "LingByte/CinyuVerse";
const CACHE_ROOT = path.join(os.homedir(), ".cinyuverse", "host-family");
const LOCAL_DIST_DIR = path.join(__dirname, "..", "dist");

function githubRepo() {
  return process.env.CINYUVERSE_GITHUB_REPO || DEFAULT_GITHUB_REPO;
}

function familyTag() {
  return process.env.CINYUVERSE_HOST_FAMILY_TAG || `v${CLI_VERSION}`;
}
...
function familyBaseUrl() {
  if (process.env.CINYUVERSE_HOST_FAMILY_BASE) {
    return process.env.CINYUVERSE_HOST_FAMILY_BASE.replace(/\/+$/, "");
  }
...
function chmodBinaries(root) {
  ...
  for (const name of ["cinyuverse-server", "cinyuverse-mcp"]) {
...
function localFamilyRoot(platform) {
  if (process.env.CINYUVERSE_HOST_FAMILY_DIR) {
    return path.resolve(process.env.CINYUVERSE_HOST_FAMILY_DIR);
  }
