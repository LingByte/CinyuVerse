...
test("builds GitHub release URLs for the official Host family tarball", () => {
  assert.equal(
    ...
    `Cinyuverse-${CLI_VERSION}-darwin-aarch64-server.tar.gz`,
  );
  assert.equal(
    familyAssetUrl("linux-x86_64"),
    `https://github.com/${DEFAULT_GITHUB_REPO}/releases/download/v${CLI_VERSION}/Cinyuverse-${CLI_VERSION}-linux-x86_64-server.tar.gz`,
  );
  assert.equal(
    familyAssetUrl("windows-x86_64", ".sha256"),
    `https://github.com/${DEFAULT_GITHUB_REPO}/releases/download/v${CLI_VERSION}/Cinyuverse-${CLI_VERSION}-windows-x86_64-server.tar.gz.sha256`,
  );
});

test("parses sha256sum lines and verifies extracted family files", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cinyuverse-family-sum-"));
  ...
  const sums = `${serverDigest}  cinyuverse-server\n${webDigest}  web/index.html\n`;

  const parsed = parseSha256Sums(sums);
  assert.equal(parsed.get("cinyuverse-server"), serverDigest);
...
test("rejects a family whose SHA256SUMS does not match the bytes on disk", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cinyuverse-family-bad-"));
  fs.writeFileSync(path.join(root, "cinyuverse-server"), "tampered");
  const expected = crypto.createHash("sha256").update("server-bin").digest("hex");
  assert.throws(
    () => verifySha256Sums(root, `${expected}  cinyuverse-server\n`),
    /Checksum mismatch/,
  );
});

test("ensureHostFamily uses a local directory after verifying SHA256SUMS", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "cinyuverse-family-local-"));
  const server = path.join(root, "cinyuverse-server");
  fs.writeFileSync(server, "server-bin");
  const digest = crypto.createHash("sha256").update("server-bin").digest("hex");
  fs.writeFileSync(path.join(root, "SHA256SUMS"), `${digest}  cinyuverse-server\n`);
  const previous = process.env.CINYUVERSE_HOST_FAMILY_DIR;
  process.env.CINYUVERSE_HOST_FAMILY_DIR = root;
  try {
    ...
    if (previous === undefined) {
      delete process.env.CINYUVERSE_HOST_FAMILY_DIR;
    } else {
      process.env.CINYUVERSE_HOST_FAMILY_DIR = previous;
    }
  }
});
