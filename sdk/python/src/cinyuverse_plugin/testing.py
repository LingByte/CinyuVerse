...
Fidelity gap versus a real Cinyuverse Host:
...
REDACT_KEYS = re.compile(
    r"(token|authorization|secret|password|CINYUVERSE_SERVER_TOKEN|CINYUVERSE_PLUGIN_DEV_GRANT)",
    re.IGNORECASE,
)
...
async def create_worker_harness(
    ...
    worker = await activate_plugin_worker(
        definition,
        context={
            "pluginId": context.get("pluginId") or "dev.cinyuverse.test",
