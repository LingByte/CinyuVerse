...
import { Files, BookOpen, List, Bot, Search, GitBranch, Puzzle } from 'lucide-vue-next'
...
const rightActiveId = ref<string | null>('ai')
...
const rightItems = [{ id: 'ai', label: 'AI 助手', icon: Bot }]
const rightPanels = [{ id: 'ai', title: 'AI 助手', minWidth: 300, defaultWidth: 340 }]
...
async function refreshGitBranch() {
  if (!rootPath.value || !isDesktop()) {
    gitBranch.value = ''
    return
  }
  try {
    const isRepo = await ideApi.isGitRepository(rootPath.value)
    gitBranch.value = isRepo ? await ideApi.gitCurrentBranch(rootPath.value) : ''
  } catch {
    gitBranch.value = ''
  }
}
...
watch(rootPath, () => { void refreshGitBranch() }, { immediate: true })
    ...
    <template v-else>
      ...
      <div class="ide-main">
        ...
        >
          ...
          />
          <GitPanel
            v-show="activePanelId === 'git'"
            :root-path="rootPath"
            :on-output="appendOutput"
          />
          <ExtensionsPanel v-show="activePanelId === 'extensions'" />
        ...
        <ResizableRightPanel
          ...
          :on-active-change="(id) => { rightActiveId = id }"
        >
          <template #ai>
            <AiChatPanel
              :connected="!!currentWorkspace"
