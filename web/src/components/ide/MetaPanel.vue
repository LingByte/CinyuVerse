...
import { getCharacters, saveCharacters, getGlossary, saveGlossary } from '@/api/ide'
import type { CharacterCard, GlossaryEntry } from '@/api/ide'
...
async function load() {
  ...
  characters.value = await getCharacters(props.workspaceId)
  glossary.value = await getGlossary(props.workspaceId)
}
...
async function persistCharacters() {
  ...
  saving.value = true
  try {
    characters.value = await saveCharacters(props.workspaceId, characters.value)
  } finally {
    saving.value = false
  }
}
...
async function persistGlossary() {
  ...
  saving.value = true
  try {
    glossary.value = await saveGlossary(props.workspaceId, glossary.value)
  } finally {
    saving.value = false
  }
}
      ...
      <div v-if="subTab === 'characters'" class="panel-scroll">
        <div v-for="(c, i) in characters" :key="c.id || i" class="card">
          <div class="card-head">
            ...
            <button class="icon-btn danger" title="删除" @click="removeCharacter(i)">×</button>
          </div>
        ...
        <button class="add-btn" @click="addCharacter">+ 添加人物</button>
      </div>
      ...
      <div v-else class="panel-scroll">
        ...
        <div v-for="(g, i) in filteredGlossary()" :key="g.id || i" class="card">
          <div class="card-head">
            ...
            <button class="icon-btn danger" @click="removeGlossaryEntry(g)">×</button>
          </div>
        ...
        <button class="add-btn" @click="addGlossary">+ 添加词条</button>
      </div>
