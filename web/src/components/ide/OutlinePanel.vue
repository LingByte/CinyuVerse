...
import { getOutline, saveOutline, importOutlineMd, exportOutlineMd } from '@/api/ide'
import type { ProjectOutline, OutlineNode, TimelineEvent } from '@/api/ide'
...
const outline = ref<ProjectOutline>({ book_outline: '', volume_nodes: [], timeline: [] })
const saving = ref(false)

async function load() {
  if (!props.workspaceId) return
  outline.value = await getOutline(props.workspaceId)
}
...
async function persist() {
  ...
  saving.value = true
  try {
    outline.value = await saveOutline(props.workspaceId, outline.value)
  } finally {
    saving.value = false
  }
}
...
async function exportMd() {
  if (!props.workspaceId) return
  const blob = await exportOutlineMd(props.workspaceId)
...
async function importMd() {
  ...
  outline.value = await importOutlineMd(props.workspaceId, text.trim())
}
