...
import { useThemeStore, type AccentColor } from '@/features/theme/stores/themeStore'
import { useEditorSchemeStore } from '@/features/editor/stores/editorSchemeStore'
import { EDITABLE_THEME_KEYS } from '@/features/theme/config/themePresets'
import { EDITABLE_SCHEME_KEYS, SCHEME_LABELS } from '@/features/theme/config/editorSchemes'
import type { ThemePreset } from '@/features/theme/config/themePresets'
import type { EditableSchemeKey } from '@/features/theme/config/editorSchemes'
...
const bgPreviewStyle = computed(() => {
  const img = theme.backgroundImage
...
async function pickFile(_filters: { name: string; extensions: string[] }[]): Promise<{ content: string; encoding?: 'utf8' | 'base64'; name: string } | null> {
  if (!isDesktop()) return null
  const files = await desktopApi.openFiles()
  if (!files?.length) return null
  const f = files[0]
  return { content: f.content, encoding: f.encoding, name: f.name }
}
...
function applyBackgroundDataUrl(dataUrl: string, filename: string) {
  const persisted = theme.setBackgroundImage(dataUrl, { size: theme.backgroundSize })
  importInfo.value = persisted
    ? `背景图已应用（${filename}）`
    : `背景图已应用（${filename}），但图片过大未能保存到本地，刷新后将丢失`
  importError.value = ''
}
...
async function saveExport(content: string, filename: string) {
  if (isDesktop()) {
    await desktopApi.saveFile({ defaultPath: filename, content })
    return
  }
