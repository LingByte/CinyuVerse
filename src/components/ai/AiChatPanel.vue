...
const statusText = computed(() => {
  ...
  if (!storyStore.currentBookId) return '选择书籍'
...
async function runAction(action: PipelineAction) {
  ...
  error.value = ''
  try {
    const out = await agent.runPipeline(action, instruction.value.trim())
...
<template>
  <div class="agent-panel ai-chat" @click="showBookMenu = false">
    ...
    <div class="ai-thread">
      ...
      <div v-if="!storyStore.connected" class="empty-state">
        <div class="empty-icon"><Sparkles :size="22" /></div>
        <h3 class="empty-title">后端未连接</h3>
        <p class="empty-desc">启动 Go 后端后即可使用 AI 助手</p>
        <code class="empty-code">cd backend && go run ./cmd/cinyu-story serve</code>
        <button type="button" class="empty-action" @click="storyStore.ping()">重试连接</button>
      </div>

      <div v-else-if="!storyStore.currentBookId" class="empty-state">
        <div class="empty-icon"><BookOpen :size="22" /></div>
        <h3 class="empty-title">选择一本书</h3>
        <p class="empty-desc">在左侧「后端」面板创建或选择书籍后开始对话</p>
      </div>
    ...
    <!-- Pipeline actions -->
    <div
      v-if="panelMode === 'pipeline' && storyStore.connected && storyStore.currentBookId"
        ...
        <div class="composer-footer">
          ...
          <div class="composer-actions">
            ...
            <button
              ...
              @click="handleSend"
            >
              <Loader2 v-if="storyStore.busy" :size="15" class="spin" />
