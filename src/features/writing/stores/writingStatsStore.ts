import { writingStatsKey, wordTargetKey } from '@/core/storage/keys'
...
export const useWritingStatsStore = defineStore('writingStats', () => {
  ...
  function load(workspaceId: string) {
    wsId = workspaceId
    try {
      const raw = localStorage.getItem(writingStatsKey(workspaceId))
      dailyStats.value = raw ? JSON.parse(raw) : []
      const t = localStorage.getItem(wordTargetKey(workspaceId))
  ...
  function persist() {
    if (!wsId) return
    localStorage.setItem(writingStatsKey(wsId), JSON.stringify(dailyStats.value))
    localStorage.setItem(wordTargetKey(wsId), String(targetWords.value))
  }
