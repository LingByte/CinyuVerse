...
function resolveKotlinCompiler() {
  if (process.env.CINYUVERSE_KOTLINC) {
    return process.env.CINYUVERSE_KOTLINC;
  }
...
test("companion-core pairing client compiles and rejects workstation scopes", () => {
  ...
  const sources = [
    ...
    path.join(core, "src/smoke/kotlin/dev/cinyuverse/companion/CompanionCoreSmoke.kt"),
  ];
  const output = path.join(os.tmpdir(), `cinyuverse-companion-core-${process.pid}.jar`);
