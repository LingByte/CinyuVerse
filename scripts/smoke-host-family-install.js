...
const PLATFORM = process.env.CINYUVERSE_PLATFORM || platformFromNode(process.platform, process.arch);
if (!PLATFORM) {
...
function writeFamily(dir) {
  ...
  const server = path.join(dir, process.platform === 'win32' ? 'cinyuverse-server.exe' : 'cinyuverse-server');
  const script =
    process.platform === 'win32'
      ? '@echo off\necho cinyuverse-server-smoke\n'
      : '#!/bin/sh\necho cinyuverse-server-smoke\n';
  ...
  for (const relative of [
    process.platform === 'win32' ? 'cinyuverse-server.exe' : 'cinyuverse-server',
    'web/index.html',
  ]) {
...
async function main() {
  const work = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-install-smoke-'));
  ...
  const sharedEnv = {
    HOME: home,
    CINYUVERSE_VERSION: VERSION,
    CINYUVERSE_PLATFORM: PLATFORM,
    CINYUVERSE_HOST_FAMILY_BASE: origin,
    CINYUVERSE_INSTALL_DIR: bin,
    CINYUVERSE_PRINT_PLAN: '',
  };

  try {
    const plan = await runInstall({ ...sharedEnv, CINYUVERSE_PRINT_PLAN: '1' });
    if (plan.status !== 0) {
      throw new Error(
    ...
    const installed = path.join(home, '.cinyuverse', 'host-family', `v${VERSION}`, PLATFORM, 'family');
    ...
    fs.rmSync(path.join(home, '.cinyuverse'), { recursive: true, force: true });
