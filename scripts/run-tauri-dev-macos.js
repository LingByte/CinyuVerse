...
const HELPER_NAMES = [
  'cinyuverse Helper',
  'cinyuverse Helper (Alerts)',
  'cinyuverse Helper (GPU)',
  'cinyuverse Helper (Plugin)',
  'cinyuverse Helper (Renderer)',
];
const DEV_BUNDLE_IDENTIFIER = 'com.cinyuverse.app.dev';
...
function resolveMacosDevPaths(workspaceRoot, sourceAppExecutable, targetRoot) {
  ...
  const appRoot = path.join(stageRoot, 'app', 'cinyuverse.app');
  ...
  return {
    appExecutable: path.join(appRoot, 'Contents', 'MacOS', 'cinyuverse'),
    appRoot,
...
function isStagedBundleReady(paths) {
  ...
  try {
    ...
    // Reuse it only when the cached bundle is already Cinyuverse Dev.
    return (
...
function refreshBundleExecutables(paths, profileDirectory) {
  ...
  for (const name of ['cinyuverse-mcp', 'cinyuverse-workflow-mcp']) {
...
async function run() {
  ...
  const paths = resolveMacosDevPaths(
    workspaceRoot,
    path.join(profileDirectory, 'cinyuverse'),
    targetRoot
  );
  ...
  const mcpBuildArgs = ['build', '-p', 'cinyuverse-mcp', '--bin', 'cinyuverse-mcp'];
