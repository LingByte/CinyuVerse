#!/bin/sh
# Cinyuverse public tunnel installer. POSIX sh. Safe to pipe: curl -fsSL https://cinyuverse.xforever.xin/tunnel.sh | sh
set -eu

TOKEN=""
PORT=""
BIND="0.0.0.0"
PY_URL="https://cinyuverse.xforever.xin/tunnel.py"
MODE="install"
...
service_status() {
  if command -v systemctl >/dev/null 2>&1 && [ -f /etc/systemd/system/cinyuverse-tunnel.service ]; then
    case "$(systemctl is-active cinyuverse-tunnel 2>/dev/null || true)" in
      active) printf '%s\n' "running"; return ;;
      failed) printf '%s\n' "failed"; return ;;
...
if [ "$(id -u)" -eq 0 ]; then
  LIBDIR="/usr/local/lib/cinyuverse-tunnel"
  CONFDIR="/etc/cinyuverse-tunnel"
  LOGFILE="/var/log/cinyuverse-tunnel.log"
else
  LIBDIR="${HOME}/.cinyuverse-tunnel"
  CONFDIR="${HOME}/.cinyuverse-tunnel"
  LOGFILE="${HOME}/.cinyuverse-tunnel/tunnel.log"
fi
...
if command -v systemctl >/dev/null 2>&1 && [ "$(id -u)" -eq 0 ] && [ -d /etc/systemd/system ]; then
  cat > /etc/systemd/system/cinyuverse-tunnel.service <<EOF
[Unit]
Description=Cinyuverse public tunnel
...
EOF
  ...
  systemctl enable cinyuverse-tunnel >/dev/null 2>&1 || true
  systemctl restart cinyuverse-tunnel
  print_report
  exit 0
fi

if command -v launchctl >/dev/null 2>&1 && [ "$(uname -s)" = "Darwin" ]; then
  PLIST="${HOME}/Library/LaunchAgents/com.cinyuverse.tunnel.plist"
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.cinyuverse.tunnel</string>
