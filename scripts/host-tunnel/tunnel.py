#!/usr/bin/env python3
# Cinyuverse public tunnel server. Stdlib only. Python 3.6+.
...
def handle_client(sock, token):
    try:
        ...
        if prefix.startswith(b"CINYUVERSE-CTRL ") or prefix.startswith(b"CINYUVERSE-DATA "):
            line, extra = prefix.split(b"\n", 1) if b"\n" in prefix else (prefix, b"")
            text = line.decode("ascii", "replace").strip()
            parts = text.split(" ")
            if parts[0] == "CINYUVERSE-CTRL" and len(parts) == 2 and parts[1] == token:
                register_control(sock)
                return
            if parts[0] == "CINYUVERSE-DATA" and len(parts) == 3 and parts[1] == token:
...
def serve(bind, port, token):
    ...
    server.listen(128)
    write_host_status("waiting")
    log("Cinyuverse tunnel listening on %s:%s" % (bind, port))
    while True:
