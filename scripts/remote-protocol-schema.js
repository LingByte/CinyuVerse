...
function kotlinModels(schema) {
  ...
  const models = Object.keys(definitions)
    ...
    .join('\n');
  return `${generatedHeader('//')}
package dev.cinyuverse.remote.v1
...
function protocolReadme() {
  return `# Cinyuverse Remote Protocol v1
...
\`cinyuverse.token.<base64url-token>\`; credentials are never accepted in a URL.
...
diagnostics, file paths, tokens, or other secrets. Cinyuverse does not connect this
...
function resolveKotlinCompiler() {
  if (process.env.CINYUVERSE_KOTLINC) return process.env.CINYUVERSE_KOTLINC;
...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-protocol-'));
try {
