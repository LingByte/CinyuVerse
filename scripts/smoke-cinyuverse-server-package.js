...
async function waitForHealth(port, child, readSpawnError) {
  ...
  while (Date.now() < deadline) {
    ...
    if (child.exitCode !== null) {
      throw new Error(`cinyuverse-server exited early with ${child.exitCode}`);
    }
    try {
  ...
  throw new Error('timed out waiting for packaged cinyuverse-server');
}
...
async function main() {
  ...
  const build = spawnSync(
    'cargo',
    ['build', '--release', '-p', 'server', '--bin', 'cinyuverse-server'],
    {
      cwd: root,
      env: {
        ...process.env,
  ...
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-server-smoke-'));
  ...
  const child = spawn(binary, [], {
    ...
    env: {
      ...
      CINYUVERSE_STATIC_ROOT: staticRoot,
  ...
  process.stdout.write('packaged cinyuverse-server smoke passed\n');
}
