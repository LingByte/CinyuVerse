...
test('CEF binaries resolve under the cargo target triple directory', () => {
  ...
  assert.equal(
    resolveProfileBin({
      ...
      target: null,
      profile: 'debug',
      name: 'cinyuverse_cef_helper',
      platform: 'win32',
    }),
    path.join('/repo/target', 'debug', 'cinyuverse_cef_helper.exe')
  );
});

test('Linux stager execution prepends the CEF library directory', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-'));
...
test('already-staged CEF runtime is detected from the manifest and required files', () => {
  ...
  const targetRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-stage-'));
