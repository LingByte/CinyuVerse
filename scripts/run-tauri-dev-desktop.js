...
const DESKTOP_DEV_IDENTITY = {
  deepLinkScheme: 'cinyuverse-dev',
  identifier: 'com.cinyuverse.app.dev',
  productName: 'Cinyuverse Dev',
};
...
$target = [System.IO.Path]::GetFullPath($env:CINYUVERSE_STALE_EXE_PATH)
$processes = Get-Process -Name 'cinyuverse' -ErrorAction SilentlyContinue | Where-Object {
  try {
...
`;
  ...
  const result = spawnSync(
    ...
    ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', powershellScript],
    {
      env: {
        ...process.env,
        CINYUVERSE_STALE_EXE_PATH: executablePath,
      },
      stdio: 'inherit',
      timeout: 10000,
      windowsHide: true,
