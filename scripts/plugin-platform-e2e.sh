...
pnpm --filter @cinyuverse/plugin-cli exec node -e "import('./dist/cli.js')" >/dev/null 2>&1 || true
pnpm --filter @cinyuverse/plugin-cli test
...
echo "== toolchain =="
pnpm --filter @cinyuverse/plugin-cli exec node dist/cli.js toolchain | grep -q hostVersion
