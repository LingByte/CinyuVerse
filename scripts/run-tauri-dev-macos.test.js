...
function writeStagedBundleFixture(bundleIdentifier) {
  const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-cef-identity-'));
...
test('macOS CEF dev cache is ready only for the Cinyuverse Dev bundle identity', () => {
  const product = writeStagedBundleFixture('com.cinyuverse.app');
  const missing = writeStagedBundleFixture(undefined);
  const dev = writeStagedBundleFixture('com.cinyuverse.app.dev');
...
test('macOS CEF dev staging requests the isolated development bundle identity', () => {
  ...
  assert.match(runner, /com\.cinyuverse\.app\.dev/);
