...
test('asset names match the archives CI already publishes', () => {
  assert.equal(archiveName('0.1.9', 'linux-x86_64'), 'Cinyuverse-0.1.9-linux-x86_64-server.tar.gz');
  assert.equal(archiveName('v0.1.9', 'darwin-aarch64'), 'Cinyuverse-0.1.9-darwin-aarch64-server.tar.gz');
  assert.equal(
    checksumName('0.1.9', 'windows-x86_64'),
    'Cinyuverse-0.1.9-windows-x86_64-server.tar.gz.sha256'
  );
  assert.equal(versionFromTag('v0.1.9'), '0.1.9');
  assert.equal(
    releaseBaseUrl('LingByte/CinyuVerse', 'v0.1.9'),
    'https://github.com/LingByte/CinyuVerse/releases/download/v0.1.9'
  );
});
...
test('npx, install.sh, and install.ps1 all ask for the CI archive name', () => {
  ...
  for (const platform of PLATFORMS) {
    ...
    assert.equal(
      ...
      `https://github.com/LingByte/CinyuVerse/releases/download/${tag}/${archive}`
    );
  ...
  assert.match(installSh, /archive="Cinyuverse-\$\{version\}-\$\{platform\}-server\.tar\.gz"/);
  ...
  assert.match(installPs1, /\$archive = "Cinyuverse-\$version-\$platform-server\.tar\.gz"/);
  for (const platform of PLATFORMS) {
...
test('the release workflow derives the archive name from the naming module', () => {
  ...
  assert.doesNotMatch(workflow, /ARCHIVE="Cinyuverse-\$\{VERSION\}-/);
  for (const platform of PLATFORMS) {
