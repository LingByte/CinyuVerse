...
function withTauriBuildEnv(env, target) {
  return {
    ...
    ...(target ? { CINYUVERSE_BUILD_TARGET: target } : {}),
  };
}

function withCefHelperCargoArgs(args) {
  const helperArgs = ["--bin", "cinyuverse_cef_helper"];
  ...
  if (
    cargoArgs.some(
      (arg, index) =>
        arg === "--bin" && cargoArgs[index + 1] === "cinyuverse_cef_helper",
    )
  ) {
    return args;
  }
...
const releaseConfig = process.env.CINYUVERSE_TAURI_RELEASE_CONFIG;
const configArgs =
