...
test('sidecar layout prefers CINYUVERSE_BUILD_TARGET and the active cargo profile', () => {
  const resolved = resolveSidecarBinDir({
    repo: '/repo',
    env: {
      ...
      CINYUVERSE_BUILD_TARGET: 'aarch64-pc-windows-msvc',
    },
...
test('sidecar rebuild is skipped only when both Host binaries exist', () => {
  const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'cinyuverse-sidecars-'));
  const env = {
    CARGO_TARGET_DIR: path.join(repo, 'target'),
    CINYUVERSE_BUILD_TARGET: 'x86_64-unknown-linux-gnu',
  ...
  fs.writeFileSync(path.join(binDir, 'cinyuverse-mcp'), '');
  fs.writeFileSync(path.join(binDir, 'cinyuverse-workflow-mcp'), '');
  assert.equal(
    sidecarBinsExist({
      repo,
      env,
