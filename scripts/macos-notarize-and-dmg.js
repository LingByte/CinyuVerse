...
function createDmg({ appPath, dmgPath, productName, identity }) {
  ...
  const staging = fs.mkdtempSync(path.join(os.tmpdir(), "cinyuverse-dmg-"));
  try {
...
function main(args = process.argv.slice(2), env = process.env) {
  ...
  const timeoutSeconds = Number(
    readOption("--timeout", args) || env.CINYUVERSE_NOTARY_TIMEOUT_SECONDS || DEFAULT_NOTARY_TIMEOUT_SECONDS,
  );
