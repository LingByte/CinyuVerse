...
function cargoTarget(env = process.env) {
  return env.CINYUVERSE_BUILD_TARGET || env.TAURI_ENV_TARGET_TRIPLE || null;
}
...
function sidecarBuildArgs({ debug, target }) {
  const args = ["build", "-p", "cinyuverse-mcp", "-p", "cinyuverse-workflow-mcp"];
  if (!debug) {
    args.push("--release");
  }
  if (target) {
...
function main(env = process.env) {
  ...
  const nextEnv = {
    ...
    CINYUVERSE_BUILD_TARGET: env.CINYUVERSE_BUILD_TARGET || target,
