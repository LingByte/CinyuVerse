...
test("maps Apple triples onto Tauri DMG file names", () => {
  ...
  assert.equal(
    ...
    path.join(
      workspaceRoot,
      "target/x86_64-apple-darwin/release/bundle/macos/Cinyuverse.app",
    ),
  );
  assert.equal(
    x64.dmgPath,
    path.join(
      workspaceRoot,
      "target/x86_64-apple-darwin/release/bundle/dmg/Cinyuverse_0.1.6_x64.dmg",
    ),
  );
  assert.equal(
    arm.dmgPath,
    path.join(
      workspaceRoot,
      "target/aarch64-apple-darwin/release/bundle/dmg/Cinyuverse_0.1.6_aarch64.dmg",
    ),
  );
});
