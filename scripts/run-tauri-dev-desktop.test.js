...
test('desktop dev uses a distinct app identity from the installed product', () => {
  ...
  assert.equal(product.identifier, 'com.cinyuverse.app');
  assert.equal(product.productName, 'Cinyuverse');
  assert.deepEqual(product.plugins['deep-link'].desktop.schemes, ['cinyuverse']);
