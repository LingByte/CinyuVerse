...
const executable = path.join(appBundle, "Contents", "MacOS", "cinyuverse");
const smokeConfig = JSON.stringify({
  identifier: "com.cinyuverse.cef-smoke",
...
async function main() {
  ...
  const child = spawn(executable, [], {
    cwd: workspaceRoot,
    detached: true,
    env: { ...process.env, RUST_LOG: "browser_cef=debug,cinyuverse=info" },
  ...
  try {
    ...
    while (Date.now() < deadline) {
      if (child.exitCode !== null) {
        throw new Error(`Cinyuverse exited before CEF initialized.\n${output}`);
      }
      const processes = processList();
      if (
        processes.includes(`${appBundle}/Contents/Frameworks/`) &&
        processes.includes("cinyuverse Helper") &&
