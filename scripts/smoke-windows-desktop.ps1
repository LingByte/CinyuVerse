...
if ((Get-PeSubsystem $Executable) -ne 2) {
  throw 'Cinyuverse must use IMAGE_SUBSYSTEM_WINDOWS_GUI and must not allocate a console.'
}
...
public static class CinyuverseWindowInspector {
...
try {
  ...
  if ($process.HasExited) {
    throw "Cinyuverse exited during the Windows startup smoke test with code $($process.ExitCode)."
  }

  $consoleOwners = [CinyuverseWindowInspector]::ConsoleWindowOwners()
  foreach ($owner in $consoleOwners) {
    if (Test-DescendsFrom $owner ([uint32]$process.Id)) {
      throw "Cinyuverse spawned a visible console window owned by process $owner."
    }
  }
}
finally {
