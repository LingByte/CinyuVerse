---
name: cinyuverse-plugin-development
description: Build, migrate, test, link, debug, or package full-trust Cinyuverse v4 product plugins with README summaries, contents, root config, dependencies, App surfaces, editable file tabs, Agent Skills, workflows, MCP, Workers, and Runtimes. Use for .cinyuverse-plugin/plugin.json, @cinyuverse/plugin-sdk, cinyuverse-plugin CLI, or linked plugin development.
---

# Cinyuverse Plugin Development
...
The Host product plugin (Skill `/create-skill` and contract references) lives in the git submodule `assets/plugins/plugin-development` (`https://github.com/LingByte/cinyuverse-plugin-development`). Edit that repository for the user-facing Skill. This Skill is the in-tree authoring procedure against the checked-out SDK and CLI.

## Load the local contract

1. Run `python3 .agents/skills/cinyuverse-plugin-development/scripts/locate_toolchain.py` from the Cinyuverse repository root.
...
- Put Agent/user-readable resources under `contents/` and index them in `.cinyuverse-plugin/content.index.json`.
...
Use only `@cinyuverse/plugin-sdk`, `/worker`, `/app`, `/testing`, `/protocol`, and `/stdio`. If the public SDK cannot express the product, add a generic Host capability, expose it through the SDK, test it, and then consume it from the plugin.

## Develop against Full Trust

Cinyuverse v4 executes installed plugin code with the user's trust. There is no permission grant step. Keep package identity, deterministic digest, candidate rollback, revision conflict handling, and lifecycle cleanup intact because they protect correctness and user data. Follow [references/full-trust.md](references/full-trust.md).
...
cinyuverse plugin publish
```

Prefer `cinyuverse plugin add --dev .` against a running Desktop or `cinyuverse serve`.
`cinyuverse-plugin install --link` is only an alias of that Host import when it still exists.

The product CLI also exposes `cinyuverse plugin pack` and `cinyuverse plugin publish`.

When developing Cinyuverse itself, build the local SDK and CLI and invoke `node <repo>/packages/plugin-cli/dist/cli.js`. Verify a real linked install and reload against the running Host; harness-only success is insufficient for App mounting, file editing, Runtime processes, or remote behavior.

## Completion gate

- The plugin imports only public SDK modules and contains no Cinyuverse-source special case.
